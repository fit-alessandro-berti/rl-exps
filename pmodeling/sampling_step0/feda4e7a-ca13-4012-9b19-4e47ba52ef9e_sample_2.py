...
rob_sort = Transition(label='Robotic Sort')
...
root = StrictPartialOrder(nodes=[
    site_analysis, env_scanning, farm_design, nutrient_mix, seed_automation,
    growth_monitor, pest_control, ai_diagnostics, harvest_plan, rob_sort,
    packaging_line, community_input, data_agg, waste_recycle, sustainability
])
...
root.order.add_edge(harvest_plan, rob_sort)
root.order.add_edge(rob_sort, packaging_line)
root.order.add_edge(packaging_line, community_input)
root.order.add_edge(community_input, data_agg)
root.order.add_edge(data_agg, waste_recycle)
