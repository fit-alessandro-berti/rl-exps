...
# Define dependencies
partial_order.order.add_edge(verify_provenance, assess_condition)
partial_order.order.add_edge(assess_condition, negotiate_terms)
partial_order.order.add_edge(negotiate_terms, arrange_transport)
partial_order.order.add_edge(arrange_transport, customs_clearance)
partial_order.order.add_edge(customs_clearance, secure_insurance)
partial_order.order.add_edge(secure_insurance, schedule_handlers)
partial_order.order.add_edge(schedule_handlers, install_artwork)
partial_order.order.add_edge(install_artwork, monitor_climate)
partial_order.order.add_edge(monitor_climate, manage_security)
partial_order.order.add_edge(manage_security, facilitate_access)
partial_order.order.add_edge(facilitate_access, document_display)
partial_order.order.add_edge(document_display, coordinate_events)
partial_order.order.add_edge(coordinate_events, inspect_periodically)
partial_order.order.add_edge(inspect_periodically, plan_return)
partial_order.order.add_edge(plan_return, deinstall_artwork)
partial_order.order.add_edge(deinstall_artwork, finalize_reports)
