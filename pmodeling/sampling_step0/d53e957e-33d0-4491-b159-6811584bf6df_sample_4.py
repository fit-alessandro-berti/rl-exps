...
loop1 = OperatorPOWL(operator=Operator.LOOP, children=[regulation_check, ip_alignment, supply_sync])
loop2 = OperatorPOWL(operator=Operator.LOOP, children=[market_mapping, pilot_launch])
loop3 = OperatorPOWL(operator=Operator.LOOP, children=[feedback_loop, design_iterate])
loop4 = OperatorPOWL(operator=Operator.LOOP, children=[impact_measure, strategy_adapt])

# Define the partial order
root = StrictPartialOrder(nodes=[data_capture, trend_scan, idea_workshop, concept_draft, expert_review, prototype_build, loop1, loop2, loop3, loop4])
