...
# Define loop nodes
loop1 = OperatorPOWL(operator=Operator.LOOP, children=[quality_testing, milk_pasturize])
loop2 = OperatorPOWL(operator=Operator.LOOP, children=[sensory_review, custom_packaging])

# Add loop nodes to the partial order
root.nodes.extend([loop1, loop2])

# Add dependencies between loop nodes and other nodes
root.order.add_edge(loop1, cheese_crafting)
root.order.add_edge(loop2, controlled_aging)
root.order.add_edge(cheese_crafting, sensory_review)
root.order.add_edge(controlled_aging, custom_packaging)
root.order.add_edge(sensory_review, label_printing)
root.order.add_edge(custom_packaging, export_licensing)
root.order.add_edge(export_licensing, documentation_prep)
root.order.add_edge(documentation_prep, customs_clearance)
root.order.add_edge(customs_clearance, cold_shipping)
root.order.add_edge(cold_shipping, delivery_tracking)
