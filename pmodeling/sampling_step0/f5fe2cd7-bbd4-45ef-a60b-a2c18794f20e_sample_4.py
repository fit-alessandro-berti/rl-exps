...
# Define the exclusive choice (XOR) for different outcomes
xor = OperatorPOWL(operator=Operator.XOR, children=[discrepancy_flag, skip])

# Define the loop for re-examination and alternative sourcing
loop = OperatorPOWL(operator=Operator.LOOP, children=[re_examination, alternative_source])

# Define the partial order (POWL model)
root = StrictPartialOrder(nodes=[provenance_check, material_scan, radiocarbon_test, stylistic_review, expert_consult, document_audit, legal_verify, condition_report, xor, loop, acquisition_vote, catalog_entry, exhibit_plan, final_approval])
...
root.order.add_edge(xor, acquisition_vote)
