...
# Define the loop node for data collection and yield analysis
loop = OperatorPOWL(operator=Operator.LOOP, children=[data_collection, yield_analysis])

# Define the exclusive choice for pest control and feedback loop
xor = OperatorPOWL(operator=Operator.XOR, children=[pest_control, feedback_loop])

# Define the partial order with all the nodes and dependencies
root = StrictPartialOrder(nodes=[site_audit, impact_study, design_modules, sensor_setup, hydroponics_install, nutrient_test, lighting_config, staff_training, loop, xor, packaging_prep, market_delivery])
...
root.order.add_edge(feedback_loop, site_audit)
