...
# Define operators
exclusive_choice = OperatorPOWL(operator=Operator.XOR, children=[demand_forecast, skip])
loop = OperatorPOWL(operator=Operator.LOOP, children=[exclusive_choice, courier_match, credential_check, route_design, load_assign, traffic_scan, package_secure, dispatch_alert, real_time_track, incentive_issue, dispute_review, customer_notify, feedback_collect, performance_log, ledger_update, hub_sync])

# Define root
root = StrictPartialOrder(nodes=[loop])
root.order.add_edge(loop, exclusive_choice)
root.order.add_edge(exclusive_choice, courier_match)
