...
loop1 = OperatorPOWL(operator=Operator.LOOP, children=[climate_control, sensor_deploy])
loop2 = OperatorPOWL(operator=Operator.LOOP, children=[pest_control, growth_monitor])
loop3 = OperatorPOWL(operator=Operator.LOOP, children=[harvest_plan, packaging_prep])
loop4 = OperatorPOWL(operator=Operator.LOOP, children=[delivery_route, data_analysis])
loop5 = OperatorPOWL(operator=Operator.LOOP, children=[yield_forecast, skip])

# Define partial order
root = StrictPartialOrder(nodes=[site_survey, structure_check, hydroponic_install, lighting_setup, loop1, loop2, loop3, loop4, loop5])
