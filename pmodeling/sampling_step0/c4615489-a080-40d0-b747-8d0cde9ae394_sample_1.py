...
xor = OperatorPOWL(operator=Operator.XOR, children=[waste_recycling, customer_onboarding])
loop = OperatorPOWL(operator=Operator.LOOP, children=[install_hydroponics, configure_sensors])

# Create the root POWL model
root = StrictPartialOrder(nodes=[site_survey, structural_audit, permit_filing, design_layout, install_hvac, set_lighting, build_racks, loop, select_crops, seed_planting, monitor_growth, nutrient_mixing, staff_training, market_launch, xor])
...
root.order.add_edge(build_racks, loop)
...
root.order.add_edge(market_launch, xor)
root.order.add_edge(xor, waste_recycling)
root.order.add_edge(xor, customer_onboarding)
