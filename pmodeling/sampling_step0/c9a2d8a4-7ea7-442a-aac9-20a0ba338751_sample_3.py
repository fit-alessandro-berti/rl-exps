...
loop = OperatorPOWL(operator=Operator.LOOP, children=[cross_verify_data, analyze_reports])
xor = OperatorPOWL(operator=Operator.XOR, children=[identify_gaps, recommend_actions])

# Define the root partial order
root = StrictPartialOrder(nodes=[initiate_audit, gather_documents, verify_suppliers, screen_transactions, classify_products, assess_risks, check_sanctions, review_tariffs, loop, xor, finalize_report])
...
root.order.add_edge(review_tariffs, loop)
