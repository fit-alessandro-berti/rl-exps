...
# Define the loop and XOR operations
loop = OperatorPOWL(operator=Operator.LOOP, children=[climate_control, nutrient_mix, data_monitoring])
xor = OperatorPOWL(operator=Operator.XOR, children=[pest_scouting, waste_sorting, energy_audit])

# Define the root model
root = StrictPartialOrder(nodes=[site_survey, structural_audit, system_design, permit_filing, foundation_prep, frame_build, irrigation_setup, lighting_install, climate_control, nutrient_mix, crop_planting, pest_scouting, data_monitoring, waste_sorting, energy_audit, community_meet, yield_analysis])
...
# Add the loop and XOR operations to the root model
root.order.add_edge(loop, xor)
root.order.add_edge(xor, yield_analysis)