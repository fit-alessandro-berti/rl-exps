...
# Define the loop structure for testing and analysis
loop = OperatorPOWL(operator=Operator.LOOP, children=[field_testing, test_analysis])

# Define the exclusive choice for software setup and data integration
xor = OperatorPOWL(operator=Operator.XOR, children=[software_setup, data_integration])

# Define the main process flow
root = StrictPartialOrder(nodes=[client_meet, requirement_gather, module_design, supplier_vetting, component_order, prototype_build, loop, xor, software_setup, data_integration, pilot_train, compliance_check, fleet_deploy, remote_monitor, maintenance_plan, performance_tune])
...
root.order.add_edge(xor, software_setup)
