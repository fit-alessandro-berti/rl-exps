...
# Choice operator for selecting between two activities
choice = OperatorPOWL(operator=Operator.XOR, children=[pest_control, data_logging])
# Loop operator for hive health checks
loop = OperatorPOWL(operator=Operator.LOOP, children=[health_check, choice])
# Strict Partial Order with defined transitions
root = StrictPartialOrder(nodes=[site_survey, flora_mapping, hive_setup, sensor_install, loop, data_logging, community_meet, workshop_plan, honey_extract, quality_test, packaging, market_setup, sales_report, regulation_check, waste_manage, seasonal_review])
