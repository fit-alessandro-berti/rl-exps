...
loop_audit = OperatorPOWL(operator=Operator.LOOP, children=[
    initiate_audit,
    gather_documents,
    verify_suppliers,
    screen_transactions,
...
xor_audit = OperatorPOWL(operator=Operator.XOR, children=[
    loop_audit,
    skip
])

root = StrictPartialOrder(nodes=[xor_audit])
root.order.add_edge(xor_audit, loop_audit)