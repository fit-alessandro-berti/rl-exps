...
# Define the loops
verify_loop = OperatorPOWL(operator=Operator.LOOP, children=[verify_suppliers, screen_transactions, classify_products, assess_risks, check_sanctions, review_tariffs, cross_verify_data])
interview_loop = OperatorPOWL(operator=Operator.LOOP, children=[conduct_interviews, analyze_reports, identify_gaps, recommend_actions, implement_changes])

# Define the partial order
root = StrictPartialOrder(nodes=[initiate_audit, gather_documents, verify_loop, interview_loop, finalize_report])
root.order.add_edge(initiate_audit, gather_documents)
root.order.add_edge(gather_documents, verify_loop)
root.order.add_edge(verify_loop, interview_loop)
root.order.add_edge(interview_loop, finalize_report)