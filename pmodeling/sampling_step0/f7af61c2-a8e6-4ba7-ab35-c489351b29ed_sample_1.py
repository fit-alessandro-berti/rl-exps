...
loop1 = OperatorPOWL(operator=Operator.LOOP, children=[sensor_setup, health_monitor, light_adjust, irrigation_mod, waste_recycle, energy_optimize])
loop2 = OperatorPOWL(operator=Operator.LOOP, children=[quality_check, crop_harvest, distribution_plan])
xor = OperatorPOWL(operator=Operator.XOR, children=[env_control, skip])

# Construct the root POWL model
root = StrictPartialOrder(nodes=[site_analysis, structure_check, farm_design, loop1, loop2, xor])
root.order.add_edge(site_analysis, structure_check)
root.order.add_edge(structure_check, farm_design)
root.order.add_edge(farm_design, loop1)
root.order.add_edge(loop1, sensor_setup)
...
root.order.add_edge(distribution_plan, sensor_setup)
root.order.add_edge(sensor_setup, health_monitor)
root.order.add_edge(health_monitor, light_adjust)
root.order.add_edge(light_adjust, irrigation_mod)
root.order.add_edge(irrigation_mod, waste_recycle)
