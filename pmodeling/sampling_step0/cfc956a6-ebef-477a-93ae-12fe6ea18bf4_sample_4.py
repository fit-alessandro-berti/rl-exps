...
# Define the exclusive choice (XOR) for bidding
xor = OperatorPOWL(operator=Operator.XOR, children=[bid_submit, skip])

# Define the loop for price adjustment
loop = OperatorPOWL(operator=Operator.LOOP, children=[price_adjust, smart_deploy])

# Define the partial order
root = StrictPartialOrder(nodes=[artist_onboard, asset_verify, identity_check, smart_deploy, bid_monitor, loop, xor, auction_close, ownership_transfer, fund_release, dispute_review, reputation_update, fractional_offer, secondary_sale])
...
root.order.add_edge(loop, price_adjust)
root.order.add_edge(price_adjust, smart_deploy)
root.order.add_edge(smart_deploy, bid_monitor)
root.order.add_edge(bid_monitor, xor)
root.order.add_edge(xor, bid_submit)
root.order.add_edge(xor, skip)
root.order.add_edge(skip, smart_deploy)
root.order.add_edge(skip, auction_close)
...
root.order.add_edge(secondary_sale, auction_close)
