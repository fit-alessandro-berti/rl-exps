...
# Define the loops
curd_aging_loop = OperatorPOWL(operator=Operator.LOOP, children=[controlled_aging, sensory_check])
whey_aging_loop = OperatorPOWL(operator=Operator.LOOP, children=[controlled_aging, sensory_check])

# Define the partial order
root = StrictPartialOrder(nodes=[milk_sourcing, quality_testing, milk_pasturize, curd_formation, whey_separation, press_cheese, salt_application, curd_aging_loop, whey_aging_loop, batch_packaging, label_printing, cold_storage, logistics_plan, retail_delivery, feedback_review, demand_forecast, provenance_track])
...
root.order.add_edge(salt_application, curd_aging_loop)
root.order.add_edge(curd_aging_loop, sensory_check)
