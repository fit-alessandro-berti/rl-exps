...
# Define the transitions for the loop and exclusive choice
loop = OperatorPOWL(operator=Operator.LOOP, children=[monitor_growth, manage_pests])
xor = OperatorPOWL(operator=Operator.XOR, children=[analyze_data, local_delivery])

# Define the partial order
root = StrictPartialOrder(nodes=[site_survey, zoning_check, design_layout, system_order, structure_build, install_hydroponics, calibrate_sensors, select_crops, plant_seeding, loop, xor])
...
root.order.add_edge(loop, monitor_growth)
root.order.add_edge(loop, manage_pests)
root.order.add_edge(monitor_growth, analyze_data)
root.order.add_edge(manage_pests, local_delivery)
root.order.add_edge(analyze_data, schedule_harvest)
root.order.add_edge(local_delivery, package_produce)
