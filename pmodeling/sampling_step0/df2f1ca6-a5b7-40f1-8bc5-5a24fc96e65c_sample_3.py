...
loop = OperatorPOWL(operator=Operator.LOOP, children=[quality_check, custom_packaging])

# Define the exclusive choice node
xor = OperatorPOWL(operator=Operator.XOR, children=[demand_forecast, price_adjust])

# Define the partial order
root = StrictPartialOrder(nodes=[material_sourcing, artisan_vetting, sample_review, design_finalize, batch_scheduling, loop, xor, inventory_sync, order_processing, craft_coordination, shipment_plan, market_analysis, feedback_loop, trend_monitor])
...
root.order.add_edge(custom_packaging, inventory_sync)
...
root.order.add_edge(trend_monitor, demand_forecast)
