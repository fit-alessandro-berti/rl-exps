...
# Define the workflow
loop = OperatorPOWL(operator=OperatorPOWL.LOOP, children=[flight_launch, traffic_monitor, weather_assess, obstacle_avoid, battery_check, delivery_verify, biometric_scan, package_release, return_flight])
xor = OperatorPOWL(operator=OperatorPOWL.XOR, children=[post_flight, skip])
root = StrictPartialOrder(nodes=[order_validate, route_optimize, drone_assign, loop, xor])

# Add dependencies
root.order.add_edge(order_validate, route_optimize)
root.order.add_edge(route_optimize, drone_assign)
root.order.add_edge(drone_assign, flight_launch)
...
root.order.add_edge(post_flight, xor)
