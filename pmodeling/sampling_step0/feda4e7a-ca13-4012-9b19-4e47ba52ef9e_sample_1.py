...
loop = OperatorPOWL(operator=Operator.LOOP, children=[seed_automation, growth_monitor])
xor = OperatorPOWL(operator=Operator.XOR, children=[pest_control, ai_diagnostics])
xor2 = OperatorPOWL(operator=Operator.XOR, children=[data_aggregation, waste_recycle])

# Define the root POWL model
root = StrictPartialOrder(nodes=[site_analysis, env_scanning, farm_design, nutrient_mix, loop, xor, xor2, robotic_sort, packaging_line, community_input, sustainability])
...
root.order.add_edge(env_scanning, nutrient_mix)
...
root.order.add_edge(pest_control, data_aggregation)
root.order.add_edge(pest_control, waste_recycle)
root.order.add_edge(ai_diagnostics, data_aggregation)
root.order.add_edge(ai_diagnostics, waste_recycle)
root.order.add_edge(data_aggregation, sustainability)
root.order.add_edge(waste_recycle, sustainability)
root.order.add_edge(sustainability, robotic_sort)
root.order.add_edge(sustainability, packaging_line)
root.order.add_edge(sustainability, community_input)
root.order.add_edge(sustainability, harvest_plan)
root.order.add_edge(harvest_plan, robotic_sort)
root.order.add_edge(harvest_plan, packaging_line)
root.order.add_edge(harvest_plan, community_input)
root.order.add_edge(community_input, sustainability)
root.order.add_edge(community_input, harvest_plan)
