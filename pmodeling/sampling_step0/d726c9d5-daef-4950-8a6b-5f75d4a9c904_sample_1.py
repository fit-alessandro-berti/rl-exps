...
# Define the exclusive choice for data integration and community meet
xor = OperatorPOWL(operator=Operator.XOR, children=[data_integration, community_meet])

# Define the loop for training sessions
loop = OperatorPOWL(operator=Operator.LOOP, children=[training_session])

# Define the root model
root = StrictPartialOrder(nodes=[site_survey, structure_check, soil_sample, water_test, crop_selection, material_order, planter_setup, irrigation_install, sensor_deploy, solar_setup, xor, loop, yield_monitor, adjust_plan])
...
root.order.add_edge(loop, training_session)
root.order.add_edge(training_session, data_integration)
root.order.add_edge(training_session, community_meet)
root.order.add_edge(data_integration, yield_monitor)
root.order.add_edge(community_meet, training_session)
root.order.add_edge(yield_monitor, adjust_plan)
root.order.add_edge(adjust_plan, training_session)
