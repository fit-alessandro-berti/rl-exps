...
xor = OperatorPOWL(operator=Operator.XOR, children=[revision_cycle, final_approval])
...
root = StrictPartialOrder(nodes=[client_inquiry, requirement_gather, art_creation, progress_update, quality_check, final_adjust, invoice_issue, shipment_prep, delivery_confirm, post_support, license_setup, frame_arrange])
root.order.add_edge(client_inquiry, requirement_gather)
root.order.add_edge(requirement_gather, art_creation)
...
root.order.add_edge(final_adjust, xor)
root.order.add_edge(xor, xor2)
root.order.add_edge(xor2, xor3)
root.order.add_edge(xor3, xor4)
root.order.add_edge(xor4, root)

root.order.add_edge(client_inquiry, loop)
