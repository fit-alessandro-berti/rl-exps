...
loop1 = OperatorPOWL(operator=Operator.LOOP, children=[install_sensors, calibrate_climate, select_seeds, optimize_nutrients, deploy_robots, monitor_growth, detect_pests, analyze_data, harvest_crops, customize_pack, recycle_waste, audit_energy, align_demand])

xor1 = OperatorPOWL(operator=Operator.XOR, children=[site_survey, design_modules])

root = StrictPartialOrder(nodes=[xor1, loop1])
