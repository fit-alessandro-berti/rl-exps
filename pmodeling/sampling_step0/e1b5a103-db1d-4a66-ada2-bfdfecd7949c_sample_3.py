...
# Add an XOR to allow the process to loop back to data collection and feedback
xor = OperatorPOWL(operator=Operator.XOR, children=[data_collection, cycle_feedback])
root.order.add_edge(data_collection, xor)
root.order.add_edge(cycle_feedback, xor)
