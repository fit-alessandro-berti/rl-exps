...
loop_sensor = OperatorPOWL(operator=Operator.LOOP, children=[data_logging])
choice_sensors = OperatorPOWL(operator=Operator.XOR, children=[sensor_install, skip])

# Define partial order
root = StrictPartialOrder(nodes=[
    site_survey, flora_mapping, hive_setup, choice_sensors, health_check, pest_control, loop_sensor,
...
root.order.add_edge(hive_setup, choice_sensors)
root.order.add_edge(choice_sensors, health_check)
root.order.add_edge(health_check, pest_control)
root.order.add_edge(pest_control, loop_sensor)
root.order.add_edge(loop_sensor, data_logging)
