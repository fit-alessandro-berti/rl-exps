...
# The structure of the process is complex and requires multiple choices and loops to model the various stages of the project.
# For simplicity, let's assume that each activity can be performed by a single person, and that each activity is independent of the others.
# This means that each activity can be executed in any order, and that the loop is not necessary.

# Create the POWL model
root = StrictPartialOrder(nodes=[site_survey, load_test, permit_review, design_layout, material_sourcing, soil_prep, hydroponic_setup, community_meet, crop_select, sensor_install, water_testing, pest_control, growth_monitor, harvest_plan, market_launch, feedback_collect])
