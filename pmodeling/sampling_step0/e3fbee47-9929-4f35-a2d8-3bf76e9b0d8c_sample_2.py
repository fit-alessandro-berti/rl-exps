...
xor_packaging_retail = OperatorPOWL(operator=Operator.XOR, children=[packaging_design, label_approval, retail_setup])

# Define the partial order for the entire process
root = StrictPartialOrder(nodes=[farm_selection, milk_testing, batch_pasteurize, culture_add, curd_cut, whey_drain, mold_inoculate, press_form, salt_rub, aging_monitor, flavor_adjust, loop_aging, packaging_design, label_approval, order_processing, cold_storage, delivery_schedule, retail_setup, feedback_collect])
...
root.order.add_edge(flavor_adjust, loop_aging)
root.order.add_edge(loop_aging, packaging_design)
root.order.add_edge(loop_aging, label_approval)
...
# Add the edges for the XOR
root.order.add_edge(packaging_design, xor_packaging_retail)
root.order.add_edge(label_approval, xor_packaging_retail)
root.order.add_edge(retail_setup, xor_packaging_retail)

# Add the edges for the loop
root.order.add_edge(aging_monitor, flavor_adjust)
root.order.add_edge(flavor_adjust, aging_monitor)
...
root.order.add_edge(whey_drain, mold_inoculate)
root.order.add_edge(mold_inoculate, press_form)
root.order.add_edge(press_form, salt_r