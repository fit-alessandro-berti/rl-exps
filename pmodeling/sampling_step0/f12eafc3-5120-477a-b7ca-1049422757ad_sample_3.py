...
controlled_aging_loop = OperatorPOWL(operator=Operator.LOOP, children=[controlled_aging, skip1])
controlled_aging_loop.order.add_edge(controlled_aging, skip1)

# Define the exclusive choice
exclusive_choice = OperatorPOWL(operator=Operator.XOR, children=[sensory_review, skip2])

# Define the main process
root = StrictPartialOrder(nodes=[milk_selection, quality_testing, milk_pasteurize, cheese_crafting, controlled_aging_loop, exclusive_choice, custom_packaging, label_printing, export_licensing, documentation_prep, customs_clearance, cold_shipping, delivery_tracking, feedback_review, market_analysis])
...
root.order.add_edge(cheese_crafting, controlled_aging_loop)
root.order.add_edge(controlled_aging_loop, exclusive_choice)
root.order.add_edge(exclusive_choice, custom_packaging)
