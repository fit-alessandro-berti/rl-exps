...
loop1 = OperatorPOWL(operator=Operator.LOOP, children=[soil_testing, system_design, seed_selection, irrigation_setup, lighting_install, pest_control])
loop2 = OperatorPOWL(operator=Operator.LOOP, children=[community_meet, regulation_review, waste_plan, crop_monitor, harvest_prep, market_launch])
xor = OperatorPOWL(operator=Operator.XOR, children=[loop1, loop2])
root = StrictPartialOrder(nodes=[site_survey, impact_study, structure_check, xor])
