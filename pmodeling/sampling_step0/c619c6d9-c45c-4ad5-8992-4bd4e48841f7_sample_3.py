...
loop = OperatorPOWL(operator=Operator.LOOP, children=[sensor_calibrate, firmware_integrate])
xor = OperatorPOWL(operator=Operator.XOR, children=[regulation_check, skip])

# Define partial order
root = StrictPartialOrder(nodes=[design_consult, component_sourcing, loop, xor, payload_configure, assembly_setup, wiring_connect, chassis_build, software_load, flight_testing, data_analyze, regulation_check, quality_inspect, packaging_prep, logistics_plan, client_review])

# Add edges to the partial order
root.order.add_edge(design_consult, component_sourcing)
root.order.add_edge(component_sourcing, loop)
...
root.order.add_edge(regulation_check, xor)
