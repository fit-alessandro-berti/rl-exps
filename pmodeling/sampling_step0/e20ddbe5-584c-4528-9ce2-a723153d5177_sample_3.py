import pm4py
from pm4py.objects.powl.obj import StrictPartialOrder, OperatorPOWL, Transition, SilentTransition
from pm4py.objects.process_tree.obj import Operator
site_analysis = Transition(label='Site Analysis')
load_test = Transition(label='Load Test')
sunlight_map = Transition(label='Sunlight Map')
medium_select = Transition(label='Medium Select')
hydro_design = Transition(label='Hydro Design')
procure_seeds = Transition(label='Procure Seeds')
install_irrigation = Transition(label='Install Irrigation')
setup_climate = Transition(label='Setup Climate')
create_schedule = Transition(label='Create Schedule')
pest_control = Transition(label='Pest Control')
monitor_growth = Transition(label='Monitor Growth')
adjust_systems = Transition(label='Adjust Systems')
harvest_crops = Transition(label='Harvest Crops')
package_produce = Transition(label='Package Produce')
engage_community = Transition(label='Engage Community')
host_workshops = Transition(label='Host Workshops')
skip = SilentTransition()
xor = OperatorPOWL(operator=Operator.XOR, children=[harvest_crops, skip])
loop = OperatorPOWL(operator=Operator.LOOP, children=[monitor_growth, adjust_systems])
root = StrictPartialOrder(nodes=[site_analysis, load_test, sunlight_map, medium_select, hydro_design, procure_seeds, install_irrigation, setup_climate, create_schedule, pest_control, loop, xor, engage_community, host_workshops])
root.order.add_edge(site_analysis, load_test)
root.order.add_edge(site_analysis, sunlight_map)
root.order.add_edge(load_test, medium_select)
root.order.add_edge(load_test, hydro_design)
root.order.add_edge(medium_select, procure_seeds)
root.order.add_edge(medium_select, install_irrigation)
root.order.add_edge(medium_select, setup_climate)
root.order.add_edge(hydro_design, create_schedule)
root.order.add_edge(create_schedule, pest_control)
root.order.add_edge(pest_control, loop)
root.order.add_edge(loop, monitor_growth)
root.order.add_edge(loop, adjust_systems)
root.order.add_edge(pest_control, xor)
root.order.add_edge(xor, harvest_crops)
root.order.add_edge(xor, package_produce)
root.order.add_edge(harvest_crops, engage_community)
root.order.add_edge(harvest_crops, host_workshops)
root.order.add_edge(engage_community, host_workshops)