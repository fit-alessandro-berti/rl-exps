...
pour_egg_yolk_cheese = Transition(label='Pour egg yolk and cheese mixture')
saute_pancetta = Transition(label='Saute pancetta until crispy')
whisk_egg_yolk_parmesan = Transition(label='Whisk egg yolks, Parmesan, and black pepper')

# Define the partial order
root = StrictPartialOrder(nodes=[
    boil_water, boil_spaghetti, enjoy_spaghetti, gather_ingredients, 
    mix_with_eggs_and_pancetta, plan_cook, plate_garnish, 
    pour_egg_yolk_cheese, saute_pancetta, whisk_egg_yolk_parmesan
])
...
root.order.add_edge(mix_with_eggs_and_pancetta, plan_cook)
root.order.add_edge(plan_cook, plate_garnish)
root.order.add_edge(plate_garnish, pour_egg_yolk_cheese)
root.order.add_edge(pour_egg_yolk_cheese, saute_pancetta)
root.order.add_edge(saute_pancetta, whisk_egg_yolk_parmesan)