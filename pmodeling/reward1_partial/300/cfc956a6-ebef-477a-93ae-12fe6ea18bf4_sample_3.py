...
check_identity = Transition(label='Identity Check')
...
root = StrictPartialOrder(nodes=[artist_onboard, verify_asset, check_identity, deploy_smart_contract, monitor_bids, adjust_price, link_wallet, submit_bid, close_auction, transfer_ownership, release_funds, review_dispute, update_reputation, offer_fractional, secondary_sale])

# Define dependencies
root.order.add_edge(artist_onboard, verify_asset)
root.order.add_edge(verify_asset, check_identity)
root.order.add_edge(check_identity, deploy_smart_contract)
