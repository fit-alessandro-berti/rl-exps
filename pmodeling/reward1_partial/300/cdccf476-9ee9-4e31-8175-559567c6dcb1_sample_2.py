...
# Define silent transitions (no-op)
skip = SilentTransition()

# Define loop nodes
aging_loop = OperatorPOWL(operator=Operator.LOOP, children=[aging_control, quality_check])
inventory_loop = OperatorPOWL(operator=Operator.LOOP, children=[inventory_log, temp_monitoring])
carrier_loop = OperatorPOWL(operator=Operator.LOOP, children=[carrier_booking, trace_recording])

# Define choice nodes
feed_back_choice = OperatorPOWL(operator=Operator.XOR, children=[feedback_review, skip])
audit_choice = OperatorPOWL(operator=Operator.XOR, children=[compliance_audit, skip])

# Define the root POWL model
root = StrictPartialOrder(nodes=[milk_sourcing, curd_formation, microbial_test, whey_removal, pressing_cheese, salt_application, aging_loop, eco_packaging, inventory_loop, carrier_loop, feed_back_choice, audit_choice, batch_adjustment])
