...
loop = OperatorPOWL(operator=Operator.LOOP, children=[microbial_test, quality_check, eco_packaging, label_printing])
xor1 = OperatorPOWL(operator=Operator.XOR, children=[inventory_audit, order_processing])
xor2 = OperatorPOWL(operator=Operator.XOR, children=[retail_shipping, customer_feedback])
xor3 = OperatorPOWL(operator=Operator.XOR, children=[recipe_update, market_analysis])

root = StrictPartialOrder(nodes=[milk_sourcing, curd_preparation, starter_culture, temperature_control, pressing_cheese, salting_stage, aging_process, loop, xor1, xor2, xor3])
...
root.order.add_edge(market_analysis, loop)
