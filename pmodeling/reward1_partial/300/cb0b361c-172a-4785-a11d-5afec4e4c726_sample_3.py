...
loop1 = OperatorPOWL(operator=Operator.LOOP, children=[water_testing, nutrient_mix, seed_selection, planting_prep, growth_monitor, pest_inspect, harvest_plan])
loop2 = OperatorPOWL(operator=Operator.LOOP, children=[packaging_prep, distribution, sustainability])

xor = OperatorPOWL(operator=Operator.XOR, children=[loop1, loop2])

root = StrictPartialOrder(nodes=[site_survey, structural_audit, layout_design, system_install, climate_setup, xor])
