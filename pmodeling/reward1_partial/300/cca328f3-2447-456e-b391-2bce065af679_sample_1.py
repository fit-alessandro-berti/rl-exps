...
# Define the exclusive choice for expert review
expert_choice = OperatorPOWL(operator=Operator.XOR, children=[expert_review, lender_notify])
...
root = StrictPartialOrder(nodes=[artifact_intake, provenance_check, material_scan, style_compare, digital_capture, expert_choice, database_search, legal_audit, cultural_assess, loop_data_synthesis, report_draft, archival_store, display_approve, investigation_flag])
...
root.order.add_edge(digital_capture, expert_choice)
root.order.add_edge(expert_choice, database_search)
...
root.order.add_edge(loop_data_synthesis, report_draft)
