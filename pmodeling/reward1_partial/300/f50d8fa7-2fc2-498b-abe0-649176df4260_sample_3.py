...
xor = OperatorPOWL(operator=Operator.XOR, children=[compliance_check, community_meet])
xor2 = OperatorPOWL(operator=Operator.XOR, children=[market_launch, logistics_plan])
root = StrictPartialOrder(nodes=[site_survey, design_plan, permit_acquire, structural_retrofit, system_install, lighting_setup, sensor_deploy, seed_sourcing, nutrient_prep, staff_training, data_monitor, yield_analyze, xor, xor2])
...
root.order.add_edge(yield_analyze, xor)
root.order.add_edge(xor, xor2)
root.order.add_edge(xor2, market_launch)
root.order.add_edge(xor2, logistics_plan)
