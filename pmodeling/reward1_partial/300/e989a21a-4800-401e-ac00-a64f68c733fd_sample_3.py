...
# Define loop for data crosscheck
loop_data_crosscheck = OperatorPOWL(operator=Operator.LOOP, children=[data_crosscheck])

# Define XOR for expert consultation
xor_expert_consult = OperatorPOWL(operator=Operator.XOR, children=[expert_consult, final_approval])

# Define root POWL model
root = StrictPartialOrder(nodes=[visual_inspect, document_gather, material_test, pigment_analyze, style_compare, provenance_trace, loop_data_crosscheck, xor_expert_consult])
...
root.order.add_edge(provenance_trace, loop_data_crosscheck)
root.order.add_edge(loop_data_crosscheck, xor_expert_consult)
root.order.add_edge(xor_expert_consult, expert_consult)
root.order.add_edge(xor_expert_consult, final_approval)
