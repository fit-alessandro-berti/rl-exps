...
# Define the loop for nutrient mix, crop rotate, and water adjust
loop_nutrient_mix = OperatorPOWL(operator=Operator.LOOP, children=[nutrient_mix])
loop_crop_rotate = OperatorPOWL(operator=Operator.LOOP, children=[crop_rotate, water_adjust])

# Define the partial order with all activities
root = StrictPartialOrder(nodes=[sensor_setup, data_capture, loop_nutrient_mix, loop_crop_rotate, waste_collect, compost_process, drone_dispatch, pest_control, pollination_run, volunteer_assign, feedback_gather, model_update, yield_forecast, water_adjust, report_generate, resource_audit, schedule_sync])
root.order.add_edge(sensor_setup, data_capture)
root.order.add_edge(data_capture, loop_nutrient_mix)
root.order.add_edge(loop_nutrient_mix, loop_crop_rotate)
root.order.add_edge(loop_crop_rotate, waste_collect)
