...
# Assemble process steps
loop1 = OperatorPOWL(operator=Operator.LOOP, children=[firmware_load, calibration_run, stress_test])
loop2 = OperatorPOWL(operator=Operator.LOOP, children=[software_patch, algorithm_tune])
loop3 = OperatorPOWL(operator=Operator.LOOP, children=[comms_setup, validation_pass])
loop4 = OperatorPOWL(operator=Operator.LOOP, children=[data_link, onsite_deploy])

xor1 = OperatorPOWL(operator=Operator.XOR, children=[live_monitor, update_push])
xor2 = OperatorPOWL(operator=Operator.XOR, children=[recovery_plan, maintenance_log])

root = StrictPartialOrder(nodes=[component_check, spec_review, parts_sorting, mechanical_fit, loop1, loop2, loop3, loop4, xor1, xor2])
