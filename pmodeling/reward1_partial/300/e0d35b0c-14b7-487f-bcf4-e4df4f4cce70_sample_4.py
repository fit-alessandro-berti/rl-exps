...
# Define the loop for crop selection and planting
loop_crop_selection = OperatorPOWL(operator=Operator.LOOP, children=[crop_selection, planting_phase])

# Define the POWL model
root = StrictPartialOrder(nodes=[site_survey, structural_check, resource_sourcing, system_install, lighting_setup, irrigation_setup, stakeholder_activities, regulation_activities, loop_crop_selection, climate_control, growth_monitor, data_logging, harvest_event, waste_manage, feedback_collect])
...
root.order.add_edge(lighting_setup, stakeholder_activities)
root.order.add_edge(irrigation_setup, regulation_activities)
root.order.add_edge(stakeholder_activities, loop_crop_selection)
root.order.add_edge(regulation_activities, loop_crop_selection)
root.order.add_edge(loop_crop_selection, climate_control)
