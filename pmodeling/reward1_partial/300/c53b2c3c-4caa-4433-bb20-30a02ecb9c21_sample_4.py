...
# Define POWL model
loop1 = OperatorPOWL(operator=Operator.LOOP, children=[material_testing, style_compare, expert_consult, provenance_check, ownership_verify, legal_review, risk_assess, conservation_plan])
loop2 = OperatorPOWL(operator=Operator.LOOP, children=[dispute_resolve, final_archive])
xor = OperatorPOWL(operator=Operator.XOR, children=[secure_transfer, skip])

# Define the root of the POWL model
root = StrictPartialOrder(nodes=[discover_item, document_find, initial_survey, image_capture, loop1, xor, loop2])
