...
loop_initial_calibration = OperatorPOWL(operator=Operator.LOOP, children=[initial_calibration, firmware_loading, circuit_testing])
loop_performance_tuning = OperatorPOWL(operator=Operator.LOOP, children=[performance_tuning, data_logging])
xor_remote_monitoring = OperatorPOWL(operator=Operator.XOR, children=[remote_monitoring, firmware_update])

# Define the root of the POWL model
root = StrictPartialOrder(nodes=[component_sourcing, frame_assembly, sensor_mounting, wiring_harness, circuit_testing, firmware_loading, initial_calibration, software_integration, flight_testing, data_logging, performance_tuning, packaging_prep, custom_labeling, documentation_print, quality_review, client_training, remote_monitoring, firmware_update])
...
# Add the nested loops and XOR
root.order.add_edge(loop_initial_calibration, xor_remote_monitoring)
root.order.add_edge(loop_performance_tuning, xor_remote_monitoring)