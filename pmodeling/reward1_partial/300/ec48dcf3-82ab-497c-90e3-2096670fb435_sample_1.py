...
# Define a loop node for the planting phase and sensor deploy
loop_planting_sensor = OperatorPOWL(operator=Operator.LOOP, children=[planting_phase, sensor_deploy])

# Define exclusive choice for pest control and water monitor
xor_pest_water = OperatorPOWL(operator=Operator.XOR, children=[pest_control, water_monitor])

# Define the root Partial Order
root = StrictPartialOrder(nodes=[
    site_survey, design_layout, structure_build, system_install, climate_setup, nutrient_prep, seed_germinate,
    loop_planting_sensor, xor_pest_water, data_analyze, staff_train, yield_forecast, community_meet
])
...
root.order.add_edge(sensor_deploy, loop_planting_sensor)
root.order.add_edge(loop_planting_sensor, data_analyze)
