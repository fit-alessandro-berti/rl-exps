...
# Define transitions for loop (Quality Check)
quality_check_loop = OperatorPOWL(operator=Operator.LOOP, children=[quality_check])

# Define transitions for exclusive choice (Client Approval)
client_approval_choice = OperatorPOWL(operator=Operator.XOR, children=[client_approval, SilentTransition()])
...
root.order.add_edge(vendor_coordination, quality_check_loop)
root.order.add_edge(quality_check_loop, client_approval_choice)
root.order.add_edge(client_approval_choice, packaging_prep)
