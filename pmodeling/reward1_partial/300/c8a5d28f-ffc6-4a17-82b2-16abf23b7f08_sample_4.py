...
xor_certify_label = OperatorPOWL(operator=Operator.XOR, children=[health_certify, custom_labeling])

# Define the partial order for the entire process
root = StrictPartialOrder(nodes=[milk_sourcing, quality_testing, milk_pasturize, culture_addition, curd_cutting, whey_drain, cheese_molding, loop_aging, xor_certify_label, logistics_setup, export_docs, customs_clearance, shipment_track, client_feedback])
...
root.order.add_edge(cheese_molding, loop_aging)
root.order.add_edge(loop_aging, xor_certify_label)
root.order.add_edge(xor_certify_label, logistics_setup)
