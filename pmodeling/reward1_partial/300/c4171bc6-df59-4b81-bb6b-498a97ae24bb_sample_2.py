...
loop_aging = OperatorPOWL(operator=Operator.LOOP, children=[controlled_aging, sensory_check, batch_packaging, label_printing, cold_storage, logistics_plan, retail_delivery])
xor_feedback = OperatorPOWL(operator=Operator.XOR, children=[feedback_review, provenance_track])
root = StrictPartialOrder(nodes=[milk_sourcing, quality_testing, pasteurize, curd_formation, whey_separation, press_cheese, salt_application, loop_aging, xor_feedback])
...
root.order.add_edge(feedback_review, provenance_track)
root.order.add_edge(provenance_track, loop_aging)
root.order.add_edge(loop_aging, xor_feedback)