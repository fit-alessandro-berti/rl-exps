...
loop1 = OperatorPOWL(operator=Operator.LOOP, children=[iot_monitoring, data_analytics, nutrient_adjust, pest_control, maintenance_check])
loop2 = OperatorPOWL(operator=Operator.LOOP, children=[market_launch, logistics_setup])

# Define the partial order
root = StrictPartialOrder(nodes=[site_analysis, env_assessment, system_design, equipment_order, seed_selection, install_modules, calibrate_systems, staff_training, plant_seeding, loop1, loop2])
...
root.order.add_edge(loop2, site_analysis)