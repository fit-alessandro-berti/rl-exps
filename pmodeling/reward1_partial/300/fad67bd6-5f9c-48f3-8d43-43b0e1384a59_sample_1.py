...
loop = OperatorPOWL(operator=Operator.LOOP, children=[data_monitor, safety_audit, emergency_plan, maintenance_plan, battery_cycle, route_update, performance_review, impact_study, compliance_review])

# Define the exclusive choice
xor = OperatorPOWL(operator=Operator.XOR, children=[drone_assemble, flight_test])

# Define the partial order
root = StrictPartialOrder(nodes=[regulatory_check, path_design, weather_sync, traffic_align, package_secure, customer_alert, xor, loop])
...
root.order.add_edge(customer_alert, xor)
