...
# Define operators
xor = OperatorPOWL(operator=Operator.XOR, children=[collector_targeting, feedback_collection])
loop = OperatorPOWL(operator=Operator.LOOP, children=[local_cooperatives, environmental_audit, ethical_logistics, global_shipping])

# Define root
root = StrictPartialOrder(nodes=[material_sourcing, cultural_verify, eco_transport, batch_storytelling, craftsman_assignment, product_creation, provenance_catalog, community_marketing, xor, loop])
...
root.order.add_edge(community_marketing, xor)
root.order.add_edge(xor, loop)
root.order.add_edge(loop, local_cooperatives)
