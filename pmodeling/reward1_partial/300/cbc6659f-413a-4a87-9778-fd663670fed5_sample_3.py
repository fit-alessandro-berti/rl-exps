...
customs_loop = OperatorPOWL(operator=Operator.LOOP, children=[customs_clearance, risk_assessment, ethics_approval, restoration_plan])
final_loop = OperatorPOWL(operator=Operator.LOOP, children=[final_approval, catalog_entry, exhibit_prep])

root = StrictPartialOrder(nodes=[provenance_loop, condition_loop, customs_loop, final_loop])
root.order.add_edge(provenance_loop, condition_loop)
root.order.add_edge(condition_loop, customs_loop)
root.order.add_edge(customs_loop, final_loop)