...
xor = OperatorPOWL(operator=Operator.XOR, children=[compliance_check, maintenance_plan])
loop = OperatorPOWL(operator=Operator.LOOP, children=[performance_tune])
xor_loop = OperatorPOWL(operator=Operator.XOR, children=[xor, loop])
root = StrictPartialOrder(nodes=[client_meet, requirement_gather, module_design, supplier_vetting, component_order, prototype_build, field_testing, test_analysis, software_setup, data_integration, pilot_train, fleet_deploy, remote_monitor, maintenance_plan, performance_tune])
