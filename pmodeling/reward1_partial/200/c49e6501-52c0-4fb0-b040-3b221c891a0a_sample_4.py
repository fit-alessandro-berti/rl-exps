...
root = StrictPartialOrder(
    nodes=[
        component_check, spec_review, parts_sorting, mechanical_fit, firmware_load, calibration_run, stress_test,
        software_patch, algorithm_tune, comms_setup, validation_pass, data_link, onsite_deploy, live_monitor,
        update_push, recovery_plan, maintenance_log
    ],
    order=[
        # Specify dependencies between activities
...
# Optionally, add edges to the partial order if necessary
...
# Optionally, add silent transitions for parallel execution
root.order.add_edge(component_check, parts_sorting)
root.order.add_edge(spec_review, mechanical_fit)
root.order.add_edge(mechanical_fit, firmware_load)
root.order.add_edge(firmware_load, calibration_run)
root.order.add_edge(calibration_run, stress_test)
root.order.add_edge(stress_test, software_patch)
