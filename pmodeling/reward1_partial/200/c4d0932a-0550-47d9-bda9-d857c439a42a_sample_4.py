...
# Define silent transitions for non-action steps
skip = SilentTransition()

# Define loop nodes for continuous processes
aging_process = OperatorPOWL(operator=Operator.LOOP, children=[aging_control, flavor_profiling, packaging_design])

# Define exclusive choice nodes for multiple possible paths
marketing_strategies = OperatorPOWL(operator=Operator.XOR, children=[retail_outreach, customer_feedback])

# Define the root process
root = StrictPartialOrder(nodes=[milk_sourcing, quality_testing, starter_culture, coagulation, curd_cutting, whey_draining, molding_cheese, pressing_block, brining_bath, aging_process, demand_forecast, marketing_strategies])
