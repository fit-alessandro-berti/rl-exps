...
pest_control_loop.order.add_edge(pest_control, ai_diagnostics)

# Define the choice for community input
community_input_choice = OperatorPOWL(operator=Operator.XOR, children=[community_input, sustainability])

# Define the root POWL model
root = StrictPartialOrder(nodes=[site_analysis, env_scanning, farm_design, nutrient_mix, seed_automation, growth_monitor, pest_control_loop, robotic_sort, packaging_line, community_input_choice])
...
root.order.add_edge(pest_control_loop, robotic_sort)
root.order.add_edge(robotic_sort, packaging_line)
root.order.add_edge(packaging_line, community_input_choice)