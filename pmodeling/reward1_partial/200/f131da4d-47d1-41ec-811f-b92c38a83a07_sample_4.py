...
loop_aging = OperatorPOWL(operator=Operator.LOOP, children=[aging_setup, quality_check, packaging_prep, label_design, distribution_plan, retail_delivery, customer_feedback])
xor_quality_control = OperatorPOWL(operator=Operator.XOR, children=[skip, loop_aging])

root = StrictPartialOrder(nodes=[milk_sourcing, culture_selection, milk_testing, fermentation_start, temperature_control, pH_monitoring, curd_cutting, whey_draining, molding_cheese, salting_process, xor_quality_control])
...
root.order.add_edge(salting_process, xor_quality_control)