...
# Define the exclusive choice between package secure and dispatch alert
exclusive_choice = OperatorPOWL(operator=Operator.XOR, children=[package_secure, dispatch_alert])

# Define the loop for traffic scan
loop = OperatorPOWL(operator=Operator.LOOP, children=[traffic_scan])

# Define the root partial order
root = StrictPartialOrder(nodes=[demand_forecast, courier_match, credential_check, route_design, load_assign, loop, exclusive_choice, incentive_issue, dispute_review, customer_notify, feedback_collect, performance_log, ledger_update, hub_sync])
...
root.order.add_edge(load_assign, loop)
root.order.add_edge(loop, traffic_scan)
root.order.add_edge(traffic_scan, exclusive_choice)
root.order.add_edge(exclusive_choice, incentive_issue)
root.order.add_edge(exclusive_choice, dispute_review)
root.order.add_edge(incentive_issue, customer_notify)
