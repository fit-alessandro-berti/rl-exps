...
xor = OperatorPOWL(operator=Operator.XOR, children=[reg_compliance, skip])

root = StrictPartialOrder(nodes=[site_survey, env_assessment, xor, modular_setup, crop_selection, iot_integration, nutrient_flow, light_calibration, staff_training, pest_control, market_strategy, logistics_plan, yield_analysis, data_review, community_engage])
root.order.add_edge(site_survey, env_assessment)
root.order.add_edge(env_assessment, xor)
root.order.add_edge(xor, modular_setup)
