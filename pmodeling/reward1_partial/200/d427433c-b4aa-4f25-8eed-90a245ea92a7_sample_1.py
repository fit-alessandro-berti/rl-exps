...
# Define loop nodes for aging control
loop_aging_control = OperatorPOWL(operator=Operator.LOOP, children=[aging_control, feedback_review])

# Define XOR for packaging design
xor_packaging_design = OperatorPOWL(operator=Operator.XOR, children=[packaging_design, SilentTransition()])

# Define root POWL model
root = StrictPartialOrder(nodes=[milk_collection, quality_testing, milk_blending, starter_culture, fermentation_check, curd_cutting, whey_separation, molding_press, salting_stage, loop_aging_control, xor_packaging_design, cold_shipping, compliance_audit, blockchain_log, market_pricing, order_fulfillment, feedback_review])
...
root.order.add_edge(salting_stage, loop_aging_control)
root.order.add_edge(loop_aging_control, xor_packaging_design)
root.order.add_edge(xor_packaging_design, cold_shipping)
...
root.order.add_edge(feedback_review, loop_aging_control)