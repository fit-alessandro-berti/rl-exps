...
# Define the loop for the prototype build and user testing
prototype_loop = OperatorPOWL(operator=Operator.LOOP, children=[prototype_build, user_testing])

# Define the exclusive choice for community voting and expert review
community_xor = OperatorPOWL(operator=Operator.XOR, children=[community_voting, expert_review])

# Define the partial order
root = StrictPartialOrder(nodes=[idea_solicitation, ai_filtering, community_xor, prototype_loop, iterate_feedback, risk_assess, compliance_check, pilot_launch, performance_track, impact_analyze, insight_gather, cycle_adjust, final_report])

# Define the edges
root.order.add_edge(idea_solicitation, ai_filtering)
root.order.add_edge(ai_filtering, community_xor)
root.order.add_edge(community_xor, prototype_loop)
root.order.add_edge(prototype_loop, iterate_feedback)
