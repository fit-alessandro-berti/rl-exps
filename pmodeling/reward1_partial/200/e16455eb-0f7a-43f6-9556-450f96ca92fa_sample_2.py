...
# Define silent transitions for looping and exclusive choice
skip = SilentTransition()
loop_verify_provenance = OperatorPOWL(operator=Operator.LOOP, children=[verify_provenance, assess_condition, negotiate_terms])
xor_inspect_periodically_plan_return = OperatorPOWL(operator=Operator.XOR, children=[inspect_periodically, plan_return])

# Define the root POWL model
root = StrictPartialOrder(nodes=[loop_verify_provenance, arrange_transport, customs_clearance, secure_insurance, schedule_handlers, install_artwork, monitor_climate, manage_security, facilitate_access, document_display, coordinate_events, xor_inspect_periodically_plan_return, deinstall_artwork, finalize_reports])
root.order.add_edge(loop_verify_provenance, arrange_transport)
...
root.order.add_edge(coordinate_events, xor_inspect_periodically_plan_return)
root.order.add_edge(xor_inspect_periodically_plan_return, deinstall_artwork)
