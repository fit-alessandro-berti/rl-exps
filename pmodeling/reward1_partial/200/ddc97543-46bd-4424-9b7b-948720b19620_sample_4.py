...
root = StrictPartialOrder(nodes=[
    site_survey, structure_prep, system_install, env_control, nutrient_mix, crop_select, ai_setup,
    worker_train, pest_control, irrigation_plan, data_monitor, yield_forecast, energy_audit,
...
# Optionally, you can add silent transitions or choices (exclusive or concurrent nodes)
# For example, if there's an exclusive choice between two paths:
# xor = OperatorPOWL(operator=Operator.XOR, children=[path1, path2])
# root.order.add_edge(path1, xor)
# root.order.add_edge(path2, xor)
