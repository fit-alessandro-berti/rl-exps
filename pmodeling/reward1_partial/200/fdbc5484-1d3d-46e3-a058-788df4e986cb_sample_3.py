...
loop = OperatorPOWL(operator=Operator.LOOP, children=[Asset_ID, Value_Assess, Risk_Scan, Market_Review, Initial_Offer, Counter_Offer, Negotiation, Contract_Draft, Legal_Review, Digital_Sign, Royalty_Setup, Transfer_Record, Compliance_Check, Audit_Schedule, Market_Feedback, Strategy_Update])

root = StrictPartialOrder(nodes=[loop])
root.order.add_edge(loop, Asset_ID)
