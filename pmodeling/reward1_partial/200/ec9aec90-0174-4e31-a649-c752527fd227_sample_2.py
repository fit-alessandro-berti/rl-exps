...
xor = OperatorPOWL(operator=Operator.XOR, children=[PackageSecurely, ArrangeTransport])

root = StrictPartialOrder(nodes=[AssessArtifact, VerifyProvenance, AnalyzeCondition, PlanConservation, CleanSurface, StabilizeStructure, SourceMaterials, FabricateParts, PerformRepairs, ApplyPatina, MatchColors, DocumentProcess, ReviewQuality, ObtainApproval, xor])
...
root.order.add_edge(ObtainApproval, xor)
root.order.add_edge(xor, PackageSecurely)
root.order.add_edge(xor, ArrangeTransport)