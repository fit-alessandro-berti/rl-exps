...
for node in root.nodes:
    if not root.order.get_sources(node):
        root.order.add_edge(SilentTransition(), node)
    if not root.order.get_targets(node):
        root.order.add_edge(node, SilentTransition())
