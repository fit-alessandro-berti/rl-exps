...
# Define exclusive choice (XOR) for fabricating gears
xor_fabricate_gears = OperatorPOWL(operator=Operator.XOR, children=[fabricate_gears, skip])

# Define partial order for dial restoration
partial_order_dial_restoration = StrictPartialOrder(nodes=[dial_restoration, repaint_markers])

# Define partial order for lubricating bearings
partial_order_lubricate_bearings = StrictPartialOrder(nodes=[lubricate_bearings, calibrate_timing])

# Define partial order for polishing case
partial_order_polish_case = StrictPartialOrder(nodes=[polish_case, re_case_watch])

# Define partial order for quality testing
partial_order_quality_testing = StrictPartialOrder(nodes=[quality_testing, document_process])
...
root = StrictPartialOrder(nodes=[
    ...
    inspect_components,
    xor_fabricate_gears,
    partial_order_dial_restoration,
    partial_order_lubricate_bearings,
    partial_order_polish_case,
    partial_order_quality_testing,
    package_product
])
...
root.order.add_edge(inspect_components, xor_fabricate_gears)
root.order.add_edge(xor_fabricate_gears, dial_restoration)
