...
loop1 = OperatorPOWL(operator=Operator.LOOP, children=[data_capture, nutrient_mix, crop_rotate, waste_collect, compost_process, drone_dispatch, pest_control, pollination_run, volunteer_assign, feedback_gather, model_update, yield_forecast, water_adjust, report_generate, resource_audit, schedule_sync])

xor1 = OperatorPOWL(operator=Operator.XOR, children=[loop1, skip])

root = StrictPartialOrder(nodes=[sensor_setup, xor1])
root.order.add_edge(sensor_setup, xor1)