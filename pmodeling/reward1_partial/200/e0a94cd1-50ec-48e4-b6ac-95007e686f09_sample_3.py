...
# Define operators
xor = OperatorPOWL(operator=Operator.XOR, children=[inventory_adjust, procurement_plan])
loop = OperatorPOWL(operator=Operator.LOOP, children=[scenario_simulate, risk_assess, anomaly_detect, supplier_sync, quantum_communicate, performance_track, feedback_loop, decision_automate, cost_analyze, network_adapt])
root = StrictPartialOrder(nodes=[data_ingestion, quantum_setup, route_optimize, demand_forecast, xor, loop])
...
root.order.add_edge(loop, inventory_adjust)
root.order.add_edge(loop, procurement_plan)
root.order.add_edge(procurement_plan, scenario_simulate)
...
root.order.add_edge(quantum_communicate, performance_track)
