...
root.order.add_edge(community_poll, soil_testing)
root.order.add_edge(community_poll, biochar_apply)
root.order.add_edge(community_poll, microbial_add)
root.order.add_edge(community_poll, automated_harvest)
root.order.add_edge(community_poll, yield_review)
root.order.add_edge(community_poll, waste_process)
root.order.add_edge(community_poll, feedback_loop)

# Optionally, add additional edges to model the loop structure
# For example, if there's a loop involving some of the activities
# root.order.add_edge(...)
