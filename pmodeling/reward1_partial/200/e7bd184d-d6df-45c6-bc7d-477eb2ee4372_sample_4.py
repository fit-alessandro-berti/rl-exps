...
loop = OperatorPOWL(operator=Operator.LOOP, children=[yield_optimize])
xor = OperatorPOWL(operator=Operator.XOR, children=[pilot_crop, skip])
root = StrictPartialOrder(nodes=[site_analysis, structure_check, enviro_design, hydro_setup, aeroponics_add, lighting_procure, water_recycle, rack_install, seed_select, germinate_seeds, nutrient_mix, staff_train, pest_control, xor, loop])
...
root.order.add_edge(yield_optimize, loop)