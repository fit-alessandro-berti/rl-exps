...
# Ensure that stakeholder update can happen concurrently with the rest of the process
root.order.add_edge(stakeholder_update, stakeholder_update)

# Add a loop for stakeholder updates
loop = OperatorPOWL(operator=Operator.LOOP, children=[stakeholder_update])
root.order.add_edge(root, loop)
root.order.add_edge(loop, stakeholder_update)

# Add a choice for stakeholder update
xor = OperatorPOWL(operator=Operator.XOR, children=[stakeholder_update, skip])
root.order.add_edge(root, xor)
root.order.add_edge(xor, stakeholder_update)
