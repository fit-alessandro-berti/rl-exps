...
seasonal_review_loop = OperatorPOWL(operator=Operator.LOOP, children=[seasonal_review, skip])
feedback_loop = OperatorPOWL(operator=Operator.LOOP, children=[feedback_analysis, skip])

# Define the root POWL model
root = StrictPartialOrder(nodes=[farm_selection, milk_testing, starter_culture, curd_formation, pressing_cheese, microbial_profiling, aging_loop, hand_wrapping, quality_audit, packaging_prep, climate_shipping, retail_coordination, seasonal_review_loop, consumer_survey, feedback_loop, market_adjustment])
...
root.order.add_edge(retail_coordination, seasonal_review_loop)
root.order.add_edge(seasonal_review_loop, consumer_survey)
root.order.add_edge(consumer_survey, feedback_loop)
root.order.add_edge(feedback_loop, market_adjustment)