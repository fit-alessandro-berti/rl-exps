...
xor = OperatorPOWL(operator=Operator.XOR, children=[cross_verify_data, analyze_reports])
xor2 = OperatorPOWL(operator=Operator.XOR, children=[identify_gaps, recommend_actions])
loop = OperatorPOWL(operator=Operator.LOOP, children=[implement_changes, monitor_compliance])

# Define the root partial order
root = StrictPartialOrder(nodes=[initiate_audit, gather_documents, verify_suppliers, screen_transactions, classify_products, assess_risks, check_sanctions, review_tariffs, cross_verify_data, conduct_interviews, xor, xor2, loop, finalize_report])
...
root.order.add_edge(cross_verify_data, analyze_reports)
...
root.order.add_edge(monitor_compliance, loop)
root.order.add_edge(loop, finalize_report)
root.order.add_edge(finalize_report, loop)
