...
# Define the loop node for continuous monitoring and adaptive pest management
monitor_pest_management = OperatorPOWL(operator=Operator.LOOP, children=[monitor_growth, manage_pests])

# Define the partial order for the process
root = StrictPartialOrder(nodes=[site_survey, zoning_check, design_layout, system_order, structure_build, install_hydroponics, calibrate_sensors, select_crops, plant_seeding, monitor_pest_management, schedule_harvest, package_produce, local_delivery, analyze_data])
...
root.order.add_edge(plant_seeding, monitor_pest_management)
root.order.add_edge(monitor_pest_management, schedule_harvest)
