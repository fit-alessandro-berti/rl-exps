...
loop = OperatorPOWL(operator=Operator.LOOP, children=[
    microbial_profiling, hand_wrapping, quality_audit, packaging_prep
])
xor = OperatorPOWL(operator=Operator.XOR, children=[
    climate_shipping, retail_coordination
])
root = StrictPartialOrder(nodes=[
    farm_selection, milk_testing, starter_culture, curd_formation, pressing_cheese,
    loop, aging_control, xor, seasonal_review, consumer_survey, feedback_analysis, market_adjustment
])
...
root.order.add_edge(microbial_profiling, hand_wrapping)
