...
# Define the exclusive choice for packaging and labeling
packaging_labeling_choice = OperatorPOWL(operator=Operator.XOR, children=[packaging_design, label_approval])

# Define the partial order
root = StrictPartialOrder(nodes=[milk_sourcing, farm_selection, quality_testing_loop, milk_pasteurize, starter_culture, coagulation, curd_cutting, whey_draining, mold_inoculate, aging_control, flavor_tasting, packaging_labeling_choice, inventory_audit, order_fulfill, retail_shipping])
root.order.add_edge(milk_sourcing, farm_selection)
root.order.add_edge(farm_selection, quality_testing_loop)
root.order.add_edge(quality_testing_loop, milk_pasteurize)
...
root.order.add_edge(flavor_tasting, packaging_labeling_choice)
root.order.add_edge(packaging_labeling_choice, inventory_audit)
