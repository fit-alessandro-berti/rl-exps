...
# Define the loop for nutrient solution preparation and seed selection
nutrient_mix_loop = OperatorPOWL(operator=Operator.LOOP, children=[nutrient_mix, seed_selection])
nutrient_mix_loop.order.add_edge(nutrient_mix, seed_selection)

# Define the process flow
root = StrictPartialOrder(nodes=[site_survey, design_layout, module_build, system_install, water_prep, nutrient_mix_loop, climate_setup, sensor_deploy, pest_scan, growth_monitor, data_sync, energy_manage, harvest_plan, community_link])
...
root.order.add_edge(water_prep, nutrient_mix_loop)
root.order.add_edge(nutrient_mix_loop, climate_setup)
