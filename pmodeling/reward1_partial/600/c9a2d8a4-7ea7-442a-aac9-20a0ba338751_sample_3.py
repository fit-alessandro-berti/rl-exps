...
# Define the loop for cross-verifying data
cross_verify_loop = OperatorPOWL(operator=Operator.LOOP, children=[cross_verify_data])

# Define the choice between conducting interviews and analyzing reports
interview_or_analyze = OperatorPOWL(operator=Operator.XOR, children=[conduct_interviews, analyze_reports])

# Define the partial order
root = StrictPartialOrder(nodes=[initiate_audit, gather_documents, verify_suppliers, screen_transactions, classify_products, assess_risks, check_sanctions, review_tariffs, cross_verify_loop, interview_or_analyze, identify_gaps, recommend_actions, implement_changes, monitor_compliance, finalize_report])
...
root.order.add_edge(review_tariffs, cross_verify_loop)
root.order.add_edge(cross_verify_loop, interview_or_analyze)
root.order.add_edge(interview_or_analyze, identify_gaps)
