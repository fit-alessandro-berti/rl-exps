...
loop_aging = OperatorPOWL(operator=Operator.LOOP, children=[flavor_check, texture_scan])
xor_custom_pack = OperatorPOWL(operator=Operator.XOR, children=[custom_pack, skip])

root = StrictPartialOrder(nodes=[farm_select, milk_test, milk_pasteurize, curd_form, whey_drain, cheese_press, salt_rub, aging_set, loop_aging, xor_custom_pack, quality_approve, cold_ship, retail_train, feedback_log, batch_adjust])
...
root.order.add_edge(aging_set, loop_aging)
root.order.add_edge(loop_aging, xor_custom_pack)
root.order.add_edge(xor_custom_pack, quality_approve)
root.order.add_edge(quality_approve, cold_ship)
