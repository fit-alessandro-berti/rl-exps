...
# Define partial order with transitions and dependencies
root = StrictPartialOrder(nodes=[
    site_analysis, climate_setup, nutrient_mix, seed_germinate, auto_planting,
    irrigation_setup, iot_monitoring, pest_detection, drone_pollinate, pesticide_spray,
    robotic_harvest, quality_check, package_product, waste_recycle, energy_optimize, data_logging
])

# Define dependencies between activities
root.order.add_edge(site_analysis, climate_setup)
root.order.add_edge(site_analysis, nutrient_mix)
root.order.add_edge(climate_setup, seed_germinate)
...
root.order.add_edge(pest_detection, pesticide_spray)
root.order.add_edge(drone_pollinate, robotic_harvest)
