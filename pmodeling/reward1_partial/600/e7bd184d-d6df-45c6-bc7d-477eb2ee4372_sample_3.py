...
pilot_crop_loop = OperatorPOWL(operator=Operator.LOOP, children=[pilot_crop, data_gather, yield_optimize])

# Define the exclusive choice between staff training and pest control
staff_train_or_pest_control = OperatorPOWL(operator=Operator.XOR, children=[staff_train, pest_control])

# Define the root of the POWL model
root = StrictPartialOrder(nodes=[site_analysis, structure_check, enviro_design, hydro_setup, aeroponics_add, lighting_procure, water_recycle, rack_install, seed_select, germinate_seeds, nutrient_mix, staff_train_or_pest_control, pilot_crop_loop])
...
root.order.add_edge(nutrient_mix, staff_train_or_pest_control)
root.order.add_edge(staff_train_or_pest_control, pilot_crop_loop)
root.order.add_edge(pilot_crop_loop, data_gather)
