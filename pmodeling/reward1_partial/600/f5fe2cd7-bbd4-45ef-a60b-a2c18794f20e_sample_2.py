...
loop = OperatorPOWL(operator=Operator.LOOP, children=[discrepancy_flag, re_examination, alternative_source])
xor = OperatorPOWL(operator=Operator.XOR, children=[legal_verify, condition_report, expert_consult])
root = StrictPartialOrder(nodes=[provenance_check, material_scan, radiocarbon_test, stylistic_review, xor, loop, catalog_entry, exhibit_plan, acquisition_vote, final_approval])
...
root.order.add_edge(stylistic_review, xor)
root.order.add_edge(xor, loop)
root.order.add_edge(loop, catalog_entry)
root.order.add_edge(catalog_entry, exhibit_plan)
root.order.add_edge(exhibit_plan, acquisition_vote)
