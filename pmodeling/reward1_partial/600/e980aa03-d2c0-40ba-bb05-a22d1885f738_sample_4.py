...
root = StrictPartialOrder(nodes=[
    client_brief, spec_analysis, material_sourcing, component_vetting,
...
# Implement the choice (XOR) between AI training and QA review
xor_choice = OperatorPOWL(operator=Operator.XOR, children=[ai_training, qa_review])

# Add the loop for the flight test
loop_flight_test = OperatorPOWL(operator=Operator.LOOP, children=[flight_test])

# Connect all nodes according to the partial order
...
root.order.add_edge(flight_test, xor_choice)
root.order.add_edge(xor_choice, mission_pack)
