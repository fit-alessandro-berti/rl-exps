...
negotiation_loop = OperatorPOWL(operator=Operator.LOOP, children=[initial_offer, counter_offer, negotiation])
market_review_loop = OperatorPOWL(operator=Operator.LOOP, children=[market_review, negotiation_loop])
audit_loop = OperatorPOWL(operator=Operator.LOOP, children=[audit_schedule, market_feedback, strategy_update])

# Define the partial order
root = StrictPartialOrder(nodes=[asset_id, value_assess, risk_scan, market_review_loop, contract_draft, legal_review, digital_sign, royalty_setup, transfer_record, compliance_check, audit_loop])
root.order.add_edge(asset_id, value_assess)
root.order.add_edge(value_assess, risk_scan)
root.order.add_edge(risk_scan, market_review_loop)
root.order.add_edge(market_review_loop, contract_draft)
...
root.order.add_edge(compliance_check, audit_loop)
