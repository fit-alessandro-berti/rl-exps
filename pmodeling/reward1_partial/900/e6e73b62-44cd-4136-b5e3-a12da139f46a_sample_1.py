...
# Define the loop for the pest control and waste recycling
pest_control_loop = OperatorPOWL(operator=Operator.LOOP, children=[pest_control, recycle_waste])

# Define the partial order for the process
root = StrictPartialOrder(nodes=[site_survey, design_layout, procure_modules, install_framework, setup_sensors, calibrate_nutrients, configure_iot, plant_seeding, monitor_growth, manage_lighting, pest_control_loop, analyze_data, adjust_environment, harvest_crops, distribute_produce])
...
root.order.add_edge(manage_lighting, pest_control_loop)
root.order.add_edge(pest_control_loop, analyze_data)
