...
# Define operators
xor = OperatorPOWL(operator=Operator.XOR, children=[final_report, skip])

loop = OperatorPOWL(operator=Operator.LOOP, children=[material_check, condition_scan, ownership_verify, appraisal_update, restoration_plan, restoration_track, logistics_book, shipping_monitor, customs_clear, legal_compliance, ledger_update, exhibition_setup, public_showcase, archival_prep])

root = StrictPartialOrder(nodes=[initial_audit, artist_review, loop, xor])
root.order.add_edge(initial_audit, artist_review)
root.order.add_edge(artist_review, loop)
