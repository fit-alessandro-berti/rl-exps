...
# Define the loop for sensory evaluation and chemical refinement
sensory_loop = OperatorPOWL(operator=Operator.LOOP, children=[sensory_testing, chemical_analysis])
sensory_loop.order.add_edge(sensory_testing, chemical_analysis)

# Define the final adjustment loop
final_adjustment_loop = OperatorPOWL(operator=Operator.LOOP, children=[final_adjustment])
final_adjustment_loop.order.add_edge(final_adjustment, final_adjustment)

# Define the custom packaging loop
custom_packaging_loop = OperatorPOWL(operator=Operator.LOOP, children=[custom_packaging])
custom_packaging_loop.order.add_edge(custom_packaging, custom_packaging)
...
root.order.add_edge(sensory_loop, recipe_refinement)
...
root.order.add_edge(feedback_review, final_adjustment_loop)
root.order.add_edge(final_adjustment_loop, custom_packaging_loop)
root.order.add_edge(custom_packaging_loop, label_design)
