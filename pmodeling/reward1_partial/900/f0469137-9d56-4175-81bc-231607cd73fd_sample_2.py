...
loop = OperatorPOWL(operator=Operator.LOOP, children=[pest_detection, pesticide_spray, drone_pollinate])

# Define the XOR structure for pest detection and drone interventions
xor = OperatorPOWL(operator=Operator.XOR, children=[pest_detection, drone_pollinate])

# Define the partial order
root = StrictPartialOrder(nodes=[site_analysis, climate_setup, nutrient_mix, seed_germinate, auto_planting, irrigation_setup, iot_monitoring, loop, xor, robotic_harvest, quality_check, package_product, waste_recycle, energy_optimize, data_logging])
...
root.order.add_edge(iot_monitoring, loop)
