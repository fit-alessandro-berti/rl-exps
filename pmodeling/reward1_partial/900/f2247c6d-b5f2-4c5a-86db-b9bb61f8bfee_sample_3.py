...
xor_return_process = OperatorPOWL(operator=Operator.XOR, children=[segregate_stock, wipe_data, harvest_parts, refurbish_units, test_quality, recycle_waste, dispose_defects])
xor_inventory = OperatorPOWL(operator=Operator.XOR, children=[update_inventory, coordinate_resale, process_refunds])
xor_analysis = OperatorPOWL(operator=Operator.XOR, children=[analyze_patterns, improve_design, report_metrics])

# Define the partial order
root = StrictPartialOrder(nodes=[receive_returns, inspect_items, xor_return_process, xor_inventory, xor_analysis])
root.order.add_edge(receive_returns, inspect_items)
root.order.add_edge(inspect_items, xor_return_process)
root.order.add_edge(inspect_items, xor_inventory)
root.order.add_edge(inspect_items, xor_analysis)
