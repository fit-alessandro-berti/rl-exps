...
# Define loop
loop_retrieval = OperatorPOWL(operator=Operator.LOOP, children=[legal_review, security_check, execute_recovery, validate_items, restore_function, update_systems, train_users, document_findings, archive_records, review_lessons, close_process])

# Define XOR
xor_risk_assessment = OperatorPOWL(operator=Operator.XOR, children=[assess_risks, skip])

# Define root
root = StrictPartialOrder(nodes=[loop_retrieval, xor_risk_assessment])
root.order.add_edge(loop_retrieval, xor_risk_assessment)
