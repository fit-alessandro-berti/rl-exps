...
# User Signup -> Preference Set -> Meal Select -> Schedule Delivery -> Supplier Match -> Inventory Check -> Ingredient Order -> Quality Inspect -> Meal Pack -> Route Plan -> Dispatch Kit -> Delivery Track -> Feedback Collect -> Data Analyze -> Plan Optimize
root = StrictPartialOrder(nodes=[signup, set_preference, select_meal, schedule_delivery, match_supplier, check_inventory, order_ingredients, inspect_quality, pack_meal, plan_route, dispatch_kit, track_delivery, collect_feedback, analyze_data, optimize_plan])
