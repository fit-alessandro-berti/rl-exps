...
# For example, if there are no silent transitions in this process:
# root.order.add_edge(courier_match, credential_check)
# root.order.add_edge(credential_check, route_design)

# If you want to add a loop for any reason (not necessary here), you can do so:
# loop = OperatorPOWL(operator=Operator.LOOP, children=[load_assign])
# root.order.add_edge(load_assign, loop)
# root.order.add_edge(loop, load_assign)

# Print the root POWL model (optional)
print(root)