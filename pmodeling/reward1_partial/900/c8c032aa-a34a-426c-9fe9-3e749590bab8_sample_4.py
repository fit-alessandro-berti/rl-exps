...
# Define the loop for quality testing and data analysis
loop_quality = OperatorPOWL(operator=Operator.LOOP, children=[quality_testing, data_analysis])
loop_quality.order.add_edge(quality_testing, data_analysis)

# Define the partial order
root = StrictPartialOrder(nodes=[light_sourcing, nutrient_order, climate_setup, growth_planning, seed_planting, irrigation_check, pest_monitoring, energy_tracking, loop_quality, packaging_prep, inventory_update, delivery_scheduling, customer_feedback, market_forecast])
...
root.order.add_edge(energy_tracking, loop_quality)
root.order.add_edge(loop_quality, packaging_prep)
