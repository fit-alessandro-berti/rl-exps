...
# Define the XOR operator to model the choice between activities
xor = OperatorPOWL(operator=Operator.XOR, children=[aging_monitoring, flavor_profiling, packaging_design, compliance_check, market_research, direct_shipping, customer_feedback, recipe_adjust])

# Define the loop operator to model the aging monitoring process
loop = OperatorPOWL(operator=Operator.LOOP, children=[aging_monitoring])

# Define the root of the POWL model
root = StrictPartialOrder(nodes=[milk_sourcing, quality_testing, culture_prep, milk_pasturize, curd_cutting, whey_draining, molding_cheese, pressing_blocks, salting_process, xor])
...
root.order.add_edge(salting_process, loop)
