...
# Add a loop for the pilot launch process
pilot_launch_loop = OperatorPOWL(operator=Operator.LOOP, children=[performance_track, impact_analyze, insight_gather])
root.order.add_edge(pilot_launch, pilot_launch_loop)

# Add a choice for the final report process
final_report_choice = OperatorPOWL(operator=Operator.XOR, children=[final_report, skip])
root.order.add_edge(pilot_launch_loop, final_report_choice)

# Add a silent transition for the choice
root.order.add_edge(final_report_choice, final_report_choice)
