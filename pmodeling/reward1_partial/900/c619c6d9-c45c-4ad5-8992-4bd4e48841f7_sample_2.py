...
# Define the POWL transitions for each activity
...
xor = OperatorPOWL(operator=Operator.XOR, children=[regulation_check, client_review])
loop = OperatorPOWL(operator=Operator.LOOP, children=[chassis_build, wiring_connect, software_load, flight_testing, data_analyze, quality_inspect, packaging_prep, logistics_plan])
root = StrictPartialOrder(nodes=[design_consult, component_sourcing, sensor_calibrate, firmware_integrate, payload_configure, assembly_setup, loop, xor])
