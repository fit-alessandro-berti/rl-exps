...
aging_loop = OperatorPOWL(operator=Operator.LOOP, children=[aging_setup, humidity_control, temperature_monitor, quality_test, packaging])

# Define the exclusive choice for quality test and packaging
quality_test_or_packaging = OperatorPOWL(operator=Operator.XOR, children=[quality_test, packaging])

# Define the partial order for the entire process
root = StrictPartialOrder(nodes=[milk_sourcing, culture_prep, milk_pasturize, milk_inoculate, curd_formation, curd_cut, whey_drain, mold_inoculate, press_cheese, aging_loop, quality_test_or_packaging, order_fulfill, retail_deliver, feedback_collect])
