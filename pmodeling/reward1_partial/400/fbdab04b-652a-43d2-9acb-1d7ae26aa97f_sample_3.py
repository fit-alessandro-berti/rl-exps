...
calibration_loop = OperatorPOWL(operator=Operator.LOOP, children=[calibration_test, flight_simulate])
user_train_loop = OperatorPOWL(operator=Operator.LOOP, children=[user_train, remote_setup])

# Define the partial order
root = StrictPartialOrder(nodes=[client_consult, spec_analysis, module_select, component_order, parts_inspect, frame_assemble, sensor_install, motor_attach, wiring_connect, software_upload, calibration_loop, quality_review, user_train_loop, feedback_collect, support_schedule])
...
root.order.add_edge(software_upload, calibration_loop)
root.order.add_edge(calibration_loop, quality_review)
root.order.add_edge(quality_review, user_train_loop)
root.order.add_edge(user_train_loop, feedback_collect)
