...
root = StrictPartialOrder(nodes=[milk_sourcing, culture_blending, pasteurize, cutting, drain, mold_inoculate, press_cheese, salt_brine, age_monitor, quality_test, packaging_prep, label_design, order_allocation, transport_arrange, retail_sync, customer_review, feedback_analyze])

# Define the dependencies between activities
root.order.add_edge(milk_sourcing, culture_blending)
root.order.add_edge(culture_blending, pasteurize)
root.order.add_edge(pasteurize, cutting)
root.order.add_edge(cutting, drain)
root.order.add_edge(drain, mold_inoculate)
