...
site_prep_loop = OperatorPOWL(operator=Operator.LOOP, children=[site_prep, weather_monitor])
fabricate_loop = OperatorPOWL(operator=Operator.LOOP, children=[fabricate_parts, assemble_onsite, quality_inspect])
...
root.order.add_edge(vendor_select, permit_loop)
root.order.add_edge(permit_loop, site_prep_loop)
root.order.add_edge(site_prep_loop, fabricate_loop)
root.order.add_edge(fabricate_loop, public_unveil)
