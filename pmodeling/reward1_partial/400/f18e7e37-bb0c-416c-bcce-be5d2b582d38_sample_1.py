...
loop = OperatorPOWL(operator=Operator.LOOP, children=[ingredient_order, quality_inspect])
xor = OperatorPOWL(operator=Operator.XOR, children=[schedule_delivery, skip])
root = StrictPartialOrder(nodes=[user_signup, preference_set, meal_select, loop, xor, inventory_check, meal_pack, route_plan, dispatch_kit, delivery_track, feedback_collect, data_analyze, plan_optimize])
...
root.order.add_edge(xor, inventory_check)
root.order.add_edge(inventory_check, meal_pack)
