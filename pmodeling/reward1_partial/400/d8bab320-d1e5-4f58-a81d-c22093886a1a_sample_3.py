...
loop_recycle = OperatorPOWL(operator=Operator.LOOP, children=[waste_recycling])
xor_quality = OperatorPOWL(operator=Operator.XOR, children=[regulation_audit, customer_feedback])
root = StrictPartialOrder(nodes=[milk_sourcing, culture_selection, milk_pasteurize, curd_formation, whey_separation, mold_inoculate, cheese_pressing, loop_aging, packaging_design, label_approval, order_forecast, xor_quality])
...
root.order.add_edge(cheese_pressing, loop_aging)
...
root.order.add_edge(order_forecast, xor_quality)
