...
pref_set = Transition(label='Preference Set')
...
root = StrictPartialOrder(nodes=[
    signup, pref_set, meal_select, schedule_delivery, supplier_match, inventory_check, ingredient_order,
    quality_inspect, meal_pack, route_plan, dispatch_kit, delivery_track, feedback_collect, data_analyze, plan_optimize
])

# Define the partial order relationships
root.order.add_edge(signup, pref_set)
root.order.add_edge(pref_set, meal_select)
