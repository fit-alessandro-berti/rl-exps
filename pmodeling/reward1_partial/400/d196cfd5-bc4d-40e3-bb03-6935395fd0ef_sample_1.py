...
# Define the loop nodes for each activity
...
jury_evaluation_loop = OperatorPOWL(operator=Operator.LOOP, children=[jury_evaluation, result_compilation])

# Define the partial order model
root = StrictPartialOrder(nodes=[concept_ideation, sponsor_alignment, participant_sign_up, team_formation, workshop_loop, progress_monitor, live_support, feedback_loop, plagiarism_loop, jury_evaluation_loop, winner_announcement, post_analytics])
...
root.order.add_edge(workshop_loop, progress_monitor)
root.order.add_edge(progress_monitor, live_support)
root.order.add_edge(live_support, feedback_loop)
root.order.add_edge(feedback_loop, plagiarism_loop)
