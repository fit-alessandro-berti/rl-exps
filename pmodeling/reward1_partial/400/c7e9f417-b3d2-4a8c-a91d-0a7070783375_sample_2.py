...
exclusive_choice = OperatorPOWL(operator=Operator.XOR, children=[legal_clearance, trademark_check])
loop = OperatorPOWL(operator=Operator.LOOP, children=[customer_sync, cultural_align, internal_brief, campaign_design, resource_plan, stakeholder_meet, launch_prep, feedback_loop, performance_track])

# Define the root POWL model
root = StrictPartialOrder(nodes=[brand_audit, equity_review, market_analysis, exclusive_choice, loop])
root.order.add_edge(brand_audit, equity_review)
root.order.add_edge(equity_review, market_analysis)
root.order.add_edge(market_analysis, exclusive_choice)
root.order.add_edge(exclusive_choice, loop)
root.order.add_edge(loop, customer_sync)
