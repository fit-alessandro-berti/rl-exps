...
xor_loop = OperatorPOWL(operator=Operator.LOOP, children=[order_fulfill, retail_deliver])
root = StrictPartialOrder(nodes=[milk_sourcing, culture_prep, milk_pasturize, milk_inoculate, curd_form, curd_cut, whey_drain, mold_inoculate, press_cheese, aging_setup, humidity_control, temperature_monitor, xor, xor_loop])
...
root.order.add_edge(temperature_monitor, xor)
root.order.add_edge(xor, xor_loop)
root.order.add_edge(xor_loop, quality_test)
root.order.add_edge(xor_loop, packaging)
