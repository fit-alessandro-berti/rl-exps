...
xor = OperatorPOWL(operator=Operator.XOR, children=[weather_monitor, pre_flight_inspect, launch_drone])
loop = OperatorPOWL(operator=Operator.LOOP, children=[flight_track, delivery_confirm, data_analyze, feedback_collect, warranty_register, issue_resolve])
xor2 = OperatorPOWL(operator=Operator.XOR, children=[package_secure, package_return])

# Create the root node
root = StrictPartialOrder(nodes=[client_consult, payload_assess, drone_configure, regulation_check, flight_simulate, route_optimize, xor, loop, xor2])
...
root.order.add_edge(route_optimize, xor)
root.order.add_edge(xor, loop)
root.order.add_edge(loop, xor2)
root.order.add_edge(xor2, package_secure)
root.order.add_edge(package_secure, package_return)
