...
# Define the loop for nutrient control and pest control
nutrient_pest_loop = OperatorPOWL(operator=Operator.LOOP, children=[nutrient_control, pest_control])

# Define the partial order
root = StrictPartialOrder(nodes=[site_acquisition, impact_assess, modular_setup, crop_planting, nutrient_pest_loop, growth_monitor, community_engage, yield_forecast, supply_coordinate, compliance_check, waste_recycle, energy_optimize, market_strategy, performance_review])
...
root.order.add_edge(crop_planting, nutrient_pest_loop)
root.order.add_edge(nutrient_pest_loop, growth_monitor)
