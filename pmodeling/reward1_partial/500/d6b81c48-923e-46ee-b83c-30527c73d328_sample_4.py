...
# Define workflow
preflight_choice = OperatorPOWL(operator=Operator.XOR, children=[battery_test, route_update, firmware_patch])
sensor_choice = OperatorPOWL(operator=Operator.XOR, children=[battery_test, route_update, firmware_patch])
loop1 = OperatorPOWL(operator=Operator.LOOP, children=[preflight_check, sensor_calibrate, battery_test])
loop2 = OperatorPOWL(operator=Operator.LOOP, children=[telemetry_monitor, anomaly_detect, collision_assess, data_upload, postflight_review, battery_optimize, damage_repair, compliance_report, performance_log, mission_debrief])
