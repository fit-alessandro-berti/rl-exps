...
loop_aging = OperatorPOWL(operator=Operator.LOOP, children=[aging_monitor, feedback_collect])

# Define the exclusive choice for flavor adjustment
xor_flavor = OperatorPOWL(operator=Operator.XOR, children=[flavor_adjust, SilentTransition()])

# Define the partial order
root = StrictPartialOrder(nodes=[farm_selection, milk_testing, batch_pasteurize, culture_add, curd_cut, whey_drain, mold_inoculate, press_form, salt_rub, loop_aging, packaging_design, label_approval, order_processing, cold_storage, delivery_schedule, retail_setup, xor_flavor])
...
root.order.add_edge(retail_setup, xor_flavor)
