...
# Example: Loop for nutrient mix and climate adjust
loop_nutrient_climate = OperatorPOWL(operator=Operator.LOOP, children=[nutrient_mix, climate_adjust])

# Example: Exclusive choice for pest inspection and bio control
choice_pest_bio = OperatorPOWL(operator=Operator.XOR, children=[pest_inspection, bio_control])

# Define the root of the POWL model
root = StrictPartialOrder(nodes=[
    seed_selection, germination_start, nutrient_mix, climate_adjust, light_scheduling,
    choice_pest_bio, growth_monitor, water_recirc, harvest_plan, yield_forecast,
...
root.order.add_edge(light_scheduling, choice_pest_bio)
root.order.add_edge(choice_pest_bio, growth_monitor)
