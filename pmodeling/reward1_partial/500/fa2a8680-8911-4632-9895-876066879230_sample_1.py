...
loop_fleet_deployment = OperatorPOWL(operator=Operator.LOOP, children=[remote_monitor, maintenance_plan, performance_tune])
xor_pilot_train = OperatorPOWL(operator=Operator.XOR, children=[pilot_train, compliance_check])

# Define the root partial order
root = StrictPartialOrder(nodes=[client_meet, requirement_gather, module_design, supplier_vetting, component_order, prototype_build, field_testing, test_analysis, software_setup, data_integration, fleet_deploy, loop_fleet_deployment, xor_pilot_train])
...
root.order.add_edge(data_integration, fleet_deploy)
root.order.add_edge(fleet_deploy, loop_fleet_deployment)
root.order.add_edge(loop_fleet_deployment, xor_pilot_train)
root.order.add_edge(xor_pilot_train, pilot_train)
root.order.add_edge(xor_pilot_train, compliance_check)