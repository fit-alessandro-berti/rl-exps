...
# Define a loop for nutrient mixing and pH adjustment
loop = OperatorPOWL(operator=Operator.LOOP, children=[mix_nutrients, adjust_pH])

# Define a choice for pest detection and deployment of biocontrols
choice_pests = OperatorPOWL(operator=Operator.XOR, children=[detect_pests, deploy_biocontrols])

# Define the root POWL model
root = StrictPartialOrder(nodes=[seed_select, germinate_seeds, transplant_seedlings, loop, monitor_climate, control_humidity, co2_regulation, choice_pests, schedule_harvest, automate_picking, package_produce, compost_waste, recycle_water, data_logging, system_maintenance])

# Define the order dependencies
root.order.add_edge(seed_select, germinate_seeds)
root.order.add_edge(germinate_seeds, transplant_seedlings)
root.order.add_edge(transplant_seedlings, loop)
root.order.add_edge(loop, monitor_climate)
root.order.add_edge(monitor_climate, control_humidity)
root.order.add_edge(control_humidity, co2_regulation)
root.order.add_edge(co2_regulation, choice_pests)
root.order.add_edge(choice_pests, schedule_harvest)
