...
# Create a loop for the expert review process
expert_loop = OperatorPOWL(operator=Operator.LOOP, children=[expert_review, material_test, archival_search, style_compare, condition_check])

# Create a choice for the artifact intake process
artifact_choice = OperatorPOWL(operator=Operator.XOR, children=[artifact_intake, expert_loop])

# Create a partial order for the entire process
root = StrictPartialOrder(nodes=[artifact_choice, material_test, archival_search, style_compare, expert_review, restoration_check, provenance_trace, legal_verify, value_appraise, catalog_entry, marketing_plan, auction_setup, certify_final, sales_report])
root.order.add_edge(artifact_choice, material_test)
root.order.add_edge(artifact_choice, archival_search)
root.order.add_edge(artifact_choice, style_compare)
