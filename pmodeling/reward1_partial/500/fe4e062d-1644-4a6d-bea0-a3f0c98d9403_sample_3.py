...
refinement_loop = OperatorPOWL(operator=Operator.LOOP, children=[blend_experiment, maturation_cycle])
...
root = StrictPartialOrder(nodes=[
    client_profiling, ingredient_sourcing, quality_check, blend_experiment, maturation_cycle, sensory_panel, refinement_loop,
    stability_test, packaging_design, batch_coordination, compliance_audit, market_survey, feedback_review, order_finalize,
    distribution_plan, inventory_update
])
...
root.order.add_edge(refinement_loop, blend_experiment)
root.order.add_edge(refinement_loop, maturation_cycle)
root.order.add_edge(blend_experiment, stability_test)
root.order.add_edge(maturation_cycle, stability_test)
