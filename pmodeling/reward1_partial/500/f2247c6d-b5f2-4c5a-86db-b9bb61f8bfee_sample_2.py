...
# Define the loop for inspecting and segregating items
loop = OperatorPOWL(operator=Operator.LOOP, children=[inspect_items, segregate_stock])

# Define the XOR for wiping data and harvesting parts
xor = OperatorPOWL(operator=Operator.XOR, children=[wipe_data, harvest_parts])

# Define the partial order for the process
root = StrictPartialOrder(nodes=[receive_returns, loop, xor, refurbish_units, test_quality, recycle_waste, dispose_defects, update_inventory, coordinate_resale, process_refunds, analyze_patterns, improve_design, report_metrics])
root.order.add_edge(receive_returns, loop)
root.order.add_edge(loop, xor)
root.order.add_edge(xor, refurbish_units)
