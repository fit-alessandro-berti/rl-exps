...
# Define the loop for meal plan optimization
loop = OperatorPOWL(operator=Operator.LOOP, children=[data_analyze, plan_optimize])

# Define the exclusive choice for different stages of the process
xor = OperatorPOWL(operator=Operator.XOR, children=[user_signup, preference_set, meal_select, schedule_delivery, supplier_match, inventory_check, ingredient_order, quality_inspect, meal_pack, route_plan, dispatch_kit, delivery_track, feedback_collect])

# Create the root POWL model
root = StrictPartialOrder(nodes=[loop, xor])
root.order.add_edge(loop, xor)

# Save the root POWL model
print(root)