...
monitor_loop = OperatorPOWL(operator=Operator.LOOP, children=[monitor_growth, adjust_controls])

# Define partial order for the entire process
root = StrictPartialOrder(nodes=[site_survey, climate_check, soil_testing, media_select, design_layout, hydro_setup, nutrient_mix, sensor_install, stakeholder_or_regulation, permit_apply, community_train, plant_seed, monitor_loop, harvest_plan, waste_recycle, feedback_loop])
...
root.order.add_edge(monitor_loop, adjust_controls)
root.order.add_edge(adjust_controls, monitor_loop)
