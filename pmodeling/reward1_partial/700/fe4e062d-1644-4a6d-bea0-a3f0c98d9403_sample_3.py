...
# Define the loop for refinement
refinement_loop_node = OperatorPOWL(operator=Operator.LOOP, children=[blend_experiment, sensory_panel, stability_test])

# Define the XOR for packaging and batch coordination
xor_node = OperatorPOWL(operator=Operator.XOR, children=[packaging_design, batch_coordination])

# Define the root POWL model
root = StrictPartialOrder(nodes=[client_profiling, ingredient_sourcing, quality_check, blend_experiment, maturation_cycle, sensory_panel, refinement_loop_node, stability_test, packaging_design, batch_coordination, compliance_audit, market_survey, feedback_review, order_finalize, distribution_plan, inventory_update])
...
root.order.add_edge(sensory_panel, refinement_loop_node)
root.order.add_edge(refinement_loop_node, stability_test)
