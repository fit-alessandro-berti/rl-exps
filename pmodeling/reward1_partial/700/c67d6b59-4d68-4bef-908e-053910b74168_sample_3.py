...
# Define the process structure using OperatorPOWL
root.add_node(client_meet)
root.add_node(design_draft)
root.add_node(vendor_select)
root.add_node(component_order)
root.add_node(parts_inspect)
root.add_node(frame_build)
root.add_node(wiring_setup)
root.add_node(software_load)
root.add_node(flight_sim)
root.add_node(quality_test)
root.add_node(feedback_review)
root.add_node(adjust_design)
root.add_node(compliance_check)
root.add_node(packaging_prep)
root.add_node(final_demo)
root.add_node(ship_drone)
