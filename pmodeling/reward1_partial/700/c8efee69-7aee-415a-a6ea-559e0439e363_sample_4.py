...
# Define the loop node for the aging control and cold storage activities
loop = OperatorPOWL(operator=Operator.LOOP, children=[aging_control, cold_storage])

# Define the partial order
root = StrictPartialOrder(nodes=[milk_sourcing, quality_testing, starter_prep, curd_cutting, molding_cheese, salting_process, loop, packaging_design, label_printing, inventory_audit, order_processing, logistics_planning, retail_delivery, consumer_feedback, batch_adjustment, event_coordination])
...
root.order.add_edge(aging_control, cold_storage)
root.order.add_edge(cold_storage, packaging_design)
