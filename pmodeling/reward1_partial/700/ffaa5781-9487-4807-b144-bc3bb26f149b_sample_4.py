...
# Add the exclusive choice for final approval
final_approval_choice = OperatorPOWL(operator=Operator.XOR, children=[final_approval, skip])
root.order.add_edge(consensus_meeting, final_approval_choice)

# Add the loop for digital imaging and 3D modeling
digital_imaging_loop = OperatorPOWL(operator=Operator.LOOP, children=[digital_imaging, three_d_modeling])
root.order.add_edge(final_approval, digital_imaging_loop)
root.order.add_edge(digital_imaging_loop, final_approval_choice)
