...
# Define the loop for nutrient management and climate control
loop_nutrient_climate = OperatorPOWL(operator=Operator.LOOP, children=[mix_nutrients, adjust_pH, monitor_climate, control_humidity, CO2_regulation])

# Define the exclusive choice for pest management
xor_pest_management = OperatorPOWL(operator=Operator.XOR, children=[detect_pests, deploy_biocontrols])

# Define the process
root = StrictPartialOrder(nodes=[seed_select, germinate_seeds, transplant_seedlings, loop_nutrient_climate, xor_pest_management, schedule_harvest, automate_picking, package_produce, compost_waste, recycle_water, data_logging, system_maintenance])
root.order.add_edge(seed_select, germinate_seeds)
root.order.add_edge(germinate_seeds, transplant_seedlings)
root.order.add_edge(transplant_seedlings, loop_nutrient_climate)
root.order.add_edge(loop_nutrient_climate, xor_pest_management)
root.order.add_edge(xor_pest_management, schedule_harvest)
