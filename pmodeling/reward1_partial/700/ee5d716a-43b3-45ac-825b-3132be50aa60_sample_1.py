...
# Define the loop for the main auction process
auction_loop = OperatorPOWL(operator=Operator.LOOP, children=[
    verify_artwork,
    analyze_provenance,
    set_reserve,
    activate_auction,
...
root = StrictPartialOrder(nodes=[auction_loop])

# Add the dependency edges to the partial order
...
# Add the final edge to complete the loop
root.order.add_edge(report_analytics, auction_loop)