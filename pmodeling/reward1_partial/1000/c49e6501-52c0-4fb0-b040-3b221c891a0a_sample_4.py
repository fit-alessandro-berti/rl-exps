...
# Define transitions for parallel activities
# (assuming parallel activities are not directly supported in POWL, we'll use XOR for demonstration)
software_customization = OperatorPOWL(operator=Operator.XOR, children=[software_patch, algorithm_tune, comms_setup])

# Define the root Partial Order model
root = StrictPartialOrder(nodes=[component_check, spec_review, parts_sorting, mechanical_fit, firmware_load, calibration_run, stress_test, software_customization, validation_pass, data_link, onsite_deploy, live_monitor, update_push, recovery_plan, maintenance_log])
...
root.order.add_edge(stress_test, software_customization)
root.order.add_edge(software_customization, validation_pass)
