...
root.add_node(visual_inspect)
root.add_node(document_gather)
root.add_node(material_test)
root.add_node(pigment_analyze)
root.add_node(style_compare)
root.add_node(provenance_trace)
root.add_node(data_crosscheck)
root.add_node(infrared_scan)
root.add_node(xray_fluoresce)
root.add_node(expert_consult)
root.add_node(forger_detect)
root.add_node(report_draft)
root.add_node(stakeholder_review)
root.add_node(final_approval)
root.add_node(archive_store)
...
root.order.add_edge(forger_detect, report_draft)
