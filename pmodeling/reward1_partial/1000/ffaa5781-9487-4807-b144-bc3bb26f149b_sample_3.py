...
# Define the XOR node
xor = OperatorPOWL(operator=Operator.XOR, children=[expert_review, context_validation, legal_audit, export_verify])

# Define the strict partial order
root = StrictPartialOrder(nodes=[provenance_check, loop, xor, digital_imaging, three_d_modeling, consensus_meeting, final_approval, catalog_entry, virtual_setup, archival_backup])
root.order.add_edge(provenance_check, loop)
root.order.add_edge(loop, xor)
root.order.add_edge(xor, digital_imaging)
