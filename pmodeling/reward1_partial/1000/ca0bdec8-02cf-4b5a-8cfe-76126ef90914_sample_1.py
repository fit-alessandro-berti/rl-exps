...
# Define the loop for the aging monitoring process
aging_loop = OperatorPOWL(operator=Operator.LOOP, children=[aging_monitoring])

# Define the XOR for the compliance check and market research
xor = OperatorPOWL(operator=Operator.XOR, children=[compliance_check, market_research])

# Define the partial order for the entire process
root = StrictPartialOrder(nodes=[milk_sourcing, quality_testing, culture_prep, milk_pasteurize, curd_cutting, whey_draining, molding_cheese, pressing_blocks, salting_process, aging_loop, flavor_profiling, packaging_design, xor, direct_shipping, customer_feedback, recipe_adjust])
...
root.order.add_edge(packaging_design, xor)
root.order.add_edge(xor, direct_shipping)
