...
# Define the loop for data logging and chain of custody
data_log_chain = OperatorPOWL(operator=Operator.LOOP, children=[data_log, chain_custody])

# Define the exclusive choice for auction preparation
auction_choice = OperatorPOWL(operator=Operator.XOR, children=[auction_prep])
...
root = StrictPartialOrder(nodes=[
    ...
    legal_verify, registry_search, customs_clear, condition_assess, data_log_chain,
    report_draft, certification, secure_archive, auction_choice
])
...
root.order.add_edge(condition_assess, data_log_chain)
root.order.add_edge(data_log_chain, report_draft)
root.order.add_edge(report_draft, certification)
root.order.add_edge(certification, secure_archive)
root.order.add_edge(secure_archive, auction_choice)