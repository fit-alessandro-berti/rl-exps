...
xor = OperatorPOWL(operator=Operator.XOR, children=[fleet_deploy, insurance_verify])
loop = OperatorPOWL(operator=Operator.LOOP, children=[fleet_deploy, insurance_verify])
xor_compliance = OperatorPOWL(operator=Operator.XOR, children=[compliance_check, xor])
xor_maintenance = OperatorPOWL(operator=Operator.XOR, children=[maintenance_plan, loop])
xor_billing = OperatorPOWL(operator=Operator.XOR, children=[billing_process, xor_maintenance])
xor_performance = OperatorPOWL(operator=Operator.XOR, children=[performance_report, xor_billing])
xor_feedback = OperatorPOWL(operator=Operator.XOR, children=[feedback_collect, xor_performance])

# Create the root POWL model
root = StrictPartialOrder(nodes=[client_onboard, needs_assess, drone_config, route_program, compliance_check, insurance_verify, lease_contract, fleet_deploy, monitor_setup, usage_track, maintenance_plan, incident_manage, billing_process, performance_report, contract_renew, price_adjust, feedback_collect])
