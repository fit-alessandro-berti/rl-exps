...
# Define the loop for material analysis and data review
loop = OperatorPOWL(operator=Operator.LOOP, children=[material_analysis, data_review])

# Define the exclusive choice for expert interview and archive research
xor = OperatorPOWL(operator=Operator.XOR, children=[expert_interview, archive_research])

# Define the root POWL model
root = StrictPartialOrder(nodes=[artifact_intake, provenance_check, loop, xor, consensus_meeting, conservation_plan, preservation_setup, documentation, exhibition_prep])
root.order.add_edge(artifact_intake, provenance_check)
root.order.add_edge(provenance_check, loop)
root.order.add_edge(loop, xor)
root.order.add_edge(xor, consensus_meeting)
