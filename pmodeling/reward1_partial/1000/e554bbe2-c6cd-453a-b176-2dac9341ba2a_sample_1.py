...
# Define exclusive choice (XOR) for microbial profiling and hand wrapping
xor = OperatorPOWL(operator=Operator.XOR, children=[microbial_profiling, hand_wrapping])

# Define partial order (PO) for the rest of the activities
root = StrictPartialOrder(nodes=[farm_selection, milk_testing, starter_culture, curd_formation, pressing_cheese, xor, aging_control, quality_audit, packaging_prep, climate_shipping, retail_coordination, seasonal_review, consumer_survey, feedback_analysis, market_adjustment])
...
root.order.add_edge(xor, aging_control)
