...
# Define the loop
loop = OperatorPOWL(operator=Operator.LOOP, children=[Inventory_Scan, xor6])

# Define the partial order
root = StrictPartialOrder(nodes=[Demand_Forecast, Risk_Assess, Supplier_Audit, Inventory_Scan, xor, xor2, xor3, xor4, xor5, loop])
...
root.order.add_edge(Inventory_Scan, xor)
root.order.add_edge(xor, xor2)
root.order.add_edge(xor2, xor3)
root.order.add_edge(xor3, xor4)
root.order.add_edge(xor4, xor5)
root.order.add_edge(xor5, loop)
root.order.add_edge(loop, Inventory_Scan)
root.order.add_edge(Inventory_Scan, xor6)
