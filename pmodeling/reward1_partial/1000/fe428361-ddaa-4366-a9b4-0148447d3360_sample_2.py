...
# Define the loop for pre-flight inspection and weather monitoring
pre_flight_loop = OperatorPOWL(operator=Operator.LOOP, children=[pre_flight_inspect, weather_monitor])

# Define the process as a strict partial order
root = StrictPartialOrder(nodes=[
    client_consult, payload_assess, drone_configure, regulation_check, flight_simulate, route_optimize,
    package_secure, pre_flight_loop, launch_drone, flight_track, delivery_confirm, data_analyze,
    feedback_collect, warranty_register, issue_resolve, package_return
])
...
root.order.add_edge(package_secure, pre_flight_loop)
root.order.add_edge(pre_flight_loop, launch_drone)
