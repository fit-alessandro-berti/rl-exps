...
# Define the loop for pest monitoring and nutrient testing
pest_monitoring_loop = OperatorPOWL(operator=Operator.LOOP, children=[pest_monitoring, nutrient_testing])
pest_monitoring_loop.order.add_edge(pest_monitoring, nutrient_testing)

# Define the exclusive choice for supplier outreach and waste sorting
exclusive_choice = OperatorPOWL(operator=Operator.XOR, children=[supplier_outreach, waste_sorting])

# Define the root of the POWL model
root = StrictPartialOrder(nodes=[site_survey, load_testing, crop_selection, soil_prep, irrigation_setup, permits_acquire, exclusive_choice, pest_monitoring_loop, staff_training, community_meet, harvest_planning, market_launch, yield_tracking])
...
root.order.add_edge(permits_acquire, exclusive_choice)
root.order.add_edge(exclusive_choice, pest_monitoring_loop)
root.order.add_edge(pest_monitoring_loop, staff_training)
