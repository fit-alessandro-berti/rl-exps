...
loop = OperatorPOWL(operator=Operator.LOOP, children=[Aging_Control])

# Define the XOR for market pricing and order fulfillment
xor = OperatorPOWL(operator=Operator.XOR, children=[Market_Pricing, Order_Fulfillment])

# Define the partial order with the defined transitions and loop
root = StrictPartialOrder(nodes=[Milk_Collection, Quality_Testing, Milk_Blending, Starter_Culture, Fermentation_Check, Curd_Cutting, Whey_Separation, Molding_Press, Salting_Stage, loop, Packaging_Design, Cold_Shipping, Compliance_Audit, Blockchain_Log, xor, Feedback_Review])
...
root.order.add_edge(Blockchain_Log, xor)
