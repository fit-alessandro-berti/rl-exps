...
root.add_transition(initial_inspect)
root.add_transition(material_test)
root.add_transition(imaging_scan)
root.add_transition(historical_check)
root.add_transition(expert_consult)
root.add_transition(provenance_trace)
root.add_transition(forgery_detect)
root.add_transition(restoration_map)
root.add_transition(market_analyze)
root.add_transition(auction_review)
root.add_transition(value_assess)
root.add_transition(report_draft)
root.add_transition(board_review)
root.add_transition(certification)
root.add_transition(release_artifact)
root.add_transition(chain_custody)
