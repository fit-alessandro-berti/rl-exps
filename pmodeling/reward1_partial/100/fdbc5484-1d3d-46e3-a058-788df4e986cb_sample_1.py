...
xor = OperatorPOWL(operator=Operator.XOR, children=[Counter_Offer, skip])

# Define the loop
loop = OperatorPOWL(operator=Operator.LOOP, children=[Negotiation, Market_Review, Legal_Review, Digital_Sign, Royalty_Setup, Transfer_Record, Compliance_Check, Audit_Schedule, Market_Feedback])

# Define the strict partial order
root = StrictPartialOrder(nodes=[Asset_ID, Value_Assess, Risk_Scan, Market_Review, Initial_Offer, loop, xor, Contract_Draft, Legal_Review, Digital_Sign, Royalty_Setup, Transfer_Record, Compliance_Check, Audit_Schedule, Market_Feedback, Strategy_Update])
...
root.order.add_edge(Initial_Offer, loop)
root.order.add_edge(loop, xor)
root.order.add_edge(xor, Contract_Draft)
