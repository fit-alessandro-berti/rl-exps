...
xor = OperatorPOWL(operator=Operator.XOR, children=[legal_review, sale_prep])
loop = OperatorPOWL(operator=Operator.LOOP, children=[preservation_plan, archive_data])

# Construct the POWL model
root = StrictPartialOrder(nodes=[artifact_intake, catalog_entry, visual_inspect, material_test, spectroscopy, historical_check, provenance_trace, style_compare, three_d_scanning, condition_assess, loop, xor])
...
root.order.add_edge(archive_data, legal_review)
root.order.add_edge(legal_review, sale_prep)
root.order.add_edge(sale_prep, loop)
