...
# Define exclusive choice for pest monitor and drone deploy
xor = OperatorPOWL(operator=Operator.XOR, children=[pest_monitor, drone_deploy])

# Define loop for health scan and data logging
loop = OperatorPOWL(operator=Operator.LOOP, children=[health_scan, data_logging])

# Define partial order with all transitions
root = StrictPartialOrder(nodes=[site_analysis, impact_review, modular_design, system_integration, climate_setup, nutrient_mix, light_config, staff_training, xor, loop, supply_sync, maintenance_plan, waste_manage])
...
root.order.add_edge(loop, supply_sync)
