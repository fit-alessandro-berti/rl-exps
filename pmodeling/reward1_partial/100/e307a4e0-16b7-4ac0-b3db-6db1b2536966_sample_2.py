...
loop_phase = OperatorPOWL(operator=Operator.LOOP, children=[minor_repair, structural_reinforce, surface_restore])
coating_phase = OperatorPOWL(operator=Operator.LOOP, children=[coating_apply])

# Define the exclusive choice nodes
exclusive_choice1 = OperatorPOWL(operator=Operator.XOR, children=[coating_phase, ethics_review])
exclusive_choice2 = OperatorPOWL(operator=Operator.XOR, children=[provenance_verify, client_update])

# Define the partial order
root = StrictPartialOrder(nodes=[initial_assess, artifact_scan, condition_map, material_test, cleaning_phase, stability_check, loop_phase, exclusive_choice1, exclusive_choice2, final_report, archive_store])
