...
root = StrictPartialOrder(
    nodes=[
        seed_selection, nutrient_mix, env_setup, sensor_deployment,
        growth_monitoring, pest_detection, automated_harvest,
        quality_check, packaging_prep, biodegradable_pack,
        inventory_sync, demand_forecast, micro_fulfillment,
        local_dispatch, consumer_feedback, crop_adjustment
    ]
)

# Define the dependencies
root.order.add_edge(seed_selection, nutrient_mix)
root.order.add_edge(seed_selection, env_setup)
root.order.add_edge(seed_selection, sensor_deployment)
root.order.add_edge(nutrient_mix, growth_monitoring)
root.order.add_edge(env_setup, growth_monitoring)
