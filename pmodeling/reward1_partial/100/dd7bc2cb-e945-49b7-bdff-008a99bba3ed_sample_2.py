...
loop_aging = OperatorPOWL(operator=Operator.LOOP, children=[aging_set, flavor_check, texture_scan])
xor_distribution = OperatorPOWL(operator=Operator.XOR, children=[custom_pack, retail_train])

root = StrictPartialOrder(nodes=[farm_select, milk_test, milk_pasteurize, curd_form, whey_drain, cheese_press, salt_rub, loop_aging, xor_distribution])
...
root.order.add_edge(xor_distribution, custom_pack)
root.order.add_edge(xor_distribution, retail_train)
