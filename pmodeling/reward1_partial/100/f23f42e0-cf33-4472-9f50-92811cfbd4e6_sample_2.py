...
aging_loop = OperatorPOWL(operator=Operator.LOOP, children=[climate_control, quality_tasting, packaging_prep, label_printing])

# Define the XOR for quality testing and event coordination
xor = OperatorPOWL(operator=Operator.XOR, children=[quality_tasting, event_coordination])

# Define the root process
root = StrictPartialOrder(nodes=[milk_sourcing, culture_selection, milk_testing, curd_cutting, whey_draining, mold_inoculation, forming_cheese, salting_stage, aging_setup, aging_loop, xor, distribution_plan, retail_delivery, regulatory_audit])
...
root.order.add_edge(xor, distribution_plan)
root.order.add_edge(distribution_plan, retail_delivery)
root.order.add_edge(retail_delivery, regulatory_audit)
