...
xor = OperatorPOWL(operator=Operator.XOR, children=[component_order, skip])
loop = OperatorPOWL(operator=Operator.LOOP, children=[field_testing, test_analysis, software_setup])
root = StrictPartialOrder(nodes=[client_meet, requirement_gather, module_design, supplier_vetting, xor, prototype_build, loop, data_integration, pilot_train, compliance_check, fleet_deploy, remote_monitor, maintenance_plan, performance_tune])

# Add edges to the workflow model
