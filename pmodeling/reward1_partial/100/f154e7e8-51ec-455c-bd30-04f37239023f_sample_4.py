...
loop1 = OperatorPOWL(operator=Operator.LOOP, children=[pest_control, data_logging])
loop2 = OperatorPOWL(operator=Operator.LOOP, children=[community_meet, workshop_plan])
loop3 = OperatorPOWL(operator=Operator.LOOP, children=[honey_extract, quality_test, packaging, market_setup, sales_report])
loop4 = OperatorPOWL(operator=Operator.LOOP, children=[regulation_check, waste_manage, seasonal_review])

root = StrictPartialOrder(nodes=[site_survey, flora_mapping, hive_setup, sensor_install, health_check, loop1, loop2, loop3, loop4])
...
root.order.add_edge(health_check, loop1)
root.order.add_edge(loop1, pest_control)
root.order.add_edge(pest_control, data_logging)
root.order.add_edge(data_logging, loop2)
root.order.add_edge(loop2, community_meet)
root.order.add_edge(community_meet, workshop_plan)
root.order.add_edge(workshop_plan, loop3)
root.order.add_edge(loop3, honey_extract)
...
root.order.add_edge(sales_report, loop4)
root.order.add_edge(loop4, regulation_check)
