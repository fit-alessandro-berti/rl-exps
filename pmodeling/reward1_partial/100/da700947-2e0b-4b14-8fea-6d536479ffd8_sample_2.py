...
# Define exclusive choice for pest scan and data modeling
pest_scan_choice = OperatorPOWL(operator=Operator.XOR, children=[pest_scan, data_modeling])

# Define loop for sensor install and staff briefing
sensor_install_loop = OperatorPOWL(operator=Operator.LOOP, children=[sensor_install, staff_briefing])

# Define partial order for the entire process
root = StrictPartialOrder(nodes=[site_survey, structural_audit, climate_design, lighting_setup, irrigation_plan, nutrient_mix, sensor_install_loop, ai_calibration, pest_scan_choice, energy_audit, renewable_sync, data_modeling, staff_briefing, compliance_check, community_meet, yield_review, feedback_loop])
...
root.order.add_edge(nutrient_mix, sensor_install_loop)
root.order.add_edge(sensor_install_loop, ai_calibration)
root.order.add_edge(ai_calibration, pest_scan_choice)
root.order.add_edge(pest_scan_choice, energy_audit)
