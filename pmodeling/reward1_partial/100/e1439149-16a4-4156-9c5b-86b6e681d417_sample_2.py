...
loop = OperatorPOWL(operator=Operator.LOOP, children=[Concept_Filter, Prototype_Build, Expert_Review, Feasibility_Check, Risk_Assess])
parallel = OperatorPOWL(operator=Operator.PARALLEL, children=[Trend_Scan, Idea_Harvest, Workshop_Host])
xor = OperatorPOWL(operator=Operator.XOR, children=[parallel, skip])
root = StrictPartialOrder(nodes=[loop, xor])
root.order.add_edge(loop, xor)

# Add the remaining edges
root.order.add_edge(parallel, loop)
root.order.add_edge(Workshop_Host, loop)
root.order.add_edge(Idea_Harvest, loop)
root.order.add_edge(Trend_Scan, loop)
