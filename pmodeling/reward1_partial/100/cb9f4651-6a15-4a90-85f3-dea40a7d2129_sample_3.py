...
# Define transitions for the process
...
aging_loop = OperatorPOWL(operator=Operator.LOOP, children=[aging_setup, humidity_control, temp_monitor])
quality_loop = OperatorPOWL(operator=Operator.LOOP, children=[quality_test, feedback_collect])
packaging_loop = OperatorPOWL(operator=Operator.LOOP, children=[packaging, order_fulfill])

# Define the root process
root = StrictPartialOrder(nodes=[milk_sourcing, culture_prep, milk_pasturize, milk_inoculate, curd_form, curd_cut, whey_drain, mold_inoculate, press_cheese, aging_loop, quality_loop, packaging_loop])
...
root.order.add_edge(quality_loop, packaging_loop)
root.order.add_edge(packaging_loop, order_fulfill)
