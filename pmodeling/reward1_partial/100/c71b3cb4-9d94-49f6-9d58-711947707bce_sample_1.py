...
# Define the loop for nutrient and water delivery
nutrient_water_loop = OperatorPOWL(operator=Operator.LOOP, children=[nutrient_mix, water_delivery])

# Define the XOR for pest control and health check
pest_health_xor = OperatorPOWL(operator=Operator.XOR, children=[pest_control, health_check])
...
root.order.add_edge(seedling_transplant, nutrient_water_loop)
root.order.add_edge(nutrient_water_loop, light_adjustment)
root.order.add_edge(light_adjustment, climate_control)
root.order.add_edge(climate_control, sensor_monitoring)
root.order.add_edge(sensor_monitoring, pest_health_xor)
root.order.add_edge(pest_health_xor, harvest_planning)
