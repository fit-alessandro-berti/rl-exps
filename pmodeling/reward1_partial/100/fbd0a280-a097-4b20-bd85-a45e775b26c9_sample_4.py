...
# Define XOR node for pest control and disease prevention
xor = OperatorPOWL(operator=Operator.XOR, children=[pest_control, feedback_loop])

# Define loop node for health scan
loop = OperatorPOWL(operator=Operator.LOOP, children=[health_scan])

# Define root POWL model
root = StrictPartialOrder(nodes=[seed_selection, germination_setup, nutrient_mix, water_control, climate_adjust, sensor_monitor, lighting_tune, airflow_manage, loop, xor, cold_storage, package_prep, delivery_plan])
...
root.order.add_edge(loop, health_scan)
root.order.add_edge(health_scan, cold_storage)
