...
loop_fermentation = OperatorPOWL(operator=Operator.LOOP, children=[fermentation, aging_control])
xor_quality_control = OperatorPOWL(operator=Operator.XOR, children=[salting_stage, flavor_tasting])
xor_packaging = OperatorPOWL(operator=Operator.XOR, children=[packaging_artisanal, direct_delivery])

# Define the partial order
root = StrictPartialOrder(nodes=[milk_sourcing, quality_testing, starter_culture, milk_pasteurize, curd_cutting, whey_draining, pressing_cheese, salting_stage, fermentation, aging_control, flavor_tasting, packaging_artisanal, label_printing, order_processing, direct_delivery, customer_feedback])
...
root.order.add_edge(customer_feedback, loop_fermentation)
root.order.add_edge(loop_fermentation, xor_quality_control)
root.order.add_edge(xor_quality_control, xor_packaging)
