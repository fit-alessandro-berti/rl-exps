...
# Define the exclusive choice between AI Training and QA Review
xor_ai_qa = OperatorPOWL(operator=Operator.XOR, children=[ai_training, qa_review])

# Define the loop for flight test and software load
loop_flight_load = OperatorPOWL(operator=Operator.LOOP, children=[flight_test, software_load])

# Define the partial order
root = StrictPartialOrder(nodes=[client_brief, spec_analysis, material_sourcing, component_vetting, frame_assembly, sensor_install, propulsion_setup, calibration, loop_flight_load, xor_ai_qa, mission_pack, client_training, deployment_support])
...
root.order.add_edge(calibration, loop_flight_load)
root.order.add_edge(loop_flight_load, xor_ai_qa)
root.order.add_edge(xor_ai_qa, mission_pack)
