...
# Define the loop for monitoring and adjusting systems
monitor_adjust_loop = OperatorPOWL(operator=Operator.LOOP, children=[monitor_growth, adjust_systems])
...
root = StrictPartialOrder(nodes=[site_analysis, load_test, sunlight_map, medium_select, hydro_design, procure_seeds, install_irrigation, setup_climate, create_schedule, pest_control_choice, monitor_adjust_loop, harvest_crops, package_produce, engage_community, host_workshops])
...
root.order.add_edge(medium_select, setup_climate)
root.order.add_edge(hydro_design, create_schedule)
root.order.add_edge(create_schedule, pest_control_choice)
root.order.add_edge(pest_control_choice, monitor_adjust_loop)
root.order.add_edge(monitor_adjust_loop, harvest_crops)
