...
# Define loops
milk_loop = OperatorPOWL(operator=Operator.LOOP, children=[milk_sourcing, culture_blending, milk_pasturize, curd_cutting, whey_drain, mold_inoculate, press_cheese, salt_brine, age_monitor, quality_test, packaging_prep, label_design, order_allocation, transport_arrange, retail_sync, customer_review, feedback_analyze])

# Define XOR
xor = OperatorPOWL(operator=Operator.XOR, children=[milk_loop, skip])

# Define root
root = StrictPartialOrder(nodes=[xor])
root.order.add_edge(milk_loop, xor)
