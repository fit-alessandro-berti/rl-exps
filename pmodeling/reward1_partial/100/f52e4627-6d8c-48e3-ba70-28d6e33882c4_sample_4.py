...
# Define exclusive choice for negotiation and logistics
negotiation = OperatorPOWL(operator=Operator.XOR, children=[price_negotiate, skip])

# Define loop for continuous relationship management
relationship_management = OperatorPOWL(operator=Operator.LOOP, children=[relationship_call, traceability_log])

# Define partial order for the process
root = StrictPartialOrder(nodes=[
    farm_visit, quality_cupping, sustainability_audit, contract_draft, sample_testing, shipment_plan, customs_clear, inventory_update, supplier_review, risk_assess, forecast_adjust, payment_process, relationship_management, market_research, compliance_check
])

# Add dependencies between nodes
root.order.add_edge(farm_visit, quality_cupping)
root.order.add_edge(farm_visit, sustainability_audit)
root.order.add_edge(quality_cupping, contract_draft)
...
root.order.add_edge(payment_process, relationship_management)
root.order.add_edge(relationship_management, traceability_log)
