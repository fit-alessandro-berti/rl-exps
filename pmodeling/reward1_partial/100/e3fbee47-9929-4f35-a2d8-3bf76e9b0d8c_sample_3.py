...
# Define the exclusive choice for special orders and regular orders
special_orders = OperatorPOWL(operator=Operator.XOR, children=[packaging_design, retail_setup])

# Define the loop for batch production and regular orders
loop = OperatorPOWL(operator=Operator.LOOP, children=[batch_pasteurize, culture_add, curd_cut, whey_drain, mold_inoculate, press_form, salt_rub, aging_monitor, flavor_adjust, special_orders])

# Define the root POWL model
root = StrictPartialOrder(nodes=[loop, cold_storage, delivery_schedule, order_processing, farm_selection, milk_testing, packaging_design, retail_setup, feedback_collect])
root.order.add_edge(loop, cold_storage)
root.order.add_edge(loop, delivery_schedule)
root.order.add_edge(loop, order_processing)
...
root.order.add_edge(aging_monitor, special_orders)
root.order.add_edge(special_orders, packaging_design)
root.order.add_edge(special_orders, retail_setup)
root.order.add_edge(packaging_design, label_approval)
root.order.add_edge(retail_setup, feedback_collect)
root.order.add_edge(feedback_collect, farm_selection)