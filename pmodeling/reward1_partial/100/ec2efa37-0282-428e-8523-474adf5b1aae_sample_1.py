...
inventory_loop = OperatorPOWL(operator=Operator.LOOP, children=[inventory_sync, procurement_plan, production_align])
iot_loop = OperatorPOWL(operator=Operator.LOOP, children=[iot_monitor, risk_assess, maintenance_alert, compliance_check])
...
root.order.add_edge(ai_forecast, inventory_loop)
root.order.add_edge(inventory_loop, distribution_map)
root.order.add_edge(distribution_map, iot_loop)
root.order.add_edge(iot_loop, feedback_loop)
