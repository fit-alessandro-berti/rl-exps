...
exclusive_choice = OperatorPOWL(operator=Operator.XOR, children=[risk_assessment, buyer_outreach])
loop = OperatorPOWL(operator=Operator.LOOP, children=[negotiation_phase, contract_drafting, approval_process, portfolio_tracking, revenue_monitoring, compliance_check, innovation_scan, renewal_management])

# Define the root model
root = StrictPartialOrder(nodes=[patent_scouting, technical_review, legal_audit, market_analysis, valuation_modeling, exclusive_choice, loop])
...
root.order.add_edge(valuation_modeling, exclusive_choice)
