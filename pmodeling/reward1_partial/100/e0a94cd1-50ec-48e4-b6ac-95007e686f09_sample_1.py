...
# Define the POWL model for the given process
...
xor = OperatorPOWL(operator=Operator.XOR, children=[anomaly_detect, supplier_sync, quantum_communicate])
xor2 = OperatorPOWL(operator=Operator.XOR, children=[inventory_adjust, procurement_plan, performance_track])
loop = OperatorPOWL(operator=Operator.LOOP, children=[network_adapt, xor2])

# Create the root partial order
root = StrictPartialOrder(nodes=[data_ingestion, quantum_setup, route_optimize, demand_forecast, scenario_simulate, risk_assess, xor, xor2, loop])
...
root.order.add_edge(loop, network_adapt)