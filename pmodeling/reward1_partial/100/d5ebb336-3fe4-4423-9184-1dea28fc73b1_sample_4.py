...
xor2 = OperatorPOWL(operator=Operator.XOR, children=[system_audit, skip])

# Create the root POWL model
root = StrictPartialOrder(nodes=[data_collection, point_aggregation, conflict_check, fraud_scan, reward_adjust, redemption_verify, partner_sync, behavior_analyze, xor, loop, xor2, async_update, rollback_trigger, compliance_check, notification_send, user_feedback, report_generate, system_audit])
...
root.order.add_edge(partner_sync, xor)
root.order.add_edge(behavior_analyze, xor2)
root.order.add_edge(xor, loop)
root.order.add_edge(loop, async_update)
root.order.add_edge(async_update, rollback_trigger)
root.order.add_edge(rollback_trigger, system_audit)
root.order.add_edge(system_audit, notification_send)
root.order.add_edge(notification_send, user_feedback)
root.order.add_edge(user_feedback, report_generate)
root.order.add_edge(report_generate, compliance_check)
root.order.add_edge(compliance_check, system_audit)
