...
xor = OperatorPOWL(operator=Operator.XOR, children=[climate_packing, cold_storage])

# Define the loop node
loop = OperatorPOWL(operator=Operator.LOOP, children=[freight_booking, feedback_collection])

# Define the partial order
root = StrictPartialOrder(nodes=[milk_sourcing, quality_testing, batch_selection, curd_preparation, pressing_cheese, aging_control, flavor_profiling, packaging_prep, xor, loop, export_licensing, customs_filing, retail_delivery, feedback_collection])
...
root.order.add_edge(packaging_prep, xor)
...
root.order.add_edge(feedback_collection, loop)
