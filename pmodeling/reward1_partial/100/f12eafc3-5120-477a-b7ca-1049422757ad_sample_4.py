...
root = StrictPartialOrder(nodes=[
    milk_selection, quality_testing, milk_pasturize, cheese_crafting, controlled_aging, sensory_review,
    custom_packaging, label_printing, export_licensing, documentation_prep, customs_clearance, cold_shipping,
    delivery_tracking, xor, market_analysis
])
...
# Add a loop for the sensory review
loop = OperatorPOWL(operator=Operator.LOOP, children=[sensory_review, feedback_review])
root.order.add_edge(sensory_review, loop)
root.order.add_edge(loop, sensory_review)

# Add a loop for the feedback review
loop_feedback = OperatorPOWL(operator=Operator.LOOP, children=[feedback_review, market_analysis])
root.order.add_edge(feedback_review, loop_feedback)
root.order.add_edge(loop_feedback, feedback_review)
