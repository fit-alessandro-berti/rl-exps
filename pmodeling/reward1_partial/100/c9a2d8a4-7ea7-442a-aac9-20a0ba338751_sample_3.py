...
exclusive_choice = OperatorPOWL(operator=Operator.XOR, children=[identify_gaps, recommend_actions, implement_changes])
loop = OperatorPOWL(operator=Operator.LOOP, children=[gather_documents, verify_suppliers, screen_transactions, classify_products, assess_risks, check_sanctions, review_tariffs, cross_verify_data, conduct_interviews, analyze_reports, finalize_report])

# Construct the POWL model
root = StrictPartialOrder(nodes=[initiate_audit, loop, exclusive_choice])
root.order.add_edge(initiate_audit, loop)
