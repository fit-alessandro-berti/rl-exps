...
loop1 = OperatorPOWL(operator=Operator.LOOP, children=[forgery_detect, restoration_map])
loop2 = OperatorPOWL(operator=Operator.LOOP, children=[market_analyze, auction_review, value_assess])
xor = OperatorPOWL(operator=Operator.XOR, children=[provenance_trace, skip])

# Define main process
root = StrictPartialOrder(nodes=[initial_inspect, material_test, imaging_scan, historical_check, expert_consult, xor, loop1, loop2, report_draft, board_review, certification, release_artifact, chain_custody])
...
root.order.add_edge(loop1, report_draft)
