...
xor = OperatorPOWL(operator=Operator.XOR, children=[demand_forecast, data_feedback])
loop = OperatorPOWL(operator=Operator.LOOP, children=[sterilize_crop, package_fresh])
xor2 = OperatorPOWL(operator=Operator.XOR, children=[loop, harvest_sync])
xor3 = OperatorPOWL(operator=Operator.XOR, children=[delivery_plan, xor2])
...
root.order.add_edge(data_feedback, loop)
root.order.add_edge(loop, harvest_sync)
root.order.add_edge(harvest_sync, delivery_plan)
root.order.add_edge(delivery_plan, xor2)
root.order.add_edge(xor2, xor3)
root.order.add_edge(xor3, root)