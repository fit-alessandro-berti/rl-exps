...
loop = OperatorPOWL(operator=Operator.LOOP, children=[iot_monitoring, data_analytics, nutrient_adjust, pest_control, maintenance_check])
exclusive_choice = OperatorPOWL(operator=Operator.XOR, children=[logistics_setup, market_launch])

# Define the root POWL model
root = StrictPartialOrder(nodes=[site_analysis, env_assessment, system_design, equipment_order, seed_selection, install_modules, calibrate_systems, staff_training, plant_seeding, loop, exclusive_choice])
...
root.order.add_edge(maintenance_check, loop)
root.order.add_edge(loop, exclusive_choice)
root.order.add_edge(exclusive_choice, logistics_setup)
root.order.add_edge(exclusive_choice, market_launch)
