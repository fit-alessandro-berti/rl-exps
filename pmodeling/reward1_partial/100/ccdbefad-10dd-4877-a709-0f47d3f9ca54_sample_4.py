...
loop = OperatorPOWL(operator=Operator.LOOP, children=[custom_scheduling, feedback_loop])
xor = OperatorPOWL(operator=Operator.XOR, children=[inventory_balance, skip])
...
# Add silent transitions to the model
root.order.add_edge(impact_review, feedback_loop)
root.order.add_edge(feedback_loop, product_inspect)
root.order.add_edge(product_inspect, eco_packaging)
root.order.add_edge(eco_packaging, multi_transport)
root.order.add_edge(multi_transport, route_optimize)
root.order.add_edge(route_optimize, feedback_loop)
