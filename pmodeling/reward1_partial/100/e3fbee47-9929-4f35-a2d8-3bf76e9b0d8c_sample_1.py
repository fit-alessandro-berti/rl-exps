...
xor = OperatorPOWL(operator=Operator.XOR, children=[retail_setup, feedback_collect])
loop = OperatorPOWL(operator=Operator.LOOP, children=[cold_storage, delivery_schedule])
root = StrictPartialOrder(nodes=[farm_selection, milk_testing, batch_pasteurize, culture_add, curd_cut, whey_drain, mold_inoculate, press_form, salt_rub, aging_monitor, flavor_adjust, packaging_design, label_approval, order_processing, xor, loop])
...
root.order.add_edge(order_processing, retail_setup)
root.order.add_edge(retail_setup, feedback_collect)
root.order.add_edge(feedback_collect, retail_setup)
root.order.add_edge(cold_storage, delivery_schedule)
root.order.add_edge(delivery_schedule, cold_storage)