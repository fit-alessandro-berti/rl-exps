...
# Define a loop for pest management
pest_management_loop = OperatorPOWL(operator=Operator.LOOP, children=[pest_management, data_calibration])

# Define a choice between yield analysis and community meet
yield_analysis_or_community_meet = OperatorPOWL(operator=Operator.XOR, children=[yield_analysis, community_meet])

# Define the root POWL model
root = StrictPartialOrder(nodes=[site_survey, design_layout, material_sourcing, unit_assembly, system_wiring, sensor_install, water_testing, nutrient_mix, seed_selection, planting_setup, climate_control, pest_management_loop, yield_analysis_or_community_meet, compliance_check, expansion_plan])

# Add edges to define the flow of activities
...
root.order.add_edge(data_calibration, yield_analysis_or_community_meet)
root.order.add_edge(yield_analysis_or_community_meet, compliance_check)
