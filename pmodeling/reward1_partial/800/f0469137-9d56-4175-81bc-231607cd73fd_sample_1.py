...
loop = OperatorPOWL(operator=Operator.LOOP, children=[pest_detection, pesticide_spray])
exclusive_choice = OperatorPOWL(operator=Operator.XOR, children=[drone_pollinate, pesticide_spray])

# Define the root POWL model
root = StrictPartialOrder(nodes=[site_analysis, climate_setup, nutrient_mix, seed_germinate, auto_planting, irrigation_setup, iot_monitoring, loop, exclusive_choice, robotic_harvest, quality_check, package_product, waste_recycle, energy_optimize, data_logging])
...
root.order.add_edge(exclusive_choice, robotic_harvest)
