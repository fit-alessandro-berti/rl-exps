...
xor1 = OperatorPOWL(operator=Operator.XOR, children=[provenance_verify, client_update])
xor2 = OperatorPOWL(operator=Operator.XOR, children=[ethics_review, final_report])
xor3 = OperatorPOWL(operator=Operator.XOR, children=[archive_store])

root = StrictPartialOrder(nodes=[initial_assess, artifact_scan, condition_map, material_test, cleaning_phase, stability_check, minor_repair, structural_reinforce, surface_restore, coating_apply, xor1, xor2, xor3])
...
root.order.add_edge(coating_apply, xor1)
root.order.add_edge(coating_apply, xor2)
root.order.add_edge(coating_apply, xor3)
root.order.add_edge(xor1, final_report)
root.order.add_edge(xor1, archive_store)
root.order.add_edge(xor2, client_update)
root.order.add_edge(xor2, archive_store)
root.order.add_edge(xor3, final_report)
root.order.add_edge(xor3, client_update)