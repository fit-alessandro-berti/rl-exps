...
root = StrictPartialOrder(nodes=[inspect, verify_provenance, document_condition, disassemble, clean_components, analyze_damage, select_materials, perform_repairs, match_finishes, apply_treatments, reassemble, quality_check, photograph_results, update_archive, client_review, finalize_report])

# Define the control-flow operators
root.order.add_edge(inspect, verify_provenance)
root.order.add_edge(verify_provenance, document_condition)
root.order.add_edge(document_condition, disassemble)
root.order.add_edge(disassemble, clean_components)
...
root.order.add_edge(reassemble, quality_check)
