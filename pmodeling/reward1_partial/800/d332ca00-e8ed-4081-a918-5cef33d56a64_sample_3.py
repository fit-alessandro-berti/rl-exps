...
root = StrictPartialOrder(nodes=[
    IntakeDocument, VisualInspect, ImagingScan, MaterialTest, DatabaseCross, ProvenanceCheck, ExpertConsult,
    CarbonDating, ForensicAnalyze, AnomalyReview, RiskAssess, ReportDraft, InsuranceQuote, StoragePlan, FinalApproval
])

# Define the dependencies
root.order.add_edge(IntakeDocument, VisualInspect)
root.order.add_edge(IntakeDocument, ImagingScan)
root.order.add_edge(IntakeDocument, MaterialTest)
root.order.add_edge(IntakeDocument, DatabaseCross)
root.order.add_edge(VisualInspect, ProvenanceCheck)
root.order.add_edge(ImagingScan, ProvenanceCheck)
root.order.add_edge(MaterialTest, ProvenanceCheck)
