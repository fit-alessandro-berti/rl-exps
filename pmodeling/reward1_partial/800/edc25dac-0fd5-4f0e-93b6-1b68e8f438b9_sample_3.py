...
# Create exclusive choice for community engagement and compliance check
community_choice = OperatorPOWL(operator=Operator.XOR, children=[community_meet, compliance_check])

# Create loop for continuous monitoring and adaptive management
monitor_loop = OperatorPOWL(operator=Operator.LOOP, children=[growth_monitor, waste_recycle])

# Create the root node with the defined activities and control-flow operators
root = StrictPartialOrder(nodes=[
    load_assess, permit_review, site_survey, design_layout, soil_mix, install_beds, 
    irrigation_set, climate_test, sensor_deploy, energy_setup, crop_select, plant_seeding, 
    community_choice, monitor_loop, harvest_plan, waste_recycle
])
...
root.order.add_edge(plant_seeding, community_choice)
root.order.add_edge(community_choice, monitor_loop)
