...
# Define the loop node for aging control
aging_loop = OperatorPOWL(operator=Operator.LOOP, children=[aging_control])

# Define the XOR node for demand forecast and limited release
demand_release_xor = OperatorPOWL(operator=Operator.XOR, children=[demand_forecast, limited_release])

# Define the partial order
root = StrictPartialOrder(nodes=[milk_sourcing, farm_audit, milk_testing, batch_forming, curd_cutting, molding_cheese, salting_process, aging_loop, quality_check, packaging_design, label_printing, inventory_update, order_receiving, retail_coordination, shipping_prep, customer_feedback, demand_release_xor])
...
root.order.add_edge(customer_feedback, demand_release_xor)
root.order.add_edge(demand_release_xor, demand_forecast)
root.order.add_edge(demand_release_xor, limited_release)
