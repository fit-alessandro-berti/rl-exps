...
root = StrictPartialOrder(nodes=[
    ...
    microbial_profiling, aging_control, hand_wrapping, quality_audit, packaging_prep,
    climate_shipping, retail_coordination, seasonal_review, consumer_survey,
    feedback_analysis, market_adjustment
])

# Define the dependencies
root.order.add_edge(farm_selection, milk_testing)
root.order.add_edge(farm_selection, starter_culture)
root.order.add_edge(farm_selection, curd_formation)
root.order.add_edge(farm_selection, pressing_cheese)
root.order.add_edge(milk_testing, microbial_profiling)
