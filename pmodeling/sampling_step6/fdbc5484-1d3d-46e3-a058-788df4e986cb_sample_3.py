...
# The order of execution can be defined if there are any dependencies
# For example, if 'Asset ID' must be completed before 'Value Assess', add the edge as follows:
# root.order.add_edge(asset_id, value_assess)

# If the process is entirely sequential, no edges are needed
# root.order.add_edge(asset_id, value_assess)
...
# If there are any choices, they can be represented as exclusive choices or loops
# For example, a loop could be defined as follows:
