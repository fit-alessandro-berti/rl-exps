...
root = StrictPartialOrder(nodes=[artifact_intake, visual_inspection, material_testing, radiocarbon_dating, provenance_check, archive_research, expert_review, style_analysis, craftsmanship_eval, condition_check, restoration_plan, forger_risk, legal_review, report_drafting, catalog_entry])

# Since the process flow is not fully defined, we assume the activities are connected in a linear sequence.
...
root.order.add_edge(artifact_intake, style_analysis)
root.order.add_edge(artifact_intake, craftsmanship_eval)
root.order.add_edge(artifact_intake, condition_check)
root.order.add_edge(artifact_intake, restoration_plan)
root.order.add_edge(artifact_intake, forger_risk)
