...
# Since there are no dependencies between activities in this process, no edges are added.
# If there were dependencies, they would be added here using root.order.add_edge(source, target).
# For example, if 'Impact Assess' needs to happen before 'Team Assemble', you would add:
# root.order.add_edge(impact_assess, team_assemble)

print("POWL model for the process has been defined.")