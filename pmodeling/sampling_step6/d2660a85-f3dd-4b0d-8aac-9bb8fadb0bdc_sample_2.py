...
# Add dependencies between activities (POWL order)
...
root.order.add_edge(site_survey, setup_lighting)
root.order.add_edge(site_survey, deploy_sensors)
root.order.add_edge(site_survey, train_staff)
root.order.add_edge(site_survey, start_cultivation)
...
root.order.add_edge(site_survey, pack_produce)
root.order.add_edge(site_survey, distribute_goods)

# Now 'root' contains the POWL model for the urban vertical farming operation setup process