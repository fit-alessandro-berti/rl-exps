...
# Optionally, you can add dependencies between activities if needed
# For example, to indicate that 'Tech Deployment' should happen before 'Live Monitoring':
# root.order.add_edge(tech_deployment, live_monitoring)

# You can also add silent transitions if necessary
# silent_transition = SilentTransition()

# The root variable now holds the entire POWL model for the crisis simulation process.