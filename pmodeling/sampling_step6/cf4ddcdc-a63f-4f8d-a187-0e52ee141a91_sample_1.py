...
# Define the activities with their exact names as given
...
# The order is not explicitly defined in the problem statement, but if we assume a linear flow, we can define it as follows:
# For example, assuming a linear flow (which is not specified in the problem, but we can infer based on the activities):
...
# The above edges define a linear flow, but if the order is more complex, additional edges would need to be added.
