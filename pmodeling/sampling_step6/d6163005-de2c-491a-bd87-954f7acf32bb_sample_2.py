...
root = StrictPartialOrder(nodes=[req_analysis, comp_sourcing, quality_check, frame_assembly, motor_installation, sensor_setup, control_unit, firmware_upload, system_calibration, flight_testing, error_correction, cosmetic_finish, packaging_prep, user_manual, client_training, support_scheduling])

# Add dependencies between activities
root.order.add_edge(req_analysis, comp_sourcing)
root.order.add_edge(req_analysis, quality_check)
...
root.order.add_edge(control_unit, system_calibration)
...
root.order.add_edge(cosmetic_finish, user_manual)
root.order.add_edge(user_manual, client_training)
root.order.add_edge(user_manual, support_scheduling)
