...
root.order.add_edge(system_install, pest_check)
root.order.add_edge(system_install, sensor_calibrate)
root.order.add_edge(system_install, data_integration)
root.order.add_edge(data_integration, crop_planting)
root.order.add_edge(data_integration, growth_monitor)
root.order.add_edge(data_integration, yield_analyze)
root.order.add_edge(data_integration, waste_manage)
root.order.add_edge(data_integration, energy_audit)

# Now 'root' is the POWL model for the urban vertical farming facility establishment process