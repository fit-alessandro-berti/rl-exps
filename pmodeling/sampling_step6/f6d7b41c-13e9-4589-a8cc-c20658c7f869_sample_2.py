...
root = StrictPartialOrder(nodes=[
    client_consult, needs_assess, art_selection, inventory_check, legal_draft, contract_sign,
    insurance_setup, transport_plan, secure_transit, art_install, condition_check, appraisal_log,
    lease_renew, payment_process, deinstall_art, return_inspect, purchase_option
])

# Add dependencies
root.order.add_edge(client_consult, needs_assess)
root.order.add_edge(client_consult, art_selection)
root.order.add_edge(needs_assess, art_selection)
root.order.add_edge(needs_assess, inventory_check)
root.order.add_edge(art_selection, legal_draft)
root.order.add_edge(art_selection, contract_sign)
root.order.add_edge(insurance_setup, contract_sign)
root.order.add_edge(transport_plan, secure_transit)
root.order.add_edge(secure_transit, art_install)
root.order.add_edge(condition_check, art_install)
root.order.add_edge(appraisal_log, art_install)
