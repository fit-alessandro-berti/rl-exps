...
loop = OperatorPOWL(operator=Operator.LOOP, children=[aging_monitor, skip])

# Define the choice between aging monitor and skip
xor = OperatorPOWL(operator=Operator.XOR, children=[flavor_adjust, skip])

# Define the partial order
root = StrictPartialOrder(nodes=[farm_selection, milk_testing, batch_pasteurize, culture_add, curd_cut, whey_drain, mold_inoculate, press_form, salt_rub, loop, xor, packaging_design, label_approval, order_processing, cold_storage, delivery_schedule, retail_setup, feedback_collect])
...
root.order.add_edge(loop, aging_monitor)
root.order.add_edge(aging_monitor, xor)
root.order.add_edge(xor, flavor_adjust)
