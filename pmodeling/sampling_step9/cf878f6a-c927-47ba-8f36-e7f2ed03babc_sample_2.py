...
xor = OperatorPOWL(operator=Operator.XOR, children=[ProcureGear, skip])

# Define loop
loop = OperatorPOWL(operator=Operator.LOOP, children=[InstallSystems, SetupSensors])

# Define partial order
root = StrictPartialOrder(nodes=[SiteAssess, ZoningCheck, DesignFarm, loop, xor, SelectCrops, PrepareSeeds, MixNutrients, MonitorGrowth, AdjustClimate, RoboticHarvest, GradeQuality, PackProduce, ManageLogistics, MarketProducts, RecycleWaste, AuditSystems])
root.order.add_edge(SiteAssess, ZoningCheck)
root.order.add_edge(ZoningCheck, DesignFarm)
root.order.add_edge(DesignFarm, loop)
root.order.add_edge(loop, xor)
root.order.add_edge(xor, ProcureGear)
