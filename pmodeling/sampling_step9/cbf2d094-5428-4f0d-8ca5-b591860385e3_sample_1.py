...
# Define loop for growth analyze and harvesting
loop_growth = OperatorPOWL(operator=Operator.LOOP, children=[growth_analyze, harvest_sync])
loop_sterilize = OperatorPOWL(operator=Operator.LOOP, children=[sterilize_crop, package_fresh])

# Define choice for data feedback
xor_data = OperatorPOWL(operator=Operator.XOR, children=[data_feedback, skip])

# Define partial order with all activities
root = StrictPartialOrder(nodes=[seed_select, nutrient_mix, climate_adjust, planting_robotic, growth_monitor, pest_control, water_recycle, light_optimize, loop_growth, loop_sterilize, xor_data, demand_forecast, delivery_plan])
...
root.order.add_edge(data_feedback, xor_data)
root.order.add_edge(xor_data, loop_growth)
root.order.add_edge(xor_data, loop_sterilize)