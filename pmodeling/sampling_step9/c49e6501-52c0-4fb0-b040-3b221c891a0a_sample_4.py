...
validation_pass_loop = OperatorPOWL(operator=Operator.LOOP, children=[validation_pass])
live_monitor_loop = OperatorPOWL(operator=Operator.LOOP, children=[live_monitor])

root = StrictPartialOrder(nodes=[component_check, spec_review_choice, parts_sorting_choice, mechanical_fit_choice,
                                 firmware_load, calibration_run, stress_test_loop, software_patch_loop,
                                 algorithm_tune_loop, comms_setup, validation_pass_loop, data_link, onsite_deploy,
                                 live_monitor_loop, update_push, recovery_plan, maintenance_log])
root.order.add_edge(component_check, spec_review_choice)
root.order.add_edge(component_check, parts_sorting_choice)
root.order.add_edge(component_check, mechanical_fit_choice)
root.order.add_edge(spec_review_choice, firmware_load)
root.order.add_edge(parts_sorting_choice, mechanical_fit_choice)
root.order.add_edge(mechanical_fit_choice, calibration_run)
...
root.order.add_edge(algorithm_tune_loop, comms_setup)
root.order.add_edge(comms_setup, validation_pass_loop)
root.order.add_edge(validation_pass_loop, data_link)
root.order.add_edge(data_link, onsite_deploy)
root.order.add_edge(onsite_deploy, live_monitor_loop)
root.order.add_edge(live_monitor_loop, update_push)
