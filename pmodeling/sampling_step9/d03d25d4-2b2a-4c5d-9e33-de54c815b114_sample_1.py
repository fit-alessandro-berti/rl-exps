...
# Define the process
loop1 = OperatorPOWL(operator=Operator.LOOP, children=[farm_registration, lot_tagging, soil_testing, harvest_logging])
loop2 = OperatorPOWL(operator=Operator.LOOP, children=[coffee_sorting, sensory_profiling, quality_scoring])
loop3 = OperatorPOWL(operator=Operator.LOOP, children=[blockchain_entry, environmental_audit, farmer_feedback])
loop4 = OperatorPOWL(operator=Operator.LOOP, children=[dynamic_pricing, roast_scheduling, batch_testing])
loop5 = OperatorPOWL(operator=Operator.LOOP, children=[certification_review, distribution_prep])
loop6 = OperatorPOWL(operator=Operator.LOOP, children=[consumer_feedback])
