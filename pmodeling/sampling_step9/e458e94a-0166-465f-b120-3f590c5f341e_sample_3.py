...
feedback_cycle = OperatorPOWL(operator=Operator.XOR, children=[Transition(label='Feedback Cycle'), SilentTransition()])
...
feedback_loop = OperatorPOWL(operator=Operator.LOOP, children=[feedback_cycle, delay_mitigate])
delay_loop = OperatorPOWL(operator=Operator.LOOP, children=[revision_manage, delay_mitigate])

# Define the partial order
root = StrictPartialOrder(nodes=[inquiry_review, client_onboard, concept_draft, feedback_loop, contract_setup, payment_schedule, material_sourcing, artwork_create, quality_check, frame_selection, packaging_prep, shipment_arrange, delivery_confirm, post_sale_support, delay_loop])
...
root.order.add_edge(post_sale_support, delay_loop)
root.order.add_edge(delay_loop, feedback_loop)
root.order.add_edge(delay_loop, delay_mitigate)