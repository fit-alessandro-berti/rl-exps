...
# Define exclusive choice (XOR) between two activities
xor = OperatorPOWL(operator=Operator.XOR, children=[crop_select, waste_manage])

# Define loop for nutrient solution formulation and crop selection
loop = OperatorPOWL(operator=Operator.LOOP, children=[nutrient_mix, crop_select])

# Define partial order with dependencies
root = StrictPartialOrder(nodes=[site_survey, structure_prep, system_install, env_control, loop, xor, ai_setup, worker_train, pest_control, irrigation_plan, data_monitor, yield_forecast, energy_audit, market_setup, logistics_plan, waste_manage])
...
root.order.add_edge(env_control, loop)
root.order.add_edge(loop, xor)
root.order.add_edge(xor, ai_setup)
