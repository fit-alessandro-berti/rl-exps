...
xor = OperatorPOWL(operator=Operator.XOR, children=[expert_review, skip])

# Define loop
loop = OperatorPOWL(operator=Operator.LOOP, children=[legal_audit, export_verify])

# Define partial order
root = StrictPartialOrder(nodes=[provenance_check, radiocarbon_test, material_analysis, microscopic_scan, xor, loop, final_approval, catalog_entry, virtual_setup, archival_backup])
...
root.order.add_edge(radiocarbon_test, xor)
root.order.add_edge(material_analysis, xor)
root.order.add_edge(microscopic_scan, xor)
