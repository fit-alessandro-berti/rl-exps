...
xor = OperatorPOWL(operator=Operator.XOR, children=[skip, display_approve])
loop = OperatorPOWL(operator=Operator.LOOP, children=[database_search, legal_audit, cultural_assess, data_synthesis, report_draft, archival_store])
xor2 = OperatorPOWL(operator=Operator.XOR, children=[investigation_flag, lender_notify])
xor3 = OperatorPOWL(operator=Operator.XOR, children=[display_approve, lender_notify])

root = StrictPartialOrder(nodes=[artifact_intake, provenance_check, material_scan, style_compare, digital_capture, expert_review, database_search, legal_audit, cultural_assess, data_synthesis, report_draft, archival_store, display_approve, lender_notify, investigation_flag, xor, loop, xor2, xor3])
...
root.order.add_edge(xor3, display_approve)
root.order.add_edge(xor3, lender_notify)