...
# Define the loops
refinement_loop = OperatorPOWL(operator=Operator.LOOP, children=[blend_experiment, quality_check, maturation_cycle, sensory_panel, stability_test])
batch_coordination_loop = OperatorPOWL(operator=Operator.LOOP, children=[batch_coordination])

# Define the XOR node
xor = OperatorPOWL(operator=Operator.XOR, children=[refinement_loop, skip])

# Define the root
root = StrictPartialOrder(nodes=[client_profiling, ingredient_sourcing, quality_check, blend_experiment, maturation_cycle, sensory_panel, refinement_loop, stability_test, packaging_design, batch_coordination, compliance_audit, market_survey, feedback_review, order_finalize, distribution_plan, inventory_update, xor])
...
root.order.add_edge(inventory_update, xor)