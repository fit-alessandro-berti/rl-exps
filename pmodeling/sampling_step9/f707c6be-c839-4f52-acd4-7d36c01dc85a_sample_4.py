...
# Define the loops
artifact_processing = OperatorPOWL(operator=Operator.LOOP, children=[artifact_intake, condition_check, multi_spectral_scan, material_test, database_match, provenance_check, expert_review, historical_query, lab_collaboration, imaging_analysis, forgery_detection, legal_drafting, certification_issue, client_briefing, archival_update])

# Define the exclusive choices
artifact_processing_with_forgery = OperatorPOWL(operator=Operator.XOR, children=[artifact_processing, skip])

# Define the root
root = StrictPartialOrder(nodes=[artifact_processing_with_forgery])
root.order.add_edge(artifact_processing, artifact_processing_with_forgery)
