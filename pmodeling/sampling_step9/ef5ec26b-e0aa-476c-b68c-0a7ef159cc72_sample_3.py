...
loop1 = OperatorPOWL(operator=Operator.LOOP, children=[site_survey, soil_testing, stakeholder_meet, resource_plan, crop_selection, volunteer_signup])
loop2 = OperatorPOWL(operator=Operator.LOOP, children=[tech_setup, irrigation_check, data_collection, growth_monitoring, conflict_mediate])
loop3 = OperatorPOWL(operator=Operator.LOOP, children=[workshop_prep, market_forecast, yield_analysis, sustainability_audit])
loop4 = OperatorPOWL(operator=Operator.LOOP, children=[community_feedback, equipment_maintain, waste_manage])
