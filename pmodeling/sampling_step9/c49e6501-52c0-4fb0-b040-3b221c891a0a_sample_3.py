...
stress_loop = OperatorPOWL(operator=Operator.LOOP, children=[stress_test])
calibration_loop = OperatorPOWL(operator=Operator.LOOP, children=[calibration_run])
...
comms_setup_loop = OperatorPOWL(operator=Operator.LOOP, children=[comms_setup])

# Define the exclusive choices
stress_choice = OperatorPOWL(operator=Operator.XOR, children=[stress_loop, skip])
calibration_choice = OperatorPOWL(operator=Operator.XOR, children=[calibration_loop, skip])
software_patch_choice = OperatorPOWL(operator=Operator.XOR, children=[software_patch_loop, skip])
algorithm_tune_choice = OperatorPOWL(operator=Operator.XOR, children=[algorithm_tune_loop, skip])
comms_setup_choice = OperatorPOWL(operator=Operator.XOR, children=[comms_setup_loop, skip])
...
root.order.add_edge(calibration_run, calibration_choice)
root.order.add_edge(calibration_choice, stress_test)
root.order.add_edge(stress_test, stress_choice)
root.order.add_edge(stress_choice, software_patch)
root.order.add_edge(software_patch, software_patch_choice)
root.order.add_edge(software_patch_choice, algorithm_tune)
root.order.add_edge(algorithm_tune, algorithm_tune_choice)
root.order.add_edge(algorithm_tune_choice, comms_setup)
root.order.add_edge(comms_setup, comms_setup_choice)
root.order.add_edge(comms_setup_choice, validation_pass)
