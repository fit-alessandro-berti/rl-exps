...
xor1 = OperatorPOWL(operator=Operator.XOR, children=[refinement_loop, skip])
xor2 = OperatorPOWL(operator=Operator.XOR, children=[compliance_audit, skip])
xor3 = OperatorPOWL(operator=Operator.XOR, children=[market_survey, skip])
xor4 = OperatorPOWL(operator=Operator.XOR, children=[feedback_review, skip])
xor5 = OperatorPOWL(operator=Operator.XOR, children=[order_finalize, skip])

root = StrictPartialOrder(nodes=[client_profiling, ingredient_sourcing, quality_check, blend_experiment, maturation_cycle, sensory_panel, stability_test, packaging_design, batch_coordination, xor1, xor2, xor3, xor4, xor5, distribution_plan, inventory_update])
...
root.order.add_edge(batch_coordination, xor1)
root.order.add_edge(compliance_audit, xor2)
root.order.add_edge(market_survey, xor3)
root.order.add_edge(feedback_review, xor4)
root.order.add_edge(order_finalize, xor5)
root.order.add_edge(xor1, distribution_plan)
root.order.add_edge(xor2, distribution_plan)
root.order.add_edge(xor3, distribution_plan)
root.order.add_edge(xor4, distribution_plan)
