...
loop = OperatorPOWL(operator=Operator.LOOP, children=[provenance_search, ownership_validate, radiocarbon_test, spectroscopy_scan, material_analysis, style_assessment, context_review, expert_panel])
xor = OperatorPOWL(operator=Operator.XOR, children=[report_draft, skip])
root = StrictPartialOrder(nodes=[artifact_intake, document_check, loop, xor, quality_review, catalog_entry, insurance_setup, archive_data, reevaluation_trigger])
root.order.add_edge(artifact_intake, document_check)
root.order.add_edge(document_check, loop)
root.order.add_edge(loop, xor)
root.order.add_edge(xor, quality_review)
