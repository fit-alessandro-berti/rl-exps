...
energy_audit_loop = OperatorPOWL(operator=Operator.LOOP, children=[energy_audit, system_upgrade])

# Define the partial order for the entire process
root = StrictPartialOrder(nodes=[site_survey, design_layout, climate_setup, sensor_install, nutrient_mix, automation_code, crop_planning, pest_choice, energy_audit_loop, waste_sort, planting_tier, harvest_prep, logistics_plan, community_meet, data_review])
...
root.order.add_edge(site_survey, pest_choice)
root.order.add_edge(site_survey, energy_audit_loop)
