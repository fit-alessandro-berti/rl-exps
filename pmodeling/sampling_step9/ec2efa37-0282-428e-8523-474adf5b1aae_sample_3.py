...
loop = OperatorPOWL(operator=Operator.LOOP, children=[quantum_compute, feedback_loop])

# Define the partial order
root = StrictPartialOrder(nodes=[xor, loop, data_ingest, ai_forecast, inventory_sync, procurement_plan, production_align, distribution_map, iot_monitor, risk_assess, maintenance_alert, schedule_adjust, demand_update, delivery_track, compliance_check])
root.order.add_edge(xor, loop)
root.order.add_edge(loop, data_ingest)
