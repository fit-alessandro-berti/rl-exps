...
xor = OperatorPOWL(operator=Operator.XOR, children=[exit_strategy, skip])

# Define the loop
loop = OperatorPOWL(operator=Operator.LOOP, children=[feasibility_test, pilot_launch, feedback_gather, design_adapt, compliance_check, scaling_plan, ip_management, market_sync, partner_review, xor])

# Define the root of the POWL model
root = StrictPartialOrder(nodes=[loop, idea_workshop, concept_merge, resource_align, prototype_build, feasibility_test, pilot_launch, feedback_gather, design_adapt, compliance_check, scaling_plan, ip_management, market_sync, partner_review, exit_strategy])
root.order.add_edge(loop, idea_workshop)
...
root.order.add_edge(partner_review, xor)
