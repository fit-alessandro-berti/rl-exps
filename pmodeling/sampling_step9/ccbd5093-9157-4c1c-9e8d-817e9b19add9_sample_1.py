...
xor = OperatorPOWL(operator=Operator.XOR, children=[align_strategy, skip])

root = StrictPartialOrder(nodes=[loop, xor])
root.order.add_edge(loop, xor)

# Add edges to connect the activities within the loop and XOR
root.order.add_edge(loop, develop_prototypes)
...
root.order.add_edge(cycle_renewal, loop)