...
# Define the exclusive choice for drone customization
xor = OperatorPOWL(operator=Operator.XOR, children=[drone_config, skip])

# Define the loop for route programming and compliance checks
loop = OperatorPOWL(operator=Operator.LOOP, children=[route_program, compliance_check])

# Define the process model
root = StrictPartialOrder(nodes=[client_onboard, needs_assess, xor, loop, insurance_verify, lease_contract, fleet_deploy, monitor_setup, usage_track, maintenance_plan, incident_manage, billing_process, performance_report, contract_renew, price_adjust, feedback_collect])
root.order.add_edge(client_onboard, needs_assess)
root.order.add_edge(needs_assess, xor)
root.order.add_edge(xor, loop)
root.order.add_edge(loop, insurance_verify)
...
# Print the root of the process model
print(root)