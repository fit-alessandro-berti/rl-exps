...
loop = OperatorPOWL(operator=Operator.LOOP, children=[site_survey, climate_check])
xor = OperatorPOWL(operator=Operator.XOR, children=[soil_testing, media_select, design_layout, hydro_setup, nutrient_mix, sensor_install])

# Define root POWL model
root = StrictPartialOrder(nodes=[loop, xor, regulation_review, permit_apply, stakeholder_meet, community_train, plant_seed, monitor_growth, adjust_controls, harvest_plan, waste_recycle, feedback_loop])
