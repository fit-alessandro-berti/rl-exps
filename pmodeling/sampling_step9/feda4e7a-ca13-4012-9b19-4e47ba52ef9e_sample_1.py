...
# Define loop nodes
loop = OperatorPOWL(operator=Operator.LOOP, children=[pest_control, ai_diagnostics])

# Define exclusive choices
xor = OperatorPOWL(operator=Operator.XOR, children=[waste_recycle, skip])

# Define the root POWL model
root = StrictPartialOrder(nodes=[site_analysis, env_scanning, farm_design, nutrient_mix, seed_automation, growth_monitor, loop, harvest_plan, robotic_sort, packaging_line, community_input, data_aggregation, xor, sustainability])
...
root.order.add_edge(data_aggregation, xor)
