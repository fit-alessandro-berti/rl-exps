...
# Define exclusive choice for artifact recovery and archival
xor = OperatorPOWL(operator=Operator.XOR, children=[secure_transport, heritage_archive])

# Define loop for artifact restoration and archival
loop = OperatorPOWL(operator=Operator.LOOP, children=[restoration_begin, documentation_log, final_audit])

# Define the root POWL model
root = StrictPartialOrder(nodes=[artifact_scan, ownership_verify, risk_assess, legal_review, stakeholder_notify, recovery_plan, third_party_contact, negotiation_setup, xor, loop, process_close])
...
root.order.add_edge(negotiation_setup, xor)
root.order.add_edge(xor, loop)
root.order.add_edge(loop, process_close)
