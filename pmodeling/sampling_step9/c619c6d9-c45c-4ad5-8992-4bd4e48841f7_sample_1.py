...
loop = OperatorPOWL(operator=Operator.LOOP, children=[firmware_integrate, quality_inspect])

# Define the exclusive choice
xor = OperatorPOWL(operator=Operator.XOR, children=[flight_testing, regulation_check])

# Define the root
root = StrictPartialOrder(nodes=[design_consult, component_sourcing, sensor_calibrate, loop, xor, payload_configure, assembly_setup, wiring_connect, chassis_build, software_load, data_analyze, regulation_check, quality_inspect, packaging_prep, logistics_plan, client_review])
...
root.order.add_edge(firmware_integrate, loop)
root.order.add_edge(loop, xor)
root.order.add_edge(xor, payload_configure)
...
root.order.add_edge(flight_testing, xor)
