...
# Define the exclusive choice (XOR) between regulation review and stakeholder meet
xor = OperatorPOWL(operator=Operator.XOR, children=[regulation_review, stakeholder_meet])

# Define the loop for drone assembly and software setup
loop = OperatorPOWL(operator=Operator.LOOP, children=[drone_assembly, software_setup])

# Define the partial order
root = StrictPartialOrder(nodes=[site_survey, fleet_design, permit_request, xor, loop, test_flight, data_integration, compliance_audit, emergency_plan, launch_prep, feedback_loop, performance_tune, scale_strategy])
root.order.add_edge(site_survey, fleet_design)
root.order.add_edge(fleet_design, permit_request)
root.order.add_edge(permit_request, xor)
root.order.add_edge(xor, loop)
root.order.add_edge(loop, test_flight)
