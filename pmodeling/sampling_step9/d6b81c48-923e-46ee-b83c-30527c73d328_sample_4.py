...
# Define loop nodes
flight_loop = OperatorPOWL(operator=Operator.LOOP, children=[flight_launch, telemetry_monitor, anomaly_detect, collision_assess, data_upload, postflight_review, battery_optimize, damage_repair, compliance_report, performance_log, mission_debrief])
...
# Define XOR nodes
xor = OperatorPOWL(operator=Operator.XOR, children=[preflight_loop, flight_loop])

# Define root POWL model
root = StrictPartialOrder(nodes=[xor])
root.order.add_edge(preflight_loop, xor)
root.order.add_edge(flight_loop, xor)
