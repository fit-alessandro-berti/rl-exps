import pm4py
from pm4py.objects.powl.obj import StrictPartialOrder, OperatorPOWL, Transition, SilentTransition
from pm4py.objects.process_tree.obj import Operator

# Define transitions
artifact_intake = Transition(label='Artifact Intake')
preliminary_check = Transition(label='Preliminary Check')
historical_review = Transition(label='Historical Review')
chemical_test = Transition(label='Chemical Test')
provenance_audit = Transition(label='Provenance Audit')
expert_panel = Transition(label='Expert Panel')
token_minting = Transition(label='Token Minting')
legal_review = Transition(label='Legal Review')
compliance_check = Transition(label='Compliance Check')
insurance_valuation = Transition(label='Insurance Valuation')
risk_assessment = Transition(label='Risk Assessment')
packaging_prep = Transition(label='Packaging Prep')
climate_control = Transition(label='Climate Control')
transport_setup = Transition(label='Transport Setup')
final_approval = Transition(label='Final Approval')

# Define silent transitions
skip = SilentTransition()

# Define loop node
loop = OperatorPOWL(operator=Operator.LOOP, children=[expert_panel, token_minting])

# Define XOR node
xor = OperatorPOWL(operator=Operator.XOR, children=[compliance_check, legal_review])

# Define XOR node
xor2 = OperatorPOWL(operator=Operator.XOR, children=[insurance_valuation, risk_assessment])

# Define XOR node
xor3 = OperatorPOWL(operator=Operator.XOR, children=[packaging_prep, climate_control])

# Define XOR node
xor4 = OperatorPOWL(operator=Operator.XOR, children=[transport_setup, final_approval])

# Define POWL model
root = StrictPartialOrder(nodes=[artifact_intake, preliminary_check, historical_review, chemical_test, provenance_audit, expert_panel, token_minting, legal_review, compliance_check, insurance_valuation, risk_assessment, packaging_prep, climate_control, transport_setup, final_approval, loop, xor, xor2, xor3, xor4])
root.order.add_edge(artifact_intake, preliminary_check)
root.order.add_edge(preliminary_check, historical_review)
root.order.add_edge(preliminary_check, chemical_test)
root.order.add_edge(preliminary_check, provenance_audit)
root.order.add_edge(historical_review, expert_panel)
root.order.add_edge(chemical_test, expert_panel)
root.order.add_edge(provenance_audit, expert_panel)
root.order.add_edge(expert_panel, token_minting)
root.order.add_edge(token_minting, legal_review)
root.order.add_edge(legal_review, compliance_check)
root.order.add_edge(legal_review, insurance_valuation)
root.order.add_edge(compliance_check, insurance_valuation)
root.order.add_edge(insurance_valuation, risk_assessment)
root.order.add_edge(risk_assessment, packaging_prep)
root.order.add_edge(packaging_prep, climate_control)
root.order.add_edge(climate_control, transport_setup)
root.order.add_edge(transport_setup, final_approval)
root.order.add_edge(expert_panel, loop)
root.order.add_edge(legal_review, loop)
root.order.add_edge(compliance_check, xor)
root.order.add_edge(insurance_valuation, xor2)
root.order.add_edge(risk_assessment, xor3)
root.order.add_edge(packaging_prep, xor4)
root.order.add_edge(climate_control, xor4)