...
loop = OperatorPOWL(operator=Operator.LOOP, children=[data_analysis, quality_check])

# Define exclusive choice for the end of the process
xor = OperatorPOWL(operator=Operator.XOR, children=[skip, market_dispatch])

# Define the POWL model for the urban vertical farming facility establishment
root = StrictPartialOrder(nodes=[site_survey, structure_reinforce, hydroponic_setup, climate_install, ai_integration, seed_sourcing, nutrient_prep, system_testing, staff_training, crop_planting, growth_monitor, pest_control, harvest_schedule, quality_check, market_dispatch, waste_recycle, data_analysis, loop, xor])
...
root.order.add_edge(data_analysis, loop)
