...
loop = OperatorPOWL(operator=Operator.LOOP, children=[seed_select, pest_control])
xor = OperatorPOWL(operator=Operator.XOR, children=[waste_sort, skip])
root = StrictPartialOrder(nodes=[site_analyze, soil_enhance, loop, xor, climate_monitor, irrigate_adjust, nutrient_feed, community_engage, feedback_collect, yield_harvest, compost_create, data_analyze, network_distribute])
root.order.add_edge(site_analyze, soil_enhance)
root.order.add_edge(soil_enhance, loop)
root.order.add_edge(loop, climate_monitor)
