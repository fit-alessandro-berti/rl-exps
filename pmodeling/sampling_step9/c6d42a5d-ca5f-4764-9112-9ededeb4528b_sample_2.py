...
xor = OperatorPOWL(operator=Operator.XOR, children=[material_order, skip])

# Define the loop
loop = OperatorPOWL(operator=Operator.LOOP, children=[system_setup, supplier_contact])

# Define the root
root = StrictPartialOrder(nodes=[site_survey, load_test, soil_sample, crop_select, irrigation_plan, permit_apply, xor, bed_install, pest_control, solar_setup, staff_train, market_outreach, loop, health_monitor])
...
root.order.add_edge(xor, bed_install)
...
root.order.add_edge(market_outreach, loop)
root.order.add_edge(loop, health_monitor)