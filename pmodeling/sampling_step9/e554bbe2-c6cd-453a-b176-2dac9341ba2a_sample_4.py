import pm4py
from pm4py.objects.powl.obj import StrictPartialOrder, OperatorPOWL, Transition, SilentTransition
from pm4py.objects.process_tree.obj import Operator

# Define transitions
farm_selection = Transition(label='Farm Selection')
milk_testing = Transition(label='Milk Testing')
starter_culture = Transition(label='Starter Culture')
curd_formation = Transition(label='Curd Formation')
pressing_cheese = Transition(label='Pressing Cheese')
microbial_profiling = Transition(label='Microbial Profiling')
aging_control = Transition(label='Aging Control')
hand_wrapping = Transition(label='Hand Wrapping')
quality_audit = Transition(label='Quality Audit')
packaging_prep = Transition(label='Packaging Prep')
climate_shipping = Transition(label='Climate Shipping')
retail_coordination = Transition(label='Retail Coordination')
seasonal_review = Transition(label='Seasonal Review')
consumer_survey = Transition(label='Consumer Survey')
feedback_analysis = Transition(label='Feedback Analysis')
market_adjustment = Transition(label='Market Adjustment')

# Define silent transitions
skip = SilentTransition()

# Define exclusive choice nodes
xor1 = OperatorPOWL(operator=Operator.XOR, children=[climate_shipping, skip])
xor2 = OperatorPOWL(operator=Operator.XOR, children=[retail_coordination, skip])

# Define loop nodes
loop1 = OperatorPOWL(operator=Operator.LOOP, children=[aging_control, microbial_profiling])
loop2 = OperatorPOWL(operator=Operator.LOOP, children=[hand_wrapping, quality_audit])

# Define partial order
root = StrictPartialOrder(nodes=[farm_selection, milk_testing, starter_culture, curd_formation, pressing_cheese, loop1, loop2, xor1, xor2, packaging_prep, seasonal_review, consumer_survey, feedback_analysis, market_adjustment])
root.order.add_edge(farm_selection, milk_testing)
root.order.add_edge(milk_testing, starter_culture)
root.order.add_edge(starter_culture, curd_formation)
root.order.add_edge(curd_formation, pressing_cheese)
root.order.add_edge(pressing_cheese, loop1)
root.order.add_edge(loop1, loop2)
root.order.add_edge(loop2, xor1)
root.order.add_edge(xor1, packaging_prep)
root.order.add_edge(packaging_prep, xor2)
root.order.add_edge(xor2, seasonal_review)
root.order.add_edge(seasonal_review, consumer_survey)
root.order.add_edge(consumer_survey, feedback_analysis)
root.order.add_edge(feedback_analysis, market_adjustment)

print(root)