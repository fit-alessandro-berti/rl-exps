...
# Define loop for system monitoring
loop = OperatorPOWL(operator=Operator.LOOP, children=[monitor_systems])

# Define exclusive choice for crop selection
xor = OperatorPOWL(operator=Operator.XOR, children=[select_crops, skip])

# Define the root POWL model
root = StrictPartialOrder(nodes=[site_survey, regulation_check, design_modules, install_hydroponics, integrate_sensors, calibrate_nutrients, program_climate, xor, optimize_lighting, train_staff, plan_harvest, recycle_waste, analyze_demand, plan_logistics, loop])
...
root.order.add_edge(program_climate, xor)
root.order.add_edge(xor, optimize_lighting)
