...
xor = OperatorPOWL(operator=Operator.XOR, children=[market_launch, feedback_collect])

# Define the loop
loop = OperatorPOWL(operator=Operator.LOOP, children=[pest_control, growth_monitor, water_testing, sensor_install, material_sourcing, soil_prep, hydroponic_setup])

# Define the partial order
root = StrictPartialOrder(nodes=[loop, xor, site_survey, load_test, permit_review, design_layout, community_meet, crop_select])
...
root.order.add_edge(feedback_collect, market_launch)
root.order.add_edge(market_launch, market_launch)
