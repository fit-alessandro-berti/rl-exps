...
xor = OperatorPOWL(operator=Operator.XOR, children=[regulatory_check, community_meet])
loop = OperatorPOWL(operator=Operator.LOOP, children=[installation, system_testing, yield_analysis, resource_audit, impact_review])
xor2 = OperatorPOWL(operator=Operator.XOR, children=[energy_audit, staff_training])

# Define the root POWL model
root = StrictPartialOrder(nodes=[site_survey, light_mapping, water_testing, design_modules, iot_setup, sensor_calibration, nutrient_mix, climate_control, xor, loop, xor2])
...
root.order.add_edge(climate_control, xor)
root.order.add_edge(xor, loop)
root.order.add_edge(loop, xor2)
root.order.add_edge(xor2, energy_audit)
root.order.add_edge(xor2, staff_training)
