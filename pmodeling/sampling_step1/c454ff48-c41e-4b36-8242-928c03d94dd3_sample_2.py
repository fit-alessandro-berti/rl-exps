...
root.nodes.add(permit_obtain)
root.nodes.add(soil_testing)
root.nodes.add(crop_select)
root.nodes.add(irrigation_setup)
root.nodes.add(drainage_install)
root.nodes.add(energy_integrate)
root.nodes.add(staff_train)
root.nodes.add(pest_control)
root.nodes.add(logistics_plan)
root.nodes.add(supply_coordinate)
root.nodes.add(distribution_map)
root.nodes.add(community_engage)
root.nodes.add(monitoring_setup)
root.nodes.add(yield_optimize)
