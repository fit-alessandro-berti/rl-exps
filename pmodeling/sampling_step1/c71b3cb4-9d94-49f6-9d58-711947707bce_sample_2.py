...
# Define the loop for nutrient mix, water delivery, light adjustment, and climate control
loop = OperatorPOWL(operator=Operator.LOOP, children=[nutrient_mix, water_delivery, light_adjustment, climate_control])

# Define the XOR for pest control and health check
xor = OperatorPOWL(operator=Operator.XOR, children=[pest_control, health_check])

# Define the root POWL model
root = StrictPartialOrder(nodes=[seed_selection, germination_start, seedling_transplant, loop, sensor_monitoring, xor, harvest_planning, crop_harvest, waste_sorting, biomass_process, energy_recycling, data_analysis, yield_forecast])

# Define the edges for the POWL model
root.order.add_edge(seed_selection, germination_start)
root.order.add_edge(germination_start, seedling_transplant)
root.order.add_edge(seedling_transplant, loop)
root.order.add_edge(loop, sensor_monitoring)
root.order.add_edge(sensor_monitoring, xor)
root.order.add_edge(xor, harvest_planning)
