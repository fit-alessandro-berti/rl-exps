...
# Define the loop for nutrient solution calibration
loop = OperatorPOWL(operator=Operator.LOOP, children=[nutrient_mix])

# Define the partial order with the defined activities and the loop
root = StrictPartialOrder(nodes=[site_survey, env_analysis, structure_build, hydroponics_fit, loop, nutrient_mix, climate_setup, energy_audit, crop_select, pest_control, growth_monitor, harvest_plan, waste_recycle, community_meet, supply_sync, data_review])
...
root.order.add_edge(hydroponics_fit, loop)
