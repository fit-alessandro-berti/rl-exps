...
# Define the loop for the expert consultation and archival review
expert_archival_loop = OperatorPOWL(operator=Operator.LOOP, children=[expert_consult, archival_review])

# Define the XOR for the quality inspection and packaging preparation
quality_packaging_xor = OperatorPOWL(operator=Operator.XOR, children=[quality_inspect, packaging_prep])

# Define the root partial order
root = StrictPartialOrder(nodes=[initial_assess, condition_scan, material_test, historical_check, provenance_verify, parts_sourcing, gentle_clean, stabilize_item, structural_repair, surface_finish, expert_archival_loop, quality_packaging_xor, report_generate, certify_provenance])
...
root.order.add_edge(surface_finish, expert_archival_loop)
root.order.add_edge(expert_archival_loop, quality_packaging_xor)
root.order.add_edge(quality_packaging_xor, report_generate)
