...
root.add_transition(req_analysis)
root.add_transition(component_sourcing)
root.add_transition(quality_check)
root.add_transition(frame_assembly)
root.add_transition(motor_installation)
root.add_transition(sensor_setup)
root.add_transition(control_unit)
root.add_transition(firmware_upload)
root.add_transition(system_calibration)
root.add_transition(flight_testing)
root.add_transition(error_correction)
root.add_transition(cosmetic_finish)
root.add_transition(packaging_prep)
root.add_transition(user_manual)
root.add_transition(client_training)
root.add_transition(support_scheduling)

# Define the partial order dependencies
root.order.add_edge(req_analysis, component_sourcing)
