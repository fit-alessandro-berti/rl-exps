...
# Loop for continuous growth cycle: Scan -> Control -> Cycle -> Assess
growth_loop = OperatorPOWL(operator=Operator.LOOP, children=[growth_scan, pest_control, water_cycle, yield_assess])

# Build the partial order
root = StrictPartialOrder(nodes=[
    seed_select, nutrient_mix, sensor_setup, env_monitor,
    growth_loop,
    waste_process, energy_sync, pack_biodeg,
...
root.order.add_edge(env_monitor,   growth_loop)

root.order.add_edge(growth_loop,   waste_process)
root.order.add_edge(growth_loop,   energy_sync)
root.order.add_edge(growth_loop,   pack_biodeg)

root.order.add_edge(waste_process, market_track)
root.order.add_edge(energy_sync,   market_track)
