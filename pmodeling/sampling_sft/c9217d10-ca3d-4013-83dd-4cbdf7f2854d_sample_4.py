...
# Define the data-monitoring loop: Monitor -> Yield Analyze -> Supply Chain -> Customer Engage
monitor_body = StrictPartialOrder(nodes=[data_monitor, yield_analyze, supply_chain, customer_engage])
monitor_body.order.add_edge(data_monitor, yield_analyze)
monitor_body.order.add_edge(yield_analyze, supply_chain)
monitor_body.order.add_edge(supply_chain, customer_engage)

monitor_loop = OperatorPOWL(operator=Operator.LOOP, children=[monitor_body, data_monitor])
...
root.order.add_edge(marketing_sync, monitor_loop)