...
# Define the loop for continuous feedback and performance tracking
loop_body = StrictPartialOrder(nodes=[decision_automate, cost_analyze, network_adapt])
loop_body.order.add_edge(decision_automate, cost_analyze)
loop_body.order.add_edge(cost_analyze, network_adapt)

# LOOP(children=[A, B]): execute A, then optionally execute B and repeat
loop = OperatorPOWL(operator=Operator.LOOP, children=[performance_track, loop_body])
...
root.order.add_edge(loop, performance_track)