...
# Define the dispute-resolution loop: choose to exit or perform dispute review then back to exit
dispute_loop = OperatorPOWL(
    operator=Operator.LOOP,
    children=[dispute_review, dispute_review]
)
...
root.order.add_edge(dispatch_alert, realtime_track)
root.order.add_edge(realtime_track, incentive_issue)
...
root.order.add_edge(hub_sync, demand_forecast)  # loop back to demand forecast after hub sync