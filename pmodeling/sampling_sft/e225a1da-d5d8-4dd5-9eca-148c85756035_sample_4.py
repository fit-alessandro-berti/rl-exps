...
# Loop for continuous data analysis and yield optimization
loop_body = StrictPartialOrder(nodes=[analyze_data, optimize_yield])
loop_body.order.add_edge(analyze_data, optimize_yield)

# Loop: run Trials, then either exit or run LoopBody then Trials again
loop = OperatorPOWL(operator=Operator.LOOP, children=[run_trials, loop_body])

# Build the overall partial order
root = StrictPartialOrder(nodes=[
    site_survey, design_layout, select_crops, install_modules,
    setup_sensors, calibrate_climate, configure_lighting,
    integrate_iot, train_staff, loop,
    check_compliance, plan_marketing, launch_facility
])
...
root.order.add_edge(train_staff, loop)
root.order.add_edge(loop, check_compliance)
