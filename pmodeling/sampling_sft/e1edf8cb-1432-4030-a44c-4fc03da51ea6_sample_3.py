...
adjust_loop = OperatorPOWL(
    ...
    children=[env_control, water_cycle, nutrient_mix, lighting_adjust]
)

# Build the partial order
root = StrictPartialOrder(nodes=[
    site_survey, structural_check, adjust_loop,
    iot_install, sensor_calibrate,
    crop_select, staff_train,
    waste_manage, energy_audit,
    harvest_plan, delivery_setup, market_align
])

# Add control-flow edges
root.order.add_edge(site_survey, structural_check)
root.order.add_edge(structural_check, adjust_loop)
root.order.add_edge(adjust_loop, iot_install)
root.order.add_edge(iot_install, sensor_calibrate)
root.order.add_edge(sensor_calibrate, crop_select)
