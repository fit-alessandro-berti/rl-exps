...
# Loop for repeated aging and humidity checks
loop_aging = OperatorPOWL(operator=Operator.LOOP, children=[aging_control, humidity_check])

# Build the partial order
root = StrictPartialOrder(nodes=[
    milk_sourcing, quality_testing, starter_prep,
    curd_cutting, molding_cheese, salting_process,
    loop_aging, packaging_design, label_printing,
    inventory_audit, cold_storage, order_processing,
    logistics_planning, retail_delivery,
    consumer_feedback, batch_adjustment,
    event_coordination
])
...
root.order.add_edge(event_coordination, milk_sourcing)  # loop back to start
