...
# Build the inner loop for iterative testing and training
loop_body = StrictPartialOrder(nodes=[flight_test, ai_training])
loop_body.order.add_edge(flight_test, ai_training)

# Loop operator: execute the body, then either exit or repeat the body
drone_loop = OperatorPOWL(operator=Operator.LOOP, children=[loop_body, SilentTransition()])
...
root.order.add_edge(frame_assembly,  propulsion_setup)
root.order.add_edge(sensor_install,  calibration)
root.order.add_edge(propulsion_setup, calibration)
root.order.add_edge(calibration,    software_load)
root.order.add_edge(software_load,  drone_loop)
root.order.add_edge(drone_loop,     qa_review)
