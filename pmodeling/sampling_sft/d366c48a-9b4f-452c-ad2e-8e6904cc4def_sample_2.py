...
# Loop: after each data backup, either exit or do another cycle
data_loop = OperatorPOWL(
    operator=Operator.LOOP,
    children=[data_backup, data_backup]
)

# Build the partial order
root = StrictPartialOrder(nodes=[
    site_survey, climate_scan, module_setup,
    crop_choice, nutrient_feed, pest_control,
    energy_audit, waste_cycle, growth_track,
    demand_plan, community_link, regulation_check,
    supply_sync, system_upgrade, data_loop
])
...
root.order.add_edge(system_upgrade, data_loop)
root.order.add_edge(data_loop, data_loop)  # self-loop to ensure continuous cycle
