...
# Define the loop for continuous improvement: do Market Testing, then either exit or do Process Refinement and repeat
loop_body = StrictPartialOrder(nodes=[market_testing, process_refinement])
loop_body.order.add_edge(market_testing, process_refinement)
improvement_loop = OperatorPOWL(operator=Operator.LOOP, children=[StrictPartialOrder(nodes=[market_testing]), loop_body])

# Build the overall partial order
root = StrictPartialOrder(nodes=[
    site_analysis, permit_securing, unit_designing, led_sourcing, hydroponic_setup, staff_hiring,
    pilot_cultivation, data_integration, waste_recycling, local_distribution, subscription_setup,
    iot_deployment, sustainability_audit, improvement_loop
])
...
root.order.add_edge(sustainability_audit, improvement_loop)

# The improvement loop can start at any time after the initial setup
for node in root.nodes:
    if node != improvement_loop:
        root.order.add_edge(node, improvement_loop)