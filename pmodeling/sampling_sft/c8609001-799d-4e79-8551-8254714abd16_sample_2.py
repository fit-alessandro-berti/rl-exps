...
# Silent transition for loop body (negotiation -> approval)
skip = SilentTransition()

# Body of the loop: negotiation -> approval
body = StrictPartialOrder(nodes=[price_negotiation, final_approval])
body.order.add_edge(price_negotiation, final_approval)

# Loop: Stakeholder Meet then either exit or do body then meet again
loop = OperatorPOWL(operator=Operator.LOOP, children=[stakeholder_meet, body])
...
root.order.add_edge(loop, legal_review)
root.order.add_edge(legal_review, insurance_quote)
root.order.add_edge(insurance_quote, contract_draft)
