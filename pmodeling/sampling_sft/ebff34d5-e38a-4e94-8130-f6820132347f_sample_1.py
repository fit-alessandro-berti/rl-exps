...
# Loop for multiple flavor tastings
# A: Flavor Tasting
# B: (skip) - in case no more tastings are needed
tasting_loop = OperatorPOWL(operator=Operator.LOOP, children=[flavor_tasting, Transition(label='skip')])
...
root = StrictPartialOrder(nodes=[
    ...
    fermentation, aging_control, tasting_loop,
    packaging_artisanal, label_printing, order_processing,
...
root.order.add_edge(aging_control, tasting_loop)

# Taste loop continues until exit
root.order.add_edge(tasting_loop, packaging_artisanal)

# Packaging continues after tasting loop
