...
# Define the maintenance loop body: Maintenance Plan -> Harvest Schedule -> Waste Manage
maintenance_loop_body = StrictPartialOrder(nodes=[maintenance_plan, harvest_schedule, waste_manage])
maintenance_loop_body.order.add_edge(maintenance_plan, harvest_schedule)
maintenance_loop_body.order.add_edge(harvest_schedule, waste_manage)

# LOOP: do Plant Crop, then either exit or do maintenance_loop_body and repeat
maintenance_loop = OperatorPOWL(operator=Operator.LOOP, children=[plant_crop, maintenance_loop_body])

# Build the overall partial order
root = StrictPartialOrder(nodes=[
    struct_check, permit_apply, design_layout,
    soil_prep, bed_install,
    irrigation_setup, sensor_mount, solar_connect,
    seed_order, nutrient_mix,
    community_meet, staff_train,
    maintenance_loop
])

# Add the sequential dependencies
root.order.add_edge(struct_check, permit_apply)
