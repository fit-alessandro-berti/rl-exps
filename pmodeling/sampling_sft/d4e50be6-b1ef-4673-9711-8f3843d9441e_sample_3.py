...
# Define the loop body: Monitoring, Compliance, Innovation Scan
monitoring = StrictPartialOrder(nodes=[revenue_monitoring, compliance_check])
monitoring.order.add_edge(revenue_monitoring, compliance_check)
innovation_loop = OperatorPOWL(operator=Operator.LOOP, children=[innovation_scan, monitoring])
...
# Sequence of activities before the licensing loop
...
# Licensing loop: after approval, track portfolio, then scan & monitor
root.order.add_edge(approval_process, portfolio_tracking)
root.order.add_edge(portfolio_tracking, innovation_loop)