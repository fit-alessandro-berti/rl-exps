...
# Build the loop body: Pest Control then Optimize Energy then Recycle Water
loop_body = StrictPartialOrder(nodes=[pest_control, optimize_energy, recycle_water])
loop_body.order.add_edge(pest_control, optimize_energy)
loop_body.order.add_edge(optimize_energy, recycle_water)

# Define the loop: repeat Pest Control -> Optimize Energy -> Recycle Water until exit
...
root = StrictPartialOrder(nodes=[
    ...
    install_hvac, setup_hydroponics, prepare_nutrients, select_seeds,
    automate_planting, deploy_sensors, loop,
    automate_harvest, package_crops, coordinate_delivery, analyze_data
])
...
root.order.add_edge(deploy_sensors, loop)
root.order.add_edge(loop, automate_harvest)
