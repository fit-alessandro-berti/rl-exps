...
# Define the loop body for testing and quality check
loop_body = StrictPartialOrder(nodes=[environmental_test, quality_check])
loop_body.order.add_edge(environmental_test, quality_check)

# LOOP: repeat testing and quality check until exit
loop = OperatorPOWL(operator=Operator.LOOP, children=[loop_body, None])
...
root.order.add_edge(design_draft, ai_integration)
root.order.add_edge(aerodynamics_test, material_sourcing)
...
root.order.add_edge(firmware_install, loop)
