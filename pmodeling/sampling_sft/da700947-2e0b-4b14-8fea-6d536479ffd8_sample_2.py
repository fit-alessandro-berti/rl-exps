...
# Define the inner loop body: Pest Scan -> Data Modeling -> Feedback Loop
body = StrictPartialOrder(nodes=[pest_scan, data_modeling, feedback_loop])
body.order.add_edge(pest_scan, data_modeling)
body.order.add_edge(data_modeling, feedback_loop)

# Define the overall loop: Site Survey -> ... -> Compliance Check -> Community Meet -> Yield Review -> Feedback Loop
# The loop starts with Site Survey, then continues with all other activities
loop = OperatorPOWL(operator=Operator.LOOP, children=[site_survey, structural_audit, climate_design, lighting_setup, irrigation_plan, nutrient_mix, sensor_install, ai_calibration, body])

# Assemble the complete partial order
root = StrictPartialOrder(nodes=[
    site_survey, structural_audit, climate_design, lighting_setup, irrigation_plan, nutrient_mix,
    sensor_install, ai_calibration, pest_scan, energy_audit, renewable_sync, data_modeling,
    staff_briefing, compliance_check, community_meet, yield_review, feedback_loop, body
])
