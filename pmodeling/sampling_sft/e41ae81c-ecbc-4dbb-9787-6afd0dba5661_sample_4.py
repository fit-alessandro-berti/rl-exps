...
# Build the growth cycle partial order (concurrent monitoring & pest control)
growth_cycle = StrictPartialOrder(nodes=[growth_monitor, pest_control])
# No edges => they run concurrently

# Build the optimization loop: analyze data, then optionally repeat
opt_loop = OperatorPOWL(operator=Operator.LOOP, children=[data_analysis, growth_cycle])
...
root = StrictPartialOrder(nodes=[
    ...
    staff_training, crop_planting, harvest_schedule, quality_check,
    market_dispatch, waste_recycle, opt_loop
])
...
root.order.add_edge(waste_recycle, opt_loop)

# The optimization loop can be taken at any time after data analysis
root.order.add_edge(data_analysis, opt_loop)