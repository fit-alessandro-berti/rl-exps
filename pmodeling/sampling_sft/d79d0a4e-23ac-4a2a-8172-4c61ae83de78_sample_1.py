...
root = StrictPartialOrder(nodes=[
    spec_review, component_pick, frame_build, motor_mount, sensor_fit,
    wiring_setup, software_load, calibration_test, stress_check,
    firmware_flash, feedback_loop, package_prep, doc_compile,
    ship_arrange, remote_setup
])
...
# Final loop: remote_setup can be repeated based on client feedback
remote_loop = OperatorPOWL(operator=Operator.LOOP, children=[remote_setup, remote_setup])

# Connect the final loop to the end
root.order.add_edge(firmware_flash, remote_loop)
