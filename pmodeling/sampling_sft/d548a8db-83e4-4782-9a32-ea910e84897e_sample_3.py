...
water_recirc     = Transition(label='Water Recirculate')
light_calibration= Transition(label='Light Calibration')
rob_harvest      = Transition(label='Robotic Harvest')
...
climate_loop = OperatorPOWL(
    ...
    children=[climate_adjust, pest_control, water_recirc, light_calibration]
)
...
root.order.add_edge(nutrient_setup,    growth_monitor)
root.order.add_edge(growth_monitor,    climate_loop)
root.order.add_edge(climate_loop,      rob_harvest)
root.order.add_edge(rob_harvest,       quality_inspect)
