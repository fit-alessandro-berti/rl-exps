...
# Loop for iterative stress testing
stress_loop = OperatorPOWL(
    operator=Operator.LOOP,
    children=[stress_test, software_patch]
)

# Build the partial‐order workflow
root = StrictPartialOrder(nodes=[
    comp_check, spec_review, parts_sorting,
    mech_fit, firmware_load, calibration_run,
    stress_loop, validation_pass,
    data_link, onsite_deploy, live_monitor,
    update_push, recovery_plan, maintenance_log
])

# Define the control‐flow dependencies
root.order.add_edge(comp_check, spec_review)
root.order.add_edge(spec_review, parts_sorting)
root.order.add_edge(parts_sorting, mech_fit)
...
root.order.add_edge(stress_loop, validation_pass)
...
# Final edge for completion
root.order.add_edge(maintenance_log, pm4py.objects.powl.obj.FINISH)