...
loop_body = StrictPartialOrder(nodes=[pm, ls, fl])
loop_body.order.add_edge(pm, ls)
loop_body.order.add_edge(ls, fl)

loop = OperatorPOWL(operator=Operator.LOOP, children=[loop_body, loop_body])

# Build the root partial order
root = StrictPartialOrder(nodes=[
    ci, sa, ps, tf, ws, wd, loop, sc, psc, je, rc, wa, pa
])
