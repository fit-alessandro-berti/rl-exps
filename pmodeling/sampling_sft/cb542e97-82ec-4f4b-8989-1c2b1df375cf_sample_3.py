...
# Loop for multiple expert reviews
skip = SilentTransition()
expert_loop = OperatorPOWL(operator=Operator.LOOP, children=[expert_review, skip])

# Build the partial order
root = StrictPartialOrder(nodes=[
    artifact_intake, condition_check, material_test, archival_search, style_compare,
    expert_loop, restoration_check, provenance_trace, legal_verify,
    value_appraise, catalog_entry, marketing_plan, auction_setup, certify_final, sales_report
])
...
root.order.add_edge(style_compare, expert_loop)
root.order.add_edge(expert_loop, restoration_check)
root.order.add_edge(expert_loop, provenance_trace)
