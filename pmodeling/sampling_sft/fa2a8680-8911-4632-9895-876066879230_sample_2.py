...
# Define the loop for iterative performance tuning
# A: Performance Tune
# B: Maintenance Plan
loop = OperatorPOWL(operator=Operator.LOOP, children=[performance_tune, maintenance_plan])

# Build the partial order
root = StrictPartialOrder(nodes=[
    client_meet, requirement_gather, module_design, supplier_vetting, component_order,
    prototype_build, field_testing, test_analysis, software_setup, data_integration,
    pilot_train, compliance_check, fleet_deploy, remote_monitor, loop
])
...
root.order.add_edge(remote_monitor, loop)