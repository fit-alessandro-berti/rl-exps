...
# Define a silent transition for loop exit
skip = SilentTransition()

# Build the loop body: yield analysis -> resource audit -> impact review
loop_body = StrictPartialOrder(nodes=[yield_analysis, resource_audit, impact_review])
loop_body.order.add_edge(yield_analysis, resource_audit)
loop_body.order.add_edge(resource_audit, impact_review)

# Define the LOOP operator: do site survey, then either exit or do the loop body and repeat
loop = OperatorPOWL(operator=Operator.LOOP, children=[site_survey, skip])
...
root.order.add_edge(design_modules, sensor_calib)
root.order.add_edge(iot_setup, nutrient_mix)
root.order.add_edge(sensor_calib, nutrient_mix)
...
root.order.add_edge(loop, yield_analysis)
