...
# Define the loop body for missing or damaged gears: Fabricate Gears -> Reassemble Movement
loop_body = StrictPartialOrder(nodes=[fabricate_gears, reassemble_movement])
# No edges in the loop_body, so both transitions are concurrent

# Loop: after Inspect Components, either exit or do the loop_body and then Inspect Components again
loop = OperatorPOWL(operator=Operator.LOOP, children=[inspect_components, loop_body])
...
root.order.add_edge(inspect_components,  loop)
root.order.add_edge(loop,                dial_restoration)
root.order.add_edge(loop,                repaint_markers)
root.order.add_edge(dial_restoration,    reassemble_movement)
