...
root = StrictPartialOrder(nodes=[
    ...
    monitor_growth, adjust_systems,
    harvest_crops, package_produce,
    engage_community, host_workshops
])

# Define the sequence and concurrency relationships
...
root.order.add_edge(hydro_design, install_irrigation)
...
# Monitoring and adjustments can run in parallel with the schedule
root.order.add_edge(pest_control, monitor_growth)
root.order.add_edge(pest_control, adjust_systems)

# Concurrency between growth and adjustments
root.order.add_edge(monitor_growth, adjust_systems)

# All growth‐related activities can follow pest control
root.order.add_edge(pest_control, harvest_crops)
root.order.add_edge(pest_control, package_produce)

# Concurrency between harvest and packaging
root.order.add_edge(harvest_crops, package_produce)

# Community engagement and workshops can occur after harvest and packaging
root.order.add_edge(harvest_crops, engage_community)
...
root.order.add_edge(harvest_crops, host_workshops)
