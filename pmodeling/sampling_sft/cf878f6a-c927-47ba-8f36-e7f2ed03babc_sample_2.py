...
root = StrictPartialOrder(nodes=[
    site_assess, zoning_check, design_farm,
    procure_gear, install_systems, setup_sensors,
    select_crops, prepare_seeds, mix_nutrients,
    monitor_growth, adjust_climate,
    robotic_harvest, grade_quality, pack_produce,
...
# Parallel climate adjustment and growth monitoring
root.order.add_edge(monitor_growth,   adjust_climate)

# After growth and climate adjustments, parallel robotic harvest and quality grading
root.order.add_edge(adjust_climate,   robotic_harvest)
root.order.add_edge(adjust_climate,   grade_quality)

# After harvest and grading, parallel packaging and logistics management
root.order.add_edge(robotic_harvest,  pack_produce)
root.order.add_edge(grade_quality,    pack_produce)
root.order.add_edge(pack_produce,     manage_logistics)

# After packaging and logistics, parallel marketing and waste recycling
root.order.add_edge(manage_logistics, market_products)
root.order.add_edge(manage_logistics, recycle_waste)

# After all production‐related activities, audit systems
root.order.add_edge(market_products,  audit_systems)
root.order.add_edge(recycle_waste,    audit_systems)
root.order.add_edge(audit_systems,    None)  # optional final edge for completeness