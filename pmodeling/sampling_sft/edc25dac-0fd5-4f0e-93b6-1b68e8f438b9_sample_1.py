...
# Build the growth monitoring loop: monitor then optionally recycle then repeat
body = StrictPartialOrder(nodes=[growth_monitor, waste_recycle])
body.order.add_edge(growth_monitor, waste_recycle)
loop = OperatorPOWL(operator=Operator.LOOP, children=[body, waste_recycle])

# Assemble the overall partial order
root = StrictPartialOrder(nodes=[
    load_assess, permit_review, site_survey, design_layout,
    soil_mix, install_beds, irrigation_set, climate_test,
    sensor_deploy, energy_setup, crop_select, plant_seeding,
    community_meet, compliance_check, loop, harvest_plan
])

# Sequence all the initial setup activities
...
# Parallelize the community engagement and compliance checks
root.order.add_edge(community_meet, compliance_check)

# Finalize with the growth monitoring loop and harvest planning
