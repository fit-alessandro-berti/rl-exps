...
# Loop for repairs: analyze damage -> select materials -> perform repairs
repair_loop = OperatorPOWL(
    operator=Operator.LOOP,
    children=[analyze_damage, select_materials, perform_repairs]
)

# Build the overall partial order
root = StrictPartialOrder(nodes=[
    inspect_item, verify_provenance, document_condition,
    disassemble_parts, clean_components,
    repair_loop,
    match_finishes, apply_treatments,
    reassemble_item, quality_check,
    photograph_results, update_archive,
...
root.order.add_edge(clean_components, repair_loop)
root.order.add_edge(repair_loop, match_finishes)
root.order.add_edge(repair_loop, apply_treatments)
root.order.add_edge(match_finishes, reassemble_item)
