...
# Define the loop for expert survey and risk analysis
expert_loop = OperatorPOWL(operator=Operator.LOOP, children=[expert_survey, skip])
risk_loop = OperatorPOWL(operator=Operator.LOOP, children=[risk_analysis, skip])
...
root.order.add_edge(condition_report, expert_loop)
root.order.add_edge(condition_report, legal_review)
root.order.add_edge(expert_loop, risk_loop)
root.order.add_edge(risk_loop, legal_review)
