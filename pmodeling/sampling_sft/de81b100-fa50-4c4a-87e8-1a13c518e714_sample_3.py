...
# Loop for crop cycle: do Crop Cycle, then either exit or do Data Monitor and Crop Cycle again
loop_cycle = OperatorPOWL(
    operator=Operator.LOOP,
    children=[crop_cycle, data_monitor]
)
...
root = StrictPartialOrder(nodes=[
    ...
    waste_manage, supply_chain, loop_cycle, harvest_plan, distribution
])
...
root.order.add_edge(supply_chain, loop_cycle)
root.order.add_edge(loop_cycle, harvest_plan)
root.order.add_edge(harvest_plan, distribution)

# Final edges from the loop to the next phase
root.order.add_edge(loop_cycle, distribution)