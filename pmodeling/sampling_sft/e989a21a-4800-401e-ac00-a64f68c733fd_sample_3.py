...
# Build the loop for iterative provenance tracing
loop = OperatorPOWL(
    operator=Operator.LOOP,
    children=[provenance_trace, data_crosscheck]
)

# Build the choice for either forgery detection or expert consultation
xor = OperatorPOWL(
    operator=Operator.XOR,
    children=[forgery_detect, expert_consult]
)

# Build the main partial order
root = StrictPartialOrder(nodes=[
    visual_inspect, document_gather,
    material_test, pigment_analyze, style_compare,
    loop, xor,
    infrared_scan, xray_fluoresce,
    report_draft, stakeholder_review, final_approval, archive_store
])
...
root.order.add_edge(report_draft,    infrared_scan)
root.order.add_edge(report_draft,    xray_fluoresce)