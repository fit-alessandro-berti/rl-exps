...
# Build the monitoring loop: do Growth Monitor, then optionally Pest Control or Water Cycle, then repeat
monitor_loop = OperatorPOWL(
    operator=Operator.LOOP,
    children=[growth_monitor, OperatorPOWL(operator=Operator.XOR, children=[pest_control, water_cycle])]
)

# Build the root partial order
root = StrictPartialOrder(nodes=[
    site_survey, modular_design, system_build, env_control,
    seed_selection, nutrient_mix, planting_setup,
    monitor_loop, data_capture, yield_forecast,
    waste_reuse, stakeholder_meet, compliance_check, supply_sync
])
...
root.order.add_edge(monitor_loop, data_capture)
