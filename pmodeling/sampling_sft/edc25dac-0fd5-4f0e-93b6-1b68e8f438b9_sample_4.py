...
# Define the monitoring loop: do Growth Monitor, then optionally do Waste Recycle and then Growth Monitor again
monitor_loop = OperatorPOWL(
    operator=Operator.LOOP,
    children=[growth_monitor, waste_recycle]
)

# Build the overall partial order
root = StrictPartialOrder(nodes=[
    load_assess, permit_review, site_survey, design_layout, soil_mix, install_beds, irrigation_set,
    climate_test, sensor_deploy, energy_setup, crop_select, plant_seeding, community_meet,
    compliance_check, monitor_loop
])
...
root.order.add_edge(compliance_check, monitor_loop)