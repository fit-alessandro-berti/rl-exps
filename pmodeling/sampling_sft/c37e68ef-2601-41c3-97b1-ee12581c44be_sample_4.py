...
# Loop for seasonal inventory adjustments
inventory_loop = OperatorPOWL(
    operator=Operator.LOOP,
    children=[inventory_audit, inventory_audit]
)

# Loop for collaborative marketing campaigns
marketing_loop = OperatorPOWL(
    operator=Operator.LOOP,
    children=[marketing_sync, marketing_sync]
)
...
root.order.add_edge(fermentation, aging_setup)
...
root.order.add_edge(label_approval, inventory_loop)
root.order.add_edge(inventory_loop, order_scheduling)
...
root.order.add_edge(compliance_check, marketing_loop)

# Final loop for marketing sync (optional: can be omitted for a non-loop version)
root.order.add_edge(marketing_loop, marketing_sync)