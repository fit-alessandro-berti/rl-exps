...
# Build the loop for iterative testing and feedback
test_loop = OperatorPOWL(
    operator=Operator.LOOP,
    children=[flight_simulate, feedback_collect]
)

# Assemble the overall partial order
root = StrictPartialOrder(nodes=[
    client_consult, spec_analysis, module_select, component_order, parts_inspect,
    frame_assemble, sensor_install, motor_attach, wiring_connect, software_upload,
    calibration_test, test_loop, quality_review, user_train, remote_setup,
    support_schedule
])
...
root.order.add_edge(frame_assemble, motor_attach)
root.order.add_edge(frame_assemble, wiring_connect)
root.order.add_edge(sensor_install, software_upload)
root.order.add_edge(motor_attach, software_upload)
root.order.add_edge(wiring_connect, software_upload)
root.order.add_edge(software_upload, calibration_test)
root.order.add_edge(calibration_test, test_loop)
root.order.add_edge(test_loop, quality_review)
root.order.add_edge(quality_review, user_train)
root.order.add_edge(user_train, remote_setup)
root.order.add_edge(remote_setup, support_schedule)

# Final edges for completeness
root.order.add_edge(spec_analysis, module_select)
root.order.add_edge(component_order, parts_inspect)
root.order.add_edge(parts_inspect, frame_assemble)
root.order.add_edge(frame_assemble, sensor_install)
