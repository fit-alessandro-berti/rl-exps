...
# Build the loop body for the monitoring‐then‐optimize cycle
monitor_body = StrictPartialOrder(nodes=[monitoring_setup, yield_optimize])
monitor_body.order.add_edge(monitoring_setup, yield_optimize)

# Define the monitoring‐then‐optimize loop: do one full cycle, then optionally repeat
monitor_loop = OperatorPOWL(operator=Operator.LOOP, children=[monitor_body, yield_optimize])
...
root = StrictPartialOrder(nodes=[
    ...
    logistics_plan, supply_coordinate, distribution_map,
    community_engage,
    monitor_loop
])
...
root.order.add_edge(community_engage, monitor_loop)