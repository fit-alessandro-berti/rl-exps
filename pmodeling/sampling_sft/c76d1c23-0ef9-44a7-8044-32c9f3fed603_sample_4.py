...
# Define the monitoring/management loop body (Monitor Growth -> Manage Pests)
loop_body = StrictPartialOrder(nodes=[monitor_growth, manage_pests])
loop_body.order.add_edge(monitor_growth, manage_pests)

# Define the loop: repeat Monitor Growth -> Manage Pests until exit
loop = OperatorPOWL(operator=Operator.LOOP, children=[loop_body, Transition(label='exit')])

# Build the top-level partial order
root = StrictPartialOrder(nodes=[
    site_survey, zoning_check, design_layout, system_order, structure_build,
    install_hydro, calibrate_sensors, select_crops, plant_seeding, loop,
...
root.order.add_edge(install_hydro, calibrate_sensors)
