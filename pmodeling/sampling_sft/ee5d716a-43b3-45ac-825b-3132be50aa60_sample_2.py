...
# Loop for continuous monitoring and bid adjustment
loop_body = StrictPartialOrder(nodes=[monitor_bids, adjust_pricing])
loop_body.order.add_edge(monitor_bids, adjust_pricing)

# Loop for dispute resolution
skip = SilentTransition()
loop_disputes = OperatorPOWL(operator=Operator.LOOP, children=[resolve_disputes, skip])
...
root.order.add_edge(activate_auction, loop_body)
root.order.add_edge(loop_body, enable_fractional)
root.order.add_edge(enable_fractional, validate_bidders)
root.order.add_edge(validate_bidders, loop_disputes)
root.order.add_edge(loop_disputes, distribute_royalties)
