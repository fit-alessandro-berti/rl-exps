...
# Loop for seasonal monitoring
monitoring_loop = OperatorPOWL(
    operator=Operator.LOOP,
    children=[diet_monitoring, culture_selection]
)

# Silent transition for optional event coordination
skip_event = SilentTransition()

# Choice for optional event coordination (event or skip)
event_choice = OperatorPOWL(
    operator=Operator.XOR,
    children=[event_coordination, skip_event]
)
...
root.order.add_edge(retail_delivery,   event_choice)
root.order.add_edge(event_choice,      feedback_review)

# Add the loop dependency for monitoring
root.order.add_edge(milk_sourcing,     monitoring_loop)
root.order.add_edge(monitoring_loop,   milk_pasteurize)
