...
# Loop for expert consultations and archival reviews
# Body A: Expert Consult, then Archival Review
loop_body = StrictPartialOrder(nodes=[expert_consult, archival_review])
loop_body.order.add_edge(expert_consult, archival_review)

# Loop: do the body, then choose to exit or repeat
expert_loop = OperatorPOWL(operator=Operator.LOOP, children=[loop_body, SilentTransition()])
...
root.order.add_edge(surface_finish,     expert_loop)
root.order.add_edge(expert_loop,        ethics_audit)
