...
# Loop for cross-checking and ledger verification
cross_loop = OperatorPOWL(operator=Operator.LOOP, children=[cross_check, ledger_verification])

# Build the partial order
root = StrictPartialOrder(nodes=[
    intake_check, condition_log, provenance_review,
    material_test, spectrometry_scan, stylistic_analysis,
    expert_consult, archive_search, cross_loop,
    conservation_prep, documentation, report_creation, database_update
])
...
root.order.add_edge(spectrometry_scan, cross_loop)
root.order.add_edge(stylistic_analysis, cross_loop)
root.order.add_edge(cross_loop, expert_consult)
root.order.add_edge(expert_consult, archive_search)
root.order.add_edge(archive_search, ledger_verification)
root.order.add_edge(ledger_verification, cross_loop)
root.order.add_edge(cross_loop, conservation_prep)
