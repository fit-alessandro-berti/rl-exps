...
loop_body = StrictPartialOrder(nodes=[provenance_search, ownership_validate, radiocarbon_test,
                                      spectroscopy_scan, material_analysis,
                                      style_assessment, context_review,
                                      expert_panel, report_draft, quality_review])
loop_body.order.add_edge(provenance_search, ownership_validate)
loop_body.order.add_edge(ownership_validate, radiocarbon_test)
loop_body.order.add_edge(radiocarbon_test, spectroscopy_scan)
loop_body.order.add_edge(spectroscopy_scan, material_analysis)
loop_body.order.add_edge(material_analysis, style_assessment)
loop_body.order.add_edge(style_assessment, context_review)
loop_body.order.add_edge(context_review, expert_panel)
loop_body.order.add_edge(expert_panel, report_draft)
loop_body.order.add_edge(report_draft, quality_review)
...
# Loop at the end
root.order.add_edge(reevaluation_trigger, loop)