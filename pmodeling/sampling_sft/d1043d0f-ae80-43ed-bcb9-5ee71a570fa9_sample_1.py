...
# Loop for continuous improvement: Data Review then optional Yield Forecast and Market Plan
improve_loop = OperatorPOWL(
    operator=Operator.LOOP,
    children=[data_review, StrictPartialOrder(nodes=[yield_forecast, market_plan], order=set())]
)

# Build the partial order
root = StrictPartialOrder(
    nodes=[
        site_survey, env_analysis, module_design, seed_selection,
        nutrient_mix, climate_setup, led_install, sensor_deploy,
        pest_control, waste_recycle, hydro_test, staff_train,
        improve_loop
    ]
)
...
root.order.add_edge(staff_train, improve_loop)