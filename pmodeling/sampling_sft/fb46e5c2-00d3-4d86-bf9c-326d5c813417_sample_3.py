...
# Create a strict partial order with defined dependencies
root = StrictPartialOrder(nodes=[
    site_analysis, impact_review,
    modular_design, system_integration,
    climate_setup, nutrient_mix, light_config,
    staff_training,
    pest_monitor, drone_deploy, health_scan, data_logging,
    supply_sync, maintenance_plan, waste_manage
])
...
root.order.add_edge(staff_training, drone_deploy)
root.order.add_edge(staff_training, health_scan)
root.order.add_edge(pest_monitor, data_logging)
root.order.add_edge(drone_deploy, data_logging)
