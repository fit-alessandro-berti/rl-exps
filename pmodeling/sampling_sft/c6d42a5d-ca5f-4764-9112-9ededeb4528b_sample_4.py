...
# Define the loop for continuous monitoring
skip = SilentTransition()
loop = OperatorPOWL(operator=Operator.LOOP, children=[health_monitor, skip])
...
root = StrictPartialOrder(nodes=[
    ...
    staff_train, market_outreach, system_setup, supplier_contact, loop
])
...
root.order.add_edge(supplier_contact, loop)

# Ensure the monitoring loop is concurrent with other activities
root.order.add_edge(permit_apply, loop)
root.order.add_edge(material_order, loop)
root.order.add_edge(bed_install, loop)
root.order.add_edge(pest_control, loop)
root.order.add_edge(solar_setup, loop)
