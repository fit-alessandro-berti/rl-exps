...
root = StrictPartialOrder(nodes=[
    ...
    crop_cycle, data_monitor, harvest_plan, distribution
])
...
# Staff and training must follow testing
root.order.add_edge(system_test, staff_hire)
root.order.add_edge(system_test, training_lead)

# After staff and training, proceed with waste and supply chain
root.order.add_edge(staff_hire, waste_manage)
root.order.add_edge(training_lead, supply_chain)

# Crop cycle starts after testing and staff/training
root.order.add_edge(system_test, crop_cycle)
root.order.add_edge(staff_hire, crop_cycle)
root.order.add_edge(training_lead, crop_cycle)

# Monitoring and data analysis follow crop cycle
root.order.add_edge(crop_cycle, data_monitor)

# Harvest plan and distribution depend on data monitoring
root.order.add_edge(data_monitor, harvest_plan)
root.order.add_edge(data_monitor, distribution)