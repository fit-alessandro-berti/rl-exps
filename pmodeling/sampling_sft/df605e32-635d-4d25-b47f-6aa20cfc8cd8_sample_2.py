...
# Define the loop body (negotiation & transport)
loop_body = StrictPartialOrder(nodes=[negotiation_setup, secure_transport])

# Define the loop: perform Artifact Scan -> Ownership Verify -> Risk Assess -> Legal Review -> Stakeholder Notify -> Recovery Plan -> loop_body then repeat
loop = OperatorPOWL(operator=Operator.LOOP, children=[recovery_plan, loop_body])
