...
loop_body = StrictPartialOrder(nodes=[flight_sim, pattern_adj])
loop_body.order.add_edge(flight_sim, pattern_adj)
flight_loop = OperatorPOWL(operator=Operator.LOOP, children=[loop_body, loop_body])

# Build the top-level partial order
root = StrictPartialOrder(nodes=[
    comp_sourcing, frame_assembly, motor_install, sensor_mount,
    wiring_setup, firmware_upload, ai_module, calibration,
    stress_test, flight_loop, quality_inspect, compliance_chk,
    packaging_final, delivery_setup
])
...
root.order.add_edge(motor_install, wiring_setup)
...
root.order.add_edge(stress_test, flight_loop)
root.order.add_edge(flight_loop, quality_inspect)
root.order.add_edge(quality_inspect, compliance_chk)
root.order.add_edge(compliance_chk, packaging_final)
