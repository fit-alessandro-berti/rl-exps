...
hydroponics_cfg   = Transition(label='Hydroponics Config')
...
# Define the loop for continuous data analytics and yield optimization
loop = OperatorPOWL(operator=Operator.LOOP, children=[data_analytics, yield_optimize])

# Build the partial order
root = StrictPartialOrder(nodes=[
    site_analysis, structural_check, rack_install,
    system_setup, hydroponics_cfg, aeroponics_tune,
...
root.order.add_edge(system_setup, hydroponics_cfg)
root.order.add_edge(system_setup, aeroponics_tune)
root.order.add_edge(system_setup, lighting_setup)
root.order.add_edge(hydroponics_cfg, enviro_control)
...
root.order.add_edge(test_grow, loop)