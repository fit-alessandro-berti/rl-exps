...
# Loop for iterative testing and refinement
# A = final_testing (main activity)
# B = (firmware_flash -> sensor_install) (refinement)
A = final_testing
B = StrictPartialOrder(nodes=[firmware_flash, sensor_install])
B.order.add_edge(firmware_flash, sensor_install)

# LOOP operator: do A, then optionally B then A again
test_loop = OperatorPOWL(operator=Operator.LOOP, children=[A, B])

# Build the overall partial order
root = StrictPartialOrder(nodes=[
    client_consult, spec_gathering, supplier_sourcing,
    design_review, simulation_test, test_loop,
    quality_check, brand_packaging, shipping_prep,
...
root.order.add_edge(simulation_test, test_loop)
root.order.add_edge(test_loop, quality_check)
