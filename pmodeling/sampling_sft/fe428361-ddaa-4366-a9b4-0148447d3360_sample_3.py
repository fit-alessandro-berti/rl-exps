...
# Loop for handling potential issues during flight
issue_handling = OperatorPOWL(
    operator=Operator.LOOP,
    children=[issue_resolve, package_return]
)

# Build the partial‐order workflow
root = StrictPartialOrder(nodes=[
    client_consult, payload_assess, drone_configure,
    regulation_check, flight_simulate, route_optimize,
    package_secure, pre_flight_inspect, weather_monitor,
    launch_drone, flight_track, delivery_confirm,
    data_analyze, feedback_collect, warranty_register,
    issue_handling
])
...
root.order.add_edge(regulation_check, route_optimize)

root.order.add_edge(flight_simulate, package_secure)
root.order.add_edge(route_optimize, package_secure)

root.order.add_edge(package_secure, pre_flight_inspect)
root.order.add_edge(package_secure, weather_monitor)
...
root.order.add_edge(launch_drone, issue_handling)

root.order.add_edge(flight_track, delivery_confirm)
root.order.add_edge(flight_track, data_analyze)
root.order.add_edge(flight_track, feedback_collect)

root.order.add_edge(data_analyze, warranty_register)
