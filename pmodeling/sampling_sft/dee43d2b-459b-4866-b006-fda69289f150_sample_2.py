...
# Loop for continuous data analysis and system maintenance
loop = OperatorPOWL(operator=Operator.LOOP, children=[data_analysis, system_maintenance])

# Build the top-level partial order
root = StrictPartialOrder(nodes=[
    seed_selection, nutrient_mix, planting_setup, climate_control, water_cycling,
    growth_monitoring, pest_detection, light_adjustment, loop,
    harvest_planning, crop_harvest, yield_sorting, packaging_prep,
    distribution_plan, regulation_check, waste_recycling
])
...
root.order.add_edge(light_adjustment, loop)
root.order.add_edge(loop,             harvest_planning)
