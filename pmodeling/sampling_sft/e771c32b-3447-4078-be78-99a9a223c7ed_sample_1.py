...
# Loop for continuous optimization: do Optimize Environment, then optionally repeat
...
root.order.add_edge(ai_setup, nutrient_mix)
...
# The loop itself is not directly connected to any other node,
# so it runs after all initial setup activities
root.order.add_edge(loop_optimize, loop_optimize)