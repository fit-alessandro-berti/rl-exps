...
# Build the loop for the iterative ideation and prototyping cycle
body = StrictPartialOrder(nodes=[proto_build, user_feedback, risk_review, ip_audit])
body.order.add_edge(proto_build, user_feedback)
body.order.add_edge(user_feedback, risk_review)
body.order.add_edge(risk_review, ip_audit)

loop = OperatorPOWL(operator=Operator.LOOP, children=[idea_sprint, body])
