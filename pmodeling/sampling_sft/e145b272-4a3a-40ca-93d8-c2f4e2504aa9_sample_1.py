...
root = StrictPartialOrder(nodes=[
    site_survey, permit_filing, modular_build, sensor_install,
    climate_setup, nutrient_mix, waste_setup, iot_deploy,
    ai_scheduling, energy_audit, compliance_check,
    crop_planting, yield_monitor, data_analysis, system_upgrade
])
...
# After compliance, the farm enters the operational loop
root.order.add_edge(compliance_check, crop_planting)

# The operational loop: plant crops, monitor yield, then optionally upgrade
operational_loop = OperatorPOWL(operator=Operator.LOOP, children=[crop_planting, yield_monitor])
root.order.add_edge(compliance_check, operational_loop)

# After the operational loop, analyze data and optionally upgrade the system
root.order.add_edge(operational_loop, data_analysis)
