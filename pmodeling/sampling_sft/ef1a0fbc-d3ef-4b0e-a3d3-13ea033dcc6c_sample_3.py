...
# Define the quality testing loop: repeat until no defects found
loop = OperatorPOWL(operator=Operator.LOOP, children=[quality_testing, milk_sourcing])

# Build the overall partial order
root = StrictPartialOrder(nodes=[
    milk_sourcing, farm_selection, loop,
    milk_pasteurize, starter_culture, coagulation, curd_cutting, whey_draining, mold_inoculate,
    aging_control, flavor_tasting,
    packaging_design, label_approval,
    inventory_audit, order_fulfill, retail_shipping
])

# Sequential edges within the loop
root.order.add_edge(milk_sourcing, loop)

# Flow after loop: no defects found
...
# Final sequential edges after flavor tasting
