...
# A = aging set -> flavor check -> texture scan
A = StrictPartialOrder(nodes=[aging_set, flavor_check, texture_scan])
A.order.add_edge(aging_set, flavor_check)
A.order.add_edge(flavor_check, texture_scan)

# B = quality approve
B = quality_approve

# LOOP(A, B): perform A, then either exit or do B and repeat A
...
root.order.add_edge(aging_loop, custom_pack)
