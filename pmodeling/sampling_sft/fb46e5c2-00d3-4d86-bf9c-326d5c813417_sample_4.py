...
# Create the overall partial‐order workflow
...
root.order.add_edge(pest_monitor, health_scan)
root.order.add_edge(drone_deploy, health_scan)

root.order.add_edge(health_scan, data_logging)
root.order.add_edge(health_scan, supply_sync)
root.order.add_edge(health_scan, maintenance_plan)
root.order.add_edge(health_scan, waste_manage)

# The end-to-end sequence is now fully specified