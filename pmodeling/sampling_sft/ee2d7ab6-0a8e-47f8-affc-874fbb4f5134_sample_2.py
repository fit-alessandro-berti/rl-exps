...
# Loop for iterative testing and rework
testing_loop = OperatorPOWL(
    operator=Operator.LOOP,
    children=[final_testing, firmware_flash, sensor_install]
)
...
root = StrictPartialOrder(nodes=[
    ...
    quality_check, testing_loop,
    brand_packaging, shipping_prep, delivery_schedule,
    client_training, diagnostics_setup
])
...
root.order.add_edge(quality_check, testing_loop)
root.order.add_edge(testing_loop, testing_loop)  # rejoin the loop
root.order.add_edge(testing_loop, brand_packaging)
