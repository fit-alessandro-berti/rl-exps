...
# Inner loop: gather feedback -> update model -> forecast yield
inner_loop = OperatorPOWL(
    operator=Operator.LOOP,
    children=[feedback_gather, model_update, yield_forecast]
)

# Outer loop: repeat the above sequence until exit
loop = OperatorPOWL(
    operator=Operator.LOOP,
    children=[inner_loop, inner_loop]
)

# Build the partial order
root = StrictPartialOrder(nodes=[
    sensor_setup, data_capture, nutrient_mix, crop_rotate,
    waste_collect, compost_process, drone_dispatch,
    pest_control, pollination_run, volunteer_assign,
    feedback_gather, model_update, yield_forecast,
    water_adjust, report_generate, resource_audit,
    schedule_sync, loop
])
...
root.order.add_edge(schedule_sync, loop)
root.order.add_edge(loop, feedback_gather)