...
# Loop for repeated sensory audits
loop = OperatorPOWL(operator=Operator.LOOP, children=[saud, SilentTransition()])

# Build the partial‐order workflow
root = StrictPartialOrder(nodes=[
    ms, qt, mp, ci, co, cc, wd, pr, sa, ac,
    loop, pd, la, oc, lp, md, cf, rc
])
...
root.order.add_edge(loop, saud)
root.order.add_edge(saud, pd)
