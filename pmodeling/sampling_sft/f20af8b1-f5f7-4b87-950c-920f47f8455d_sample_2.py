...
root = StrictPartialOrder(nodes=[
    ...
    modular_build, hydroponic_setup,
    seed_select, nutrient_prep,
    climate_calibrate, sensor_install,
    ai_integration, crop_monitor,
    growth_adjust, harvest_sort,
    packaging, distribution_plan,
    sustain_audit, energy_optimize
])

# Define the control‐flow dependencies
root.order.add_edge(site_survey, env_analysis)
root.order.add_edge(env_analysis, modular_build)
root.order.add_edge(env_analysis, hydroponic_setup)
...
root.order.add_edge(seed_select, climate_calibrate)
...
root.order.add_edge(growth_adjust, packaging)
root.order.add_edge(growth_adjust, distribution_plan)

root.order.add_edge(harvest_sort, packaging)
root.order.add_edge(packaging, distribution_plan)

root.order.add_edge(growth_adjust, sustain_audit)
