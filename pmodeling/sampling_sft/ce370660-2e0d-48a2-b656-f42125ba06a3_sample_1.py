...
# Define the maintenance loop: Plant Crop -> Maintenance Plan -> Harvest Schedule -> Waste Manage
maintenance_body = StrictPartialOrder(nodes=[maintenance_plan, harvest_schedule, waste_manage])
maintenance_body.order.add_edge(maintenance_plan, harvest_schedule)
maintenance_body.order.add_edge(harvest_schedule, waste_manage)

maintenance_loop = OperatorPOWL(operator=Operator.LOOP, children=[plant_crop, maintenance_body])
