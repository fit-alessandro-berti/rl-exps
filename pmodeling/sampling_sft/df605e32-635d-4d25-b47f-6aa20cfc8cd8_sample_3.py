...
# Loop for restoration and archival steps: do Restoration Begin, then either exit or do Documentation Log then Restoration Begin again
restoration_loop = OperatorPOWL(operator=Operator.LOOP, children=[restoration_begin, documentation_log])
...
root.order.add_edge(condition_inspect, restoration_loop)
root.order.add_edge(restoration_loop, heritage_archive)
root.order.add_edge(heritage_archive, final_audit)
root.order.add_edge(final_audit, process_close)

# Final edge for completeness
root.order.add_edge(process_close, skip)