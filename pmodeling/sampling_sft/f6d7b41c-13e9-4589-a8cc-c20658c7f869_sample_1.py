...
# Define the condition check & appraisal loop:
# A = Condition Check -> Appraisal Log
# B = skip (no re‐appraisal if no change)
condition_loop = OperatorPOWL(operator=Operator.LOOP, children=[condition_check, appraisal_log])

# Build the overall partial order
root = StrictPartialOrder(nodes=[
    client_consult, needs_assess, art_selection, inventory_check,
    legal_draft, contract_sign, insurance_setup, transport_plan, secure_transit,
    art_install, condition_loop,
...
# Loop body after art_install
root.order.add_edge(condition_loop, lease_renew)
root.order.add_edge(condition_loop, payment_process)

# Loop exit after lease_renew or payment_process
root.order.add_edge(lease_renew, deinstall_art)
root.order.add_edge(payment_process, deinstall_art)

# Finally, either deinstall & return or purchase
