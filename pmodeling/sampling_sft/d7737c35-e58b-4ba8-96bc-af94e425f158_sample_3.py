...
# Define the loop for continuous monitoring and quality checks
# A: pH Monitoring -> Texture Check -> Flavor Profiling
A = StrictPartialOrder(nodes=[pH_monitoring, texture_check, flavor_profiling])
A.order.add_edge(pH_monitoring, texture_check)
A.order.add_edge(texture_check, flavor_profiling)

# B: Supplier Audit -> Culture Prep -> Milk Testing
B = StrictPartialOrder(nodes=[supplier_audit, culture_prep, milk_testing])
B.order.add_edge(supplier_audit, culture_prep)
B.order.add_edge(culture_prep, milk_testing)

# Loop: execute A, then choose to exit or execute B then A again
...
root.order.add_edge(fermentation_start, loop)
