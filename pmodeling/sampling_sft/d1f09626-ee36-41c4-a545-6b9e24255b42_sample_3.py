...
# Define the maintenance loop: Growth Monitor -> Waste Recycle -> Yield Report
maintenance_loop = OperatorPOWL(operator=Operator.LOOP, children=[growth_monitor, waste_recycle, yield_report])

# Build the overall partial order
root = StrictPartialOrder(nodes=[
    site_survey, load_test, soil_sample, climate_check, crop_select,
    irrigation_plan, energy_setup, pest_control, permit_obtain,
    stakeholder_meet, bed_construction, seed_planting, water_schedule,
    maintenance_loop
])
...
root.order.add_edge(water_schedule, maintenance_loop)
root.order.add_edge(maintenance_loop, growth_monitor)
root.order.add_edge(maintenance_loop, waste_recycle)
