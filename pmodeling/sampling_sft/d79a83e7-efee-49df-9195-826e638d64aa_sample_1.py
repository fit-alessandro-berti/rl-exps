...
loop_body = StrictPartialOrder(nodes=[yield_analysis, data_review])
loop_body.order.add_edge(yield_analysis, data_review)
loop = OperatorPOWL(operator=Operator.LOOP, children=[loop_body, community_engage])

# Assemble the overall partial order
root = StrictPartialOrder(nodes=[
    site_survey, env_assessment, reg_compliance,
    modular_setup, crop_selection, iot_integration,
    nutrient_flow, light_calibration, staff_training,
    pest_control, market_strategy, logistics_plan,
    loop
])
