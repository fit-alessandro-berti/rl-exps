...
# Loop: after feedback, repeat until approved
loop = OperatorPOWL(operator=Operator.LOOP, children=[feedback_loop, skip])
...
root.order.add_edge(loop, quality_review)
root.order.add_edge(loop, technical_check)
...
root.order.add_edge(archival_record, client_meet)  # Loop back to client meet