...
# Define the loop for ongoing activities (monitoring & calibration)
monitoring_loop = OperatorPOWL(
    operator=Operator.LOOP,
    children=[pest_monitor, nutrient_calibrate]
)

# Build the partial order
root = StrictPartialOrder(nodes=[
    site_assess, structure_check, permit_obtain,
    material_source, soil_prepare,
    waterproof_roof, irrigation_setup, bed_assemble,
    crop_plan, monitoring_loop,
    harvest_schedule, community_train,
    yield_record, impact_review
])
...
root.order.add_edge(crop_plan,       monitoring_loop)
