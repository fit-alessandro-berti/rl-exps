...
# Define the monitoring and maintenance loop:
# A = {Data Monitor -> Yield Forecast -> Energy Plan -> Maintenance Plan}
A = StrictPartialOrder(nodes=[data_monitor, yield_forecast, energy_plan, maintenance_plan])
A.order.add_edge(data_monitor, yield_forecast)
A.order.add_edge(yield_forecast, energy_plan)
A.order.add_edge(energy_plan, maintenance_plan)

# B = Harvest Prep -> Supply Dispatch -> Market Launch
B = StrictPartialOrder(nodes=[harvest_prep, supply_dispatch, market_launch])
B.order.add_edge(harvest_prep, supply_dispatch)
B.order.add_edge(supply_dispatch, market_launch)

# LOOP(A, B): do A, then either exit or do B and then A again
...
root.order.add_edge(climate_control, monitor_loop)
