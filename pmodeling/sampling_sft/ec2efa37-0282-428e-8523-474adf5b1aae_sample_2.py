...
# Loop for IoT monitoring and risk assessment
iot_loop = OperatorPOWL(
    operator=Operator.LOOP,
    children=[iot_monitor, risk_assess]
)

# Loop for compliance check and maintenance alert
compliance_loop = OperatorPOWL(
    operator=Operator.LOOP,
    children=[compliance_check, maintenance_alert]
)
...
root.order.add_edge(procurement_plan, feedback_loop)
root.order.add_edge(production_align, feedback_loop)
...
root.order.add_edge(delivery_track, compliance_loop)
root.order.add_edge(compliance_loop, compliance_check)