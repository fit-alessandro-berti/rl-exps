...
# Define the monitoring & analysis loop body (yield_monitor -> data_analysis -> system_upgrade)
monitor_body = StrictPartialOrder(nodes=[yield_monitor, data_analysis, system_upgrade])
monitor_body.order.add_edge(yield_monitor, data_analysis)
monitor_body.order.add_edge(data_analysis, system_upgrade)

# Define the loop: do Crop Planting, then either exit or execute the monitor_body and repeat
loop = OperatorPOWL(operator=Operator.LOOP, children=[crop_planting, monitor_body])
