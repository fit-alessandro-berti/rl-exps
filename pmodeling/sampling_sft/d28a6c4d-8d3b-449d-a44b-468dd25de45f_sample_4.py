...
# Build the loop for the feedback‐testing cycle
loop_body = StrictPartialOrder(nodes=[expert_review, field_testing, feedback_loop])
loop_body.order.add_edge(expert_review, field_testing)
loop_body.order.add_edge(field_testing, feedback_loop)
...
root.order.add_edge(prototype_build, loop_body)
root.order.add_edge(loop_body, loop_body)  # Loop self-reference
root.order.add_edge(loop_body, ip_analysis)
...
root.order.add_edge(launch_prep, scale_planning)
