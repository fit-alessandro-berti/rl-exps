...
# Loop for ongoing maintenance and data monitoring
maintenance_loop = OperatorPOWL(
    operator=Operator.LOOP,
    children=[maintenance, data_monitor]
)
...
root.order.add_edge(equipment_buy, pilot_plant)
root.order.add_edge(sensor_setup, pilot_plant)
root.order.add_edge(irrigation_fit, pilot_plant)
root.order.add_edge(solar_install, pilot_plant)
root.order.add_edge(staff_train, pilot_plant)
root.order.add_edge(pilot_plant, crop_harvest)
root.order.add_edge(pilot_plant, maintenance_loop)
root.order.add_edge(pilot_plant, community_meet)
