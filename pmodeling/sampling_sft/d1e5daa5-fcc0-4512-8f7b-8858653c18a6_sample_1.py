...
# Loop for ongoing system performance monitoring
skip = SilentTransition()
loop = OperatorPOWL(operator=Operator.LOOP, children=[monitor_systems, skip])

# Build the partial order
root = StrictPartialOrder(nodes=[
    site_survey, regulation_check, design_modules, install_hydro,
    integrate_sensors, calibrate_nut, program_climate, select_crops,
    optimize_light, train_staff, plan_harvest, recycle_waste,
    analyze_demand, plan_logistics, loop
])
...
root.order.add_edge(install_hydro, integrate_sensors)
root.order.add_edge(integrate_sensors, calibrate_nut)
...
root.order.add_edge(plan_logistics, loop)