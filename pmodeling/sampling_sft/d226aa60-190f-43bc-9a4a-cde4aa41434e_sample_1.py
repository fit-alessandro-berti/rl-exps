...
root = StrictPartialOrder(nodes=[
    ...
    sensor_check, data_analyze, water_adjust, light_control, humidity_monitor,
    pest_inspect, growth_forecast, harvest_plan, rapid_cool,
    quality_grade, eco_package, logistics_prep, feedback_collect,
    system_maintain
])
...
# After sensor checks, parallelize growth forecast and harvest plan
root.order.add_edge(sensor_check, growth_forecast)
root.order.add_edge(sensor_check, harvest_plan)

# After growth forecast, parallelize pest inspection
root.order.add_edge(growth_forecast, pest_inspect)

# Both growth forecast and pest inspection can trigger harvest plan
root.order.add_edge(growth_forecast, harvest_plan)
root.order.add_edge(pest_inspect, harvest_plan)

# Harvest plan branches to either rapid cool or parallelize cooling and packaging
root.order.add_edge(harvest_plan, rapid_cool)
root.order.add_edge(harvest_plan, quality_grade)
root.order.add_edge(harvest_plan, eco_package)

# Both rapid cool and packaging can trigger logistics prep
root.order.add_edge(rapid_cool, logistics_prep)
root.order.add_edge(eco_package, logistics_prep)

# After logistics prep, feedback collect can start
root.order.add_edge(logistics_prep, feedback_collect)

# Feedback collect can optionally trigger system maintenance
