import pm4py
from pm4py.objects.powl.obj import StrictPartialOrder, OperatorPOWL, Transition, SilentTransition
from pm4py.objects.process_tree.obj import Operator

import pm4py
from pm4py.objects.powl.obj import StrictPartialOrder, OperatorPOWL, Transition
from pm4py.objects.process_tree.obj import Operator

# Define all activities as transitions
seed_sourcing    = Transition(label='Seed Sourcing')
germination_chk  = Transition(label='Germination Check')
nutrient_mix     = Transition(label='Nutrient Mix')
automated_plant  = Transition(label='Automated Planting')
climate_ctrl     = Transition(label='Climate Control')
crop_scanning    = Transition(label='Crop Scanning')
pest_monitoring  = Transition(label='Pest Monitoring')
growth_analysis  = Transition(label='Growth Analysis')
robotic_harvest  = Transition(label='Robotic Harvest')
quality_sort     = Transition(label='Quality Sort')
eco_packaging    = Transition(label='Eco Packaging')
blockchain_track = Transition(label='Blockchain Track')
route_planning   = Transition(label='Route Planning')
feedback_collect = Transition(label='Feedback Collect')
waste_recycling  = Transition(label='Waste Recycling')
demand_forecast  = Transition(label='Demand Forecast')
data_analytics   = Transition(label='Data Analytics')
maintenance_alert= Transition(label='Maintenance Alert')

# Loop for continuous monitoring & analysis
monitoring_loop = OperatorPOWL(
    operator=Operator.LOOP,
    children=[pest_monitoring, growth_analysis]
)

# Build the overall partial order
root = StrictPartialOrder(nodes=[
    seed_sourcing,
    germination_chk,
    nutrient_mix,
    automated_plant,
    climate_ctrl,
    crop_scanning,
    monitoring_loop,
    robotic_harvest,
    quality_sort,
    eco_packaging,
    blockchain_track,
    route_planning,
    feedback_collect,
    waste_recycling,
    demand_forecast,
    data_analytics,
    maintenance_alert
])

# Define the control-flow dependencies
root.order.add_edge(seed_sourcing,   germination_chk)
root.order.add_edge(germination_chk, nutrient_mix)
root.order.add_edge(nutrient_mix,    automated_plant)
root.order.add_edge(automated_plant, climate_ctrl)
root.order.add_edge(climate_ctrl,    crop_scanning)
root.order.add_edge(crop_scanning,   monitoring_loop)
root.order.add_edge(monitoring_loop, robotic_harvest)
root.order.add_edge(robotic_harvest, quality_sort)
root.order.add_edge(quality_sort,    eco_packaging)
root.order.add_edge(eco_packaging,   blockchain_track)
root.order.add_edge(blockchain_track, route_planning)
root.order.add_edge(route_planning,  feedback_collect)
root.order.add_edge(feedback_collect, waste_recycling)
root.order.add_edge(waste_recycling, demand_forecast)
root.order.add_edge(demand_forecast, data_analytics)
root.order.add_edge(data_analytics, maintenance_alert)