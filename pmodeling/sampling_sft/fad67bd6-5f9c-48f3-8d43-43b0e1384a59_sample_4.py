...
# Loop for continuous performance review and impact study
perf_loop = OperatorPOWL(
    operator=Operator.LOOP,
    children=[performance_review, impact_study]
)
...
root.order.add_edge(route_update, perf_loop)
root.order.add_edge(perf_loop, compliance_review)

# Final edge to compliance review
root.order.add_edge(compliance_review, compliance_review)  # loop back to itself for continuous compliance review

# Final edge to end of the process
root.order.add_edge(compliance_review, compliance_review)  # loop back to itself for continuous compliance review