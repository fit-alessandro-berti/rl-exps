...
# Loop for seasonal review and market adjustment
loop = OperatorPOWL(operator=Operator.LOOP, children=[seasonal_review, market_adjustment])
...
root.order.add_edge(retail_coordination, loop)

# Loop body: after retail coordination, do consumer survey then feedback analysis
root.order.add_edge(retail_coordination, consumer_survey)
root.order.add_edge(consumer_survey, feedback_analysis)

# Loop body: after feedback analysis, go back to seasonal review
root.order.add_edge(feedback_analysis, loop)

# Run the loop
root.order.add_edge(loop, seasonal_review)
root.order.add_edge(loop, market_adjustment)
