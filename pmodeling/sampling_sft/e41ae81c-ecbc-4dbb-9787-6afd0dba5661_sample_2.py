...
# Define the continuous monitoring loop body: Growth Monitor -> Pest Control
body = StrictPartialOrder(nodes=[growth_monitor, pest_control])
body.order.add_edge(growth_monitor, pest_control)

# Loop: do System Testing, then either exit or do body then System Testing again
monitor_loop = OperatorPOWL(operator=Operator.LOOP, children=[system_testing, body])
...
root.order.add_edge(quality_check, harvest_schedule)
