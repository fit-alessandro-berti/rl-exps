...
# Define the harvest‐to‐distribution loop: Harvest Scheduling -> Distribution Plan, then optionally repeat
harvest_to_dist = OperatorPOWL(operator=Operator.LOOP, children=[harvest_sched, distribution_plan])

# Build the top‐level partial order
root = StrictPartialOrder(nodes=[
    site_survey, permit_filing, structure_design, system_install,
    hydro_setup, climate_config, ai_integration, nutrient_sourcing,
    waste_planning, staff_training, crop_seeding, growth_monitoring,
    quality_testing, harvest_to_dist, data_analysis
])
...
root.order.add_edge(quality_testing, harvest_to_dist)
root.order.add_edge(harvest_to_dist, data_analysis)
