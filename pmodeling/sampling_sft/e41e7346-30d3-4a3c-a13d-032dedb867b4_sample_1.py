...
# Build the monitoring & control loop: monitor -> pest detect -> harvest plan
monitor_body = StrictPartialOrder(nodes=[growth_monitor, pest_detect])
monitor_body.order.add_edge(growth_monitor, pest_detect)

harvest_loop = OperatorPOWL(operator=Operator.LOOP, children=[monitor_body, harvest_plan])

# Build the overall process: seed -> nutrient -> climate -> light -> co2 -> humidity -> harvest_loop -> produce -> pack -> dispatch -> waste -> compost -> review
root = StrictPartialOrder(nodes=[
    seed_select, nutrient_mix, climate_setup, light_adjust, co2_control, humidity_tune,
    harvest_loop, produce_sort, pack_biodeg, drone_dispatch, waste_recycle, compost_create, cycle_review
])
...
root.order.add_edge(humidity_tune, harvest_loop)
root.order.add_edge(harvest_loop,  produce_sort)
