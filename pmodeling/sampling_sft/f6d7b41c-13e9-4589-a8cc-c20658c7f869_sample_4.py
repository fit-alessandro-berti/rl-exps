...
# Loop for renewal: do Lease Renew, then either exit or do Payment Process and Lease Renew again
loop_renew = OperatorPOWL(operator=Operator.LOOP, children=[lease_renew, payment_process])
root.order.add_edge(appraisal_log, loop_renew)

# Final deinstallation and return options
root.order.add_edge(loop_renew, deinstall_art)
root.order.add_edge(deinstall_art, return_inspect)
root.order.add_edge(return_inspect, purchase_option)

# Finalize the workflow
root.order.add_edge(purchase_option, Transition(label='End'))