...
# Define the monitoring & optimization loop body
body = StrictPartialOrder(nodes=[data_monitor, yield_analyze])
body.order.add_edge(data_monitor, yield_analyze)
...
# Loop entry point
root.order.add_edge(marketing_sync, body)

# Loop exit and final edges
loop_exit = Transition(label='Loop Exit')
root.order.add_edge(body, loop_exit)
root.order.add_edge(loop_exit, marketing_sync)
