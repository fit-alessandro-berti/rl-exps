...
# Build the loop body: execute updates, then optionally run the feedback loop
body = StrictPartialOrder(nodes=[execute_updates, feedback_loop])
body.order.add_edge(execute_updates, feedback_loop)

# Define the loop: Risk Assess -> Model Run -> Option Select -> Team Review -> Plan Approve -> (Procure Adjust -> Route Replan -> Inventory Shift) -> body
loop = OperatorPOWL(operator=Operator.LOOP, children=[risk_assess, model_run, option_select, team_review, plan_approve, body])

# Assemble the overall partial order
root = StrictPartialOrder(nodes=[
    data_ingest, status_check, forecast_update, risk_assess,
    scenario_sim, model_run, option_select, team_review, plan_approve,
