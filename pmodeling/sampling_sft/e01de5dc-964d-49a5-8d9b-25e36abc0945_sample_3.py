...
monitor_body.order.add_edge(monitor_climate, control_humidity)
monitor_body.order.add_edge(control_humidity, co2_regulation)
monitor_body.order.add_edge(co2_regulation, detect_pests)
monitor_body.order.add_edge(detect_pests, deploy_bio)

# Loop: repeat monitoring_body until exit
monitor_loop = OperatorPOWL(operator=Operator.LOOP, children=[monitor_body, system_maintenance])
...
root.order.add_edge(monitor_loop, schedule_harvest)
