...
# Define the maintenance loop: Data Review -> System Upgrade
maintenance_loop = OperatorPOWL(operator=Operator.LOOP, children=[data_review, system_upgrade])

# Assemble the root partial order
root = StrictPartialOrder(nodes=[
    site_survey, design_layout, climate_setup, sensor_install,
    automation_code, nutrient_mix, crop_planning, pest_control,
    energy_audit, waste_sort, planting_tier, harvest_prep,
    logistics_plan, community_meet, maintenance_loop
])
...
root.order.add_edge(automation_code, nutrient_mix)
...
root.order.add_edge(community_meet, maintenance_loop)
