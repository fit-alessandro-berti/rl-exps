...
# Loop for continuous monitoring and health maintenance
# Body A: Health Monitor -> Pest Control
body_a = StrictPartialOrder(nodes=[health_monitor, pest_control])
body_a.order.add_edge(health_monitor, pest_control)

# Body B: Data Analysis -> Maintenance Plan
body_b = StrictPartialOrder(nodes=[data_analysis, maintenance_plan])
body_b.order.add_edge(data_analysis, maintenance_plan)

# LOOP operator: do Body A, then either exit or do Body B and repeat
...
root = StrictPartialOrder(nodes=[
    ...
    installation_prep, hive_setup, sensor_install,
    honey_harvest, wax_processing, product_packaging,
    order_dispatch, workshop_setup, community_outreach,
    regulation_check, monitor_loop
])
