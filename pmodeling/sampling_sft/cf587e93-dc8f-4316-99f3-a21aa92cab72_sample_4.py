...
# Loop for continuous growth cycle: 
#   Growth Scan -> Pest Control -> Water Cycle -> Harvest Robo -> Yield Assess
growth_cycle = StrictPartialOrder(nodes=[
    growth_scan, pest_control, water_cycle, harvest_robo, yield_assess
])
...
# Loop for waste processing: Waste Process -> Energy Sync -> Pack Biodeg
waste_loop = OperatorPOWL(operator=Operator.LOOP, children=[waste_process, energy_sync])
waste_loop.order.add_edge(waste_process, energy_sync)
waste_loop.order.add_edge(energy_sync, pack_biodeg)

# Main process partial order
root = StrictPartialOrder(nodes=[
    seed_select, nutrient_mix, sensor_setup, env_monitor,
    growth_cycle, waste_loop,
...
root.order.add_edge(env_monitor, growth_cycle)
root.order.add_edge(growth_cycle, waste_loop)
root.order.add_edge(waste_loop, market_track)
