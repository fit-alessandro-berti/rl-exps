...
root = StrictPartialOrder(nodes=[
    site_select, env_assess, design_modules, hydroponics_setup,
    software_dev, seed_choose, led_install, train_staff,
    compliance_check, engage_community, plant_crops,
    monitor_growth, optimize_yields, waste_manage,
...
# Plant crops and monitoring are concurrent
root.order.add_edge(plant_crops, monitor_growth)
root.order.add_edge(plant_crops, optimize_yields)

# After monitoring, waste management and energy audit can occur
root.order.add_edge(monitor_growth, waste_manage)
root.order.add_edge(monitor_growth, energy_audit)

# After energy audit, water recycling can occur
root.order.add_edge(energy_audit, water_recycle)

# Waste management and water recycling can occur concurrently
root.order.add_edge(waste_manage, water_recycle)

# All final activities are optional and can follow either waste_manage or water_recycle
root.order.add_edge(waste_manage, optimize_yields)
root.order.add_edge(water_recycle, optimize_yields)