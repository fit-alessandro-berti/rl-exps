...
root = StrictPartialOrder(nodes=[
    milk_sourcing, farm_audit, milk_testing, batch_forming,
    curd_cutting, molding_cheese, salting_process, aging_control,
    quality_check, packaging_design, label_printing, inventory_update,
    order_receiving, retail_coordination, shipping_prep,
    customer_feedback, demand_forecast, limited_release
])

# Define the control-flow dependencies
# Milk sourcing -> Farm Audit -> Milk Testing -> Batch Forming
...
# Batch Forming -> Curd Cutting -> Molding Cheese -> Salting Process
...
# Salting Process -> Aging Control -> Quality Check
root.order.add_edge(salting_process, aging_control)
root.order.add_edge(aging_control, quality_check)

# Quality Check -> Packaging Design -> Label Printing -> Inventory Update
...
# Inventory Update -> Order Receiving -> Retail Coordination
...
# All nodes are concurrent except the final release
for node in root.nodes:
    if node != limited_release:
        root.order.add_edge(node, limited_release)