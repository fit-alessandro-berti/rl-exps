import pm4py
from pm4py.objects.powl.obj import StrictPartialOrder, OperatorPOWL, Transition, SilentTransition
from pm4py.objects.process_tree.obj import Operator

import pm4py
from pm4py.objects.powl.obj import StrictPartialOrder, OperatorPOWL, Transition, SilentTransition
from pm4py.objects.process_tree.obj import Operator

# Define all activities as POWL transitions
site_prep     = Transition(label='Site Prep')
seed_select   = Transition(label='Seed Select')
nutrient_mix  = Transition(label='Nutrient Mix')
plant_rows    = Transition(label='Planting Rows')
env_monitor   = Transition(label='Env Monitor')
water_adjust  = Transition(label='Water Adjust')
pest_control  = Transition(label='Pest Control')
growth_check  = Transition(label='Growth Check')
light_cal     = Transition(label='Light Calibrate')
energy_manage = Transition(label='Energy Manage')
harvest_crop  = Transition(label='Harvest Crop')
quality_sort  = Transition(label='Quality Sort')
pack_goods    = Transition(label='Pack Goods')
cold_store    = Transition(label='Cold Store')
market_ship   = Transition(label='Market Ship')
data_analyze  = Transition(label='Data Analyze')

# Silent transition for loop exits
skip = SilentTransition()

# Loop body: Env Monitor -> Water Adjust -> Pest Control -> Growth Check -> Light Calibrate -> Energy Manage
loop_body = StrictPartialOrder(nodes=[
    env_monitor, water_adjust, pest_control, growth_check, light_cal, energy_manage
])
loop_body.order.add_edge(env_monitor, water_adjust)
loop_body.order.add_edge(water_adjust, pest_control)
loop_body.order.add_edge(pest_control, growth_check)
loop_body.order.add_edge(growth_check, light_cal)
loop_body.order.add_edge(light_cal, energy_manage)

# Loop: execute the body, then optionally exit or do the body again
loop = OperatorPOWL(operator=Operator.LOOP, children=[loop_body, skip])

# Build the overall partial order
root = StrictPartialOrder(nodes=[
    site_prep, seed_select, nutrient_mix, plant_rows,
    loop, harvest_crop, quality_sort, pack_goods,
    cold_store, market_ship, data_analyze
])

# Sequential dependencies
root.order.add_edge(site_prep, seed_select)
root.order.add_edge(seed_select, nutrient_mix)
root.order.add_edge(nutrient_mix, plant_rows)
root.order.add_edge(plant_rows, loop)
root.order.add_edge(loop, harvest_crop)
root.order.add_edge(harvest_crop, quality_sort)
root.order.add_edge(quality_sort, pack_goods)
root.order.add_edge(pack_goods, cold_store)
root.order.add_edge(cold_store, market_ship)
root.order.add_edge(market_ship, data_analyze)