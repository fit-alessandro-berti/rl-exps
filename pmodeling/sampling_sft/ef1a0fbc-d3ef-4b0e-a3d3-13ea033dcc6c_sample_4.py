...
root.order.add_edge(retail_shipping, inventory_audit)

# Loop for continuous quality testing and aging
test_loop = OperatorPOWL(
    operator=Operator.LOOP,
    children=[flavor_tasting, aging_control]
)
root.order.add_edge(packaging_design, test_loop)

# Loop for continuous inventory audit
audit_loop = OperatorPOWL(
    operator=Operator.LOOP,
    children=[inventory_audit, retail_shipping]
)
root.order.add_edge(order_fulfill, audit_loop)