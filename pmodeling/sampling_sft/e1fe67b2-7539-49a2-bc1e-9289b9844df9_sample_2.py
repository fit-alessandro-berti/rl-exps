...
# Loop for iterative quality assurance: do Component Test, then optionally Quality Check and repeat
loop_test = OperatorPOWL(
    operator=Operator.LOOP,
    children=[component_test, quality_check]
)
...
root = StrictPartialOrder(nodes=[
    ...
    loop_test, software_sync, flight_calibrate, enviro_test,
    remote_pair, package_unit, register_drone, client_train
])
...
root.order.add_edge(sensor_align, loop_test)
root.order.add_edge(loop_test, software_sync)
...
root.order.add_edge(remote_pair, package_unit)
