...
flight_cal     = Transition(label='Flight Calibrate')
...
# Build the loop body: flight calibration and environment testing
body = StrictPartialOrder(nodes=[flight_cal, enviro_test])
# No edges => they are concurrent

# Build the loop: repeat body until exit
loop = OperatorPOWL(operator=Operator.LOOP, children=[body, body])

# Assemble the overall partial order
root = StrictPartialOrder(nodes=[
    gather_specs, design_custom, source_parts, firmware_load,
    component_test, mechanical_fit, cable_routing, sensor_align,
    software_sync, loop,
    remote_pair, quality_check, package_unit,
    register_drone, client_train
])
...
root.order.add_edge(firmware_load, component_test)
root.order.add_edge(component_test, mechanical_fit)
...
root.order.add_edge(software_sync, loop)
root.order.add_edge(loop, remote_pair)
