...
root = StrictPartialOrder(nodes=[
    site_survey, permit_filing, stakeholder_meet,
    design_layout, iot_install, sensor_calibrate,
    hydroponic_setup, nutrient_mix, seed_sowing,
    climate_control, data_monitor, yield_forecast,
    energy_plan, maintenance_plan, harvest_prep,
    supply_dispatch, market_launch
])
...
# After climate control, monitor and then forecast yield
root.order.add_edge(climate_control, data_monitor)
root.order.add_edge(data_monitor,    yield_forecast)

# Parallelize energy plan and maintenance plan after yield forecast
root.order.add_edge(yield_forecast, energy_plan)
root.order.add_edge(yield_forecast, maintenance_plan)

# After energy plan and maintenance, proceed to harvest, supply, and launch
root.order.add_edge(energy_plan,    harvest_prep)
