...
# Loop for seasonal review & repeat
loop_seasonal = OperatorPOWL(operator=Operator.LOOP, children=[seasonal_review, seasonal_review])

# Exclusive choice: either Regulation Check or Waste Manage (skip)
xor_regulation = OperatorPOWL(operator=Operator.XOR, children=[regulation_check, waste_manage])
...
root.order.add_edge(sales_report, xor_regulation)
root.order.add_edge(xor_regulation, loop_seasonal)
# Loop edges: first edge of loop is after the first repeat
root.order.add_edge(loop_seasonal, seasonal_review)
root.order.add_edge(loop_seasonal, seasonal_review)