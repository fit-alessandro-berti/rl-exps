...
# Loop for continuous quality improvement
# 1. Customer Feedback
# 2. Subscription Adjust
# 3. Repeat
feedback_loop = OperatorPOWL(
    operator=Operator.LOOP,
    children=[customer_feedback, subscription_adjust]
)
...
root.order.add_edge(fermentation, quality_control)
...
root.order.add_edge(roast_profiling, traceability_qr)
root.order.add_edge(eco_packaging, cold_transport)
root.order.add_edge(eco_packaging, env_monitoring)
root.order.add_edge(cold_transport, feedback_loop)
root.order.add_edge(env_monitoring, feedback_loop)

# Final loop node is the continuous improvement loop
