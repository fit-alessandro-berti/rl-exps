...
# Loop for continuous monitoring: monitor -> feedback -> repeat
monitor = StrictPartialOrder(nodes=[market_monitor, team_sync])
monitor.order.add_edge(market_monitor, team_sync)

loop = OperatorPOWL(operator=Operator.LOOP, children=[monitor, feedback_loop])
...
root.order.add_edge(source_altern, contract_review)
root.order.add_edge(source_altern, regulation_check)
root.order.add_edge(supplier_audit, logistics_reroute)
root.order.add_edge(contract_review, logistics_reroute)
root.order.add_edge(regulation_check, logistics_reroute)
root.order.add_edge(logistics_reroute, customs_notify)
root.order.add_edge(logistics_reroute, stakeholder_alert)
root.order.add_edge(customs_notify, data_analyze)
root.order.add_edge(stakeholder_alert, data_analyze)
root.order.add_edge(data_analyze, cost_forecast)
root.order.add_edge(data_analyze, compliance_verify)
root.order.add_edge(cost_forecast, scenario_plan)
root.order.add_edge(compliance_verify, scenario_plan)
root.order.add_edge(scenario_plan, decision_gate)
root.order.add_edge(decision_gate, report_generate)
