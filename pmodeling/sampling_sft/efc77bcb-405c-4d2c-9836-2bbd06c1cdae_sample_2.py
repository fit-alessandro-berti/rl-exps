...
# Loop body: Risk Assess -> Scenario Sim -> Feedback Loop -> AI Adjust
loop_body = StrictPartialOrder(nodes=[risk_assess, scenario_sim, feedback_loop, ai_adjust])
loop_body.order.add_edge(risk_assess, scenario_sim)
loop_body.order.add_edge(scenario_sim, feedback_loop)
loop_body.order.add_edge(feedback_loop, ai_adjust)

# LOOP operator: do Risk Assess -> Scenario Sim, then optionally do Feedback Loop -> AI Adjust and repeat
risk_loop = OperatorPOWL(operator=Operator.LOOP, children=[risk_assess, loop_body])
...
root.order.add_edge(blockchain_verify, risk_loop)
root.order.add_edge(risk_loop, waste_audit)
