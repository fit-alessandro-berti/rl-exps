...
# Loop for continuous monitoring and adaptive adjustments
monitor_loop = OperatorPOWL(
    operator=Operator.LOOP,
    children=[microclimate_mon, health_monitor]
)
...
root = StrictPartialOrder(nodes=[
    ...
    sensor_setup, monitor_loop,
    light_adjust, irrigation_mod, waste_recycle, energy_optimize,
    quality_check, crop_harvest, distribution_plan
])
...
root.order.add_edge(monitor_loop, light_adjust)
root.order.add_edge(monitor_loop, irrigation_mod)
root.order.add_edge(monitor_loop, waste_recycle)
root.order.add_edge(monitor_loop, energy_optimize)
root.order.add_edge(light_adjust, quality_check)
root.order.add_edge(irrigation_mod, quality_check)
