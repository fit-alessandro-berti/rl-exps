...
# Define the monitoring & optimization loop: repeat Data Monitor then Optimize Yield
loop_body = StrictPartialOrder(nodes=[data_monitor, optimize_yield])
loop_body.order.add_edge(data_monitor, optimize_yield)

monitor_loop = OperatorPOWL(operator=Operator.LOOP, children=[data_monitor, optimize_yield])

# Build the overall partial order
root = StrictPartialOrder(nodes=[
    site_survey, design_layout, system_build, install_sensors,
    select_crops, setup_lighting, configure_climate, nutrient_mix,
    automate_watering, test_systems, train_staff, waste_plan,
    market_link, monitor_loop
])
...
root.order.add_edge(market_link,     monitor_loop)