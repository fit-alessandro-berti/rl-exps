...
# Body of the loop: all activities except the initial scenario setup
...
# Define the loop: repeat scenario setup first, then execute the body, then repeat
loop = OperatorPOWL(operator=Operator.LOOP, children=[scenario_setup, loop_body])

# Root partial order: the entire process as a strict partial order
root = StrictPartialOrder(nodes=[loop])
# Add the control-flow edges: after the loop, do the final analysis & reporting
root.order.add_edge(loop, outcome_analysis)
root.order.add_edge(outcome_analysis, report_generation)
root.order.add_edge(loop, improvement_plan)