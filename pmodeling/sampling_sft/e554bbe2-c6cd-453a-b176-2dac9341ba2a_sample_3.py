...
# Loop for seasonal review and feedback analysis
loop_season = OperatorPOWL(operator=Operator.LOOP, children=[seasonal_review, feedback_analysis])

# Build the partial order
root = StrictPartialOrder(nodes=[
    farm_selection, milk_testing, starter_culture, curd_formation,
    pressing_cheese, microbial_profiling, aging_control, hand_wrapping,
    quality_audit, packaging_prep, climate_shipping, retail_coordination,
    loop_season, consumer_survey, market_adjustment
])
...
root.order.add_edge(retail_coordination, loop_season)

# Loop body: consumer_survey -> market_adjustment
root.order.add_edge(loop_season, consumer_survey)
root.order.add_edge(consumer_survey, market_adjustment)