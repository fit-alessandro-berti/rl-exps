...
# Build the loop body for validation: historical review -> chemical test -> provenance audit -> expert panel
validation_body = StrictPartialOrder(nodes=[historical_review, chemical_test, provenance_audit, expert_panel])
validation_body.order.add_edge(historical_review, chemical_test)
validation_body.order.add_edge(chemical_test, provenance_audit)
validation_body.order.add_edge(provenance_audit, expert_panel)

# LOOP: do validation_body, then optionally repeat
validation_loop = OperatorPOWL(operator=Operator.LOOP, children=[validation_body, expert_panel])
...
root.order.add_edge(preliminary_check, validation_loop)
