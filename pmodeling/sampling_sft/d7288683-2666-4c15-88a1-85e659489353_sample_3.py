...
fsim = Transition(label='Flight Simulation')
...
# Build the flight optimization loop: simulate -> adjust -> simulate again
flight_loop = OperatorPOWL(operator=Operator.LOOP, children=[fsim, pa])

# Build the overall partial order
root = StrictPartialOrder(nodes=[
    cs, fa, mi, sm, ws, fu, ai, cal, st, flight_loop, qi, cc, pf, ds
])

# Define the control-flow dependencies
root.order.add_edge(cs, fa)
root.order.add_edge(cs, mi)
root.order.add_edge(cs, sm)
root.order.add_edge(cs, ws)
root.order.add_edge(fa, fu)
root.order.add_edge(mi, fu)
root.order.add_edge(sm, fu)
root.order.add_edge(ws, fu)
root.order.add_edge(fu, cal)
...
# The last two nodes can run in parallel
root.order.add_edge(ds, Transition(label='Done'))