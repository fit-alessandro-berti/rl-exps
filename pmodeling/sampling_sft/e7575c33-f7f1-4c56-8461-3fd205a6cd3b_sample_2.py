...
# A = Style Compare, B = 3D Scanning
loop = OperatorPOWL(operator=Operator.LOOP, children=[style_compare, three_d_scanning])

# Build the top‐level partial order
root = StrictPartialOrder(nodes=[
    artifact_intake, catalog_entry, visual_inspect, material_test, spectroscopy,
    historical_check, provenance_trace, loop,
    condition_assess, preservation_plan, legal_review,
    report_draft, report_finalize, archive_data, sale_prep
])
...
# Note: There are no explicit exits from the loop, so it continues indefinitely