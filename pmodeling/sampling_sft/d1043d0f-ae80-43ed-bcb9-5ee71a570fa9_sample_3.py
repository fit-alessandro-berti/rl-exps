...
# Define the loop for continuous improvement
# Body (A): Data Review -> Yield Forecast
body = StrictPartialOrder(nodes=[data_review, yield_forecast])
body.order.add_edge(data_review, yield_forecast)

# Loop: do Sensor Deploy, then optionally repeat Body
loop = OperatorPOWL(operator=Operator.LOOP, children=[sensor_deploy, body])
...
root = StrictPartialOrder(nodes=[
    ...
    staff_train, yield_forecast, market_plan, loop
])
...
root.order.add_edge(staff_train, market_plan)

# Loop body must be after staff training
root.order.add_edge(staff_train, loop)

# Loop can optionally repeat after each iteration
