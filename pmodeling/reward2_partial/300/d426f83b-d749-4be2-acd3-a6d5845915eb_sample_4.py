...
root = StrictPartialOrder(nodes=[
    asset_intake, provenance_check, material_sampling, radiocarbon_test, style_compare, historical_search, expert_consult, 
    condition_review, scientific_analysis, data_compilation, peer_review, report_draft, certification, digital_archive, client_delivery
])

# Define the partial order dependencies
root.order.add_edge(asset_intake, provenance_check)
root.order.add_edge(asset_intake, material_sampling)
root.order.add_edge(provenance_check, historical_search)
...
root.order.add_edge(radiocarbon_test, peer_review)
root.order.add_edge(style_compare, peer_review)
...
# The final POWL model is defined by the 'root' variable