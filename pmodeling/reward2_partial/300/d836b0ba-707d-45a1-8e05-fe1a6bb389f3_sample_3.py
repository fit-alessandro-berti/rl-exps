...
Crop_Chose = Transition(label='Crop Choose')
...
root = StrictPartialOrder(nodes=[Site_Select, Design_Layout, Sensor_Integrate, Crop_Chose, Soil_Prepare, Irrigation_Setup, Pest_Control, Lighting_Install, Staff_Train, Compliance_Check, Market_Analyze, Package_Design, Logistics_Plan, Data_Analyze, Feedback_Loop])

# Define the dependencies (order) between the nodes
root.order.add_edge(Site_Select, Design_Layout)
root.order.add_edge(Design_Layout, Sensor_Integrate)
root.order.add_edge(Sensor_Integrate, Crop_Chose)
root.order.add_edge(Crop_Chose, Soil_Prepare)
