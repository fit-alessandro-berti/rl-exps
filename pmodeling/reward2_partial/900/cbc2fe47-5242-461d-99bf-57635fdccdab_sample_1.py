...
# Create a partial order with the defined transitions
root = StrictPartialOrder(nodes=[
    artifact_intake, document_check, provenance_search, ownership_validate,
    radiocarbon_test, spectroscopy_scan, material_analysis, style_assessment,
    context_review, expert_panel, report_draft, quality_review, catalog_entry,
    insurance_setup, archive_data, reevaluation_trigger
])
...
# Note: The 'reevaluation_trigger' node is left as a silent transition here as it does not have an explicit child node in the problem description.
# You might need to adjust the model based on additional information or requirements.