...
xor2 = OperatorPOWL(operator=Operator.XOR, children=[fleet_deploy, monitor_setup])
xor3 = OperatorPOWL(operator=Operator.XOR, children=[usage_track, maintenance_plan])
xor4 = OperatorPOWL(operator=Operator.XOR, children=[incident_manage, billing_process])
xor5 = OperatorPOWL(operator=Operator.XOR, children=[performance_report, contract_renew])
xor6 = OperatorPOWL(operator=Operator.XOR, children=[price_adjust, feedback_collect])

root = StrictPartialOrder(nodes=[client_onboard, needs_assess, drone_config, route_program, xor1, xor2, xor3, xor4, xor5, xor6])
...
root.order.add_edge(route_program, xor3)
root.order.add_edge(route_program, xor4)
root.order.add_edge(route_program, xor5)
root.order.add_edge(route_program, xor6)
