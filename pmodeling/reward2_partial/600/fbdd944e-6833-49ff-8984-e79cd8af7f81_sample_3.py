...
xor2 = OperatorPOWL(operator=Operator.XOR, children=[billing_process, performance_report])
xor3 = OperatorPOWL(operator=Operator.XOR, children=[incident_manage, contract_renew])
xor4 = OperatorPOWL(operator=Operator.XOR, children=[price_adjust, feedback_collect])

root = StrictPartialOrder(nodes=[client_onboard, needs_assess, drone_config, route_program, xor1, lease_contract, fleet_deploy, monitor_setup, usage_track, maintenance_plan, xor2, xor3, xor4])
...
root.order.add_edge(xor1, lease_contract)
...
root.order.add_edge(maintenance_plan, xor2)
root.order.add_edge(xor2, billing_process)
root.order.add_edge(billing_process, performance_report)
root.order.add_edge(performance_report, xor3)
root.order.add_edge(xor3, incident_manage)
root.order.add_edge(incident_manage, contract_renew)
root.order.add_edge(contract_renew, xor4)
root.order.add_edge(xor4, price_adjust)
