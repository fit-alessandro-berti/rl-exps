...
# Define POWL models
loop = OperatorPOWL(operator=Operator.LOOP, children=[Waiter_prepares_nonalcoholic_drinks, silent_transition])
xor = OperatorPOWL(operator=Operator.XOR, children=[Sommelier_fetches_wine_and_prepare_alcoholic_beverages, silent_transition])

# Define strict partial order
root = StrictPartialOrder(nodes=[Guest_calls_room_service, Manager_assigns_order_to_waiter, Manager_gives_order_to_sommelier, Manager_submits_order_ticket_to_kitchen, Manager_takes_down_the_order, Waiter_debits_guests_account, Waiter_delivers_order_to_guests_room, Waiter_readies_cart, Waiter_returns_to_room_service_station, Waiter_waits_delays_debiting, loop, xor])
...
root.order.add_edge(Sommelier_fetches_wine_and_prepare_alcoholic_beverages, Waiter_readies_cart)
...
root.order.add_edge(Waiter_prepares_nonalcoholic_drinks, Waiter_readies_cart)
