...
xor = OperatorPOWL(operator=Operator.XOR, children=[risk_assess, feedback_loop])
xor2 = OperatorPOWL(operator=Operator.XOR, children=[blockchain_verify, ai_adjust])
xor3 = OperatorPOWL(operator=Operator.XOR, children=[stakeholder_alert, compliance_review])

# Define the POWL root
root = StrictPartialOrder(nodes=[data_capture, quantum_calc, demand_forecast, inventory_check, route_update, shipment_plan, supplier_sync, contract_mod, xor, xor2, xor3, waste_audit, report_generate])
...
root.order.add_edge(contract_mod, xor)
root.order.add_edge(xor, xor2)
root.order.add_edge(xor2, xor3)
root.order.add_edge(xor3, waste_audit)
