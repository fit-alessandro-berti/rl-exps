...
hive_loop = OperatorPOWL(operator=Operator.LOOP, children=[hive_setup, health_monitor, pest_control, honey_harvest, wax_processing])
product_loop = OperatorPOWL(operator=Operator.LOOP, children=[product_packaging, order_dispatch])
workshop_loop = OperatorPOWL(operator=Operator.LOOP, children=[workshop_setup, community_outreach, regulation_check, data_analysis, maintenance_plan])

# Define partial order
root = StrictPartialOrder(nodes=[hive_loop, product_loop, workshop_loop, colony_sourcing, hive_design, site_survey, installation_prep, sensor_install])
...
root.order.add_edge(maintenance_plan, hive_loop)
root.order.add_edge(hive_loop, product_loop)
root.order.add_edge(product_loop, workshop_loop)