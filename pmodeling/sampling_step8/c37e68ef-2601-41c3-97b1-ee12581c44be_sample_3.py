...
fermentation_control_loop = OperatorPOWL(operator=Operator.LOOP, children=[fermentation_control])
aging_setup_loop = OperatorPOWL(operator=Operator.LOOP, children=[aging_setup])

# Define the partial order
root = StrictPartialOrder(nodes=[milk_sourcing, quality_testing, starter_prep, curd_cutting, whey_draining, molding_press, fermentation_control_loop, aging_setup_loop, humidity_check, packaging_design, label_approval, inventory_audit, order_scheduling, market_delivery, feedback_review, compliance_check, marketing_sync])
...
root.order.add_edge(molding_press, fermentation_control_loop)
root.order.add_edge(fermentation_control_loop, aging_setup_loop)
root.order.add_edge(aging_setup_loop, humidity_check)
