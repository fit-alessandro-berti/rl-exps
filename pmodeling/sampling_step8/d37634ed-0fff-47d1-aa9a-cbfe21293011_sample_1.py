...
root = StrictPartialOrder(nodes=[
    seed_selection, nutrient_mix, env_check, plant_setup, growth_monitor, pest_control,
    auto_harvest, quality_inspect, packaging_prep, order_fulfill, local_delivery, waste_collect,
    biomass_process, compost_create, energy_recover, regulation_review, community_engage
])

# Define the dependencies
root.order.add_edge(seed_selection, nutrient_mix)
root.order.add_edge(nutrient_mix, env_check)
root.order.add_edge(env_check, plant_setup)
root.order.add_edge(plant_setup, growth_monitor)
root.order.add_edge(growth_monitor, pest_control)
root.order.add_edge(pest_control, auto_harvest)
