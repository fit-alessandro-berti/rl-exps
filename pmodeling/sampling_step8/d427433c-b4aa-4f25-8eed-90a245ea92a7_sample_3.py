...
loop_fermentation = OperatorPOWL(operator=Operator.LOOP, children=[fermentation_check, curd_cutting, whey_separation, molding_press, salting_stage, aging_control])
choice_packaging = OperatorPOWL(operator=Operator.XOR, children=[packaging_design, cold_shipping])
choice_logistics = OperatorPOWL(operator=Operator.XOR, children=[order_fulfillment, feedback_review])

# Define the root model
root = StrictPartialOrder(nodes=[milk_collection, quality_testing, milk_blending, starter_culture, loop_fermentation, choice_packaging, choice_logistics])
...
root.order.add_edge(cold_shipping, order_fulfillment)
