...
skip_initial_audit = SilentTransition(label='Initial Audit')
skip_artist_review = SilentTransition(label='Artist Review')
skip_material_check = SilentTransition(label='Material Check')
skip_condition_scan = SilentTransition(label='Condition Scan')
skip_ownership_verify = SilentTransition(label='Ownership Verify')
skip_appraisal_update = SilentTransition(label='Appraisal Update')
skip_restoration_plan = SilentTransition(label='Restoration Plan')
skip_restoration_track = SilentTransition(label='Restoration Track')
skip_logistics_book = SilentTransition(label='Logistics Book')
skip_shipping_monitor = SilentTransition(label='Shipping Monitor')
skip_customs_clear = SilentTransition(label='Customs Clear')
skip_legal_compliance = SilentTransition(label='Legal Compliance')
skip_ledger_update = SilentTransition(label='Ledger Update')
skip_exhibition_setup = SilentTransition(label='Exhibition Setup')
skip_public_showcase = SilentTransition(label='Public Showcase')
skip_archival_prep = SilentTransition(label='Archival Prep')
skip_final_report = SilentTransition(label='Final Report')
