...
delivery_schedule = Transition(label='Schedule Delivery')
...
root = StrictPartialOrder(nodes=[signup, set_preferences, meal_select, delivery_schedule, supplier_match, inventory_check, ingredient_order, quality_inspect, meal_pack, route_plan, dispatch_kit, delivery_track, feedback_collect, data_analyze, plan_optimize])

# Define the dependencies
root.order.add_edge(signup, set_preferences)
root.order.add_edge(set_preferences, meal_select)
root.order.add_edge(meal_select, delivery_schedule)
root.order.add_edge(delivery_schedule, supplier_match)
