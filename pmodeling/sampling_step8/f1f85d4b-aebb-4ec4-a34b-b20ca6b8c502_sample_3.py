...
xor = OperatorPOWL(operator=Operator.XOR, children=[seller_negotiation, documentation])
loop = OperatorPOWL(operator=Operator.LOOP, children=[risk_analysis, legal_review])
xor2 = OperatorPOWL(operator=Operator.XOR, children=[condition_report, archival_entry])
xor3 = OperatorPOWL(operator=Operator.XOR, children=[committee_review, exhibit_planning])

# Define the POWL model
root = StrictPartialOrder(nodes=[initial_review, provenance_check, material_testing, expert_survey, digital_scan, condition_report, legal_review, risk_analysis, seller_negotiation, documentation, archival_entry, committee_review, final_approval, acquisition_setup, exhibit_planning])
...
root.order.add_edge(risk_analysis, legal_review)
root.order.add_edge(legal_review, xor)
...
root.order.add_edge(archival_entry, xor2)
root.order.add_edge(committee_review, exhibit_planning)
root.order.add_edge(exhibit_planning, xor3)
