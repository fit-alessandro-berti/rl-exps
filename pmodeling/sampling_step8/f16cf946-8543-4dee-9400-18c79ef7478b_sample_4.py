...
climate_study -> system_design
system_design -> seed_selection
seed_selection -> unit_install
unit_install -> sensor_setup
sensor_setup -> nutrient_mix
nutrient_mix -> energy_audit
energy_audit -> pest_control
pest_control -> crop_plan
crop_plan -> quality_check
quality_check -> yield_forecast
yield_forecast -> supply_sync
supply_sync -> staff_train
staff_train -> data_review

# Constructing the POWL model
root = StrictPartialOrder(nodes=[
    site_survey, climate_study, system_design, seed_selection, unit_install, sensor_setup,
    nutrient_mix, energy_audit, pest_control, crop_plan, quality_check, yield_forecast,
    supply_sync, staff_train, data_review
])

# Adding edges based on the process flow
