...
loop = OperatorPOWL(operator=Operator.LOOP, children=[dispute_resolve])
xor = OperatorPOWL(operator=Operator.XOR, children=[dispute_resolve, final_archive])

# Define the root node with the sequence of activities and dependencies
root = StrictPartialOrder(nodes=[discover_item, document_find, initial_survey, image_capture, material_testing, style_compare, expert_consult, provenance_check, ownership_verify, legal_review, risk_assess, conservation_plan, certification, secure_transfer, loop, xor])
...
root.order.add_edge(secure_transfer, loop)
